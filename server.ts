...
// In-Memory OTP Store for Sign-Up flow
interface OTPRecord {
  email: string;
  otp: string;
  expiresAt: number;
...
// ==========================================
// REAL-TIME EMAIL OTP SIGNUP FLOW
// ==========================================
app.post("/api/auth/request-otp", async (req, res) => {
  ...
  const existingRecord = otpStore.find(r => r.email === cleanedEmail);
  if (existingRecord && existingRecord.expiresAt > Date.now() + 4 * 60 * 1000) {
    return res.status(429).json({ error: "Please wait a minute before requesting another OTP." });
  }

  const otp = crypto.randomInt(100000, 999999).toString();

  otpStore = otpStore.filter(r => r.email !== cleanedEmail);
  otpStore.push({
    email: cleanedEmail,
    otp,
  ...
  try {
    await mailTransporter.sendMail({
      from: `"FuelPro ERP" <${process.env.SMTP_USER}>`,
      to: cleanedEmail,
      subject: `Your FuelPro Verification Code: ${otp}`,
      html: `
        <div style="font-family: sans-serif; padding: 20px; text-align: center; background: #f9f9f9;">
          <h2 style="color: #333;">Welcome to FuelPro ERP</h2>
          <p style="font-size: 16px; color: #555;">Use the following 6-digit code to verify your email address and securely complete your sign-up.</p>
          <div style="margin: 30px auto; padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 8px; width: fit-content; font-size: 32px; font-weight: bold; letter-spacing: 5px; color: #000;">
            ${otp}
          </div>
          <p style="font-size: 14px; color: #888;">This code expires in 5 minutes. If you didn't request this, you can safely ignore this email.</p>
        </div>
      `
    });
    ...
    return res.status(500).json({ error: "Failed to send OTP email. Please check your SMTP credentials." });
  }
});
...
app.post("/api/auth/verify-otp", async (req, res) => {
  ...
  const cleanedEmail = email.toLowerCase().trim();
  const recordIndex = otpStore.findIndex(r => r.email === cleanedEmail);

  if (recordIndex === -1) {
    return res.status(400).json({ error: "No active OTP request found for this email. Please request a new one." });
  }

  const record = otpStore[recordIndex];
  ...
  try {
    ...
    try {
      ...
      if (err.code === "auth/user-not-found") {
        const newUser = await adminAuthInstance.createUser({
          email: cleanedEmail,
          emailVerified: true
        });
        uid = newUser.uid;
    ...
    console.error("Failed to verify OTP & create Firebase token:", err);
    return res.status(500).json({ error: "Failed to complete verification on the backend." });
  }
});

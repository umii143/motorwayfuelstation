...
walkDir(featuresDir, (filePath) => {
  if (filePath.endsWith('.tsx') || filePath.endsWith('.jsx')) {
    ...
    // We should be careful to only replace "grid-cols-2 lg:grid-cols-3" exactly, or similar.
    // The instructions say replace `grid-cols-2 lg:grid-cols-3` to `grid-cols-1 lg:grid-cols-3`
    newContent = newContent.replace(/grid-cols-2 lg:grid-cols-3/g, 'grid-cols-1 lg:grid-cols-3');
    
    // Some might have sm:grid-cols-2 instead of grid-cols-2 but the plan says grid-cols-2 lg:grid-cols-3

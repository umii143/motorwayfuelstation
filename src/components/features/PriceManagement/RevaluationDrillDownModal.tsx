...
export default function RevaluationDrillDownModal({
 ...
 <div className="bg-subtle w-full max-w-7xl h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-border">
 
 {/* Header */}
 <div className="bg-card text-foreground px-6 py-4 flex items-center justify-between shrink-0">
 ...
 <span className="text-xs text-muted-foreground font-bold tracking-widest uppercase flex items-center gap-1">
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Total Gain</p>
 <p className="text-2xl font-black text-emerald-600">+{formatCurrency(stats.totalGain, settings)}</p>
 </div>
 <div className="premium-card p-4 border">
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Total Loss</p>
 <p className="text-2xl font-black text-rose-600">-{formatCurrency(stats.totalLoss, settings)}</p>
 </div>
 <div className="bg-card rounded-xl p-4 shadow-sm relative overflow-hidden">
 <div className="absolute -right-4 -top-4 size-16 bg-card/5 rounded-full blur-xl pointer-events-none"></div>
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Net Impact</p>
 ...
 <div className="premium-card p-4 border">
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Events Count</p>
 <p className="text-2xl font-black text-foreground">{stats.eventsCount}</p>
 </div>
 <div className="premium-card p-4 border">
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Avg Impact / Event</p>
 ...
 <div>
 <label className="text-xs font-bold text-muted-foreground mb-1 block">Date Range</label>
 <div className="flex gap-2">
 <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="premium-input text-xs p-2 bg-subtle" />
 <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="premium-input text-xs p-2 bg-subtle" />
 </div>
 </div>
 <div>
 <label className="text-xs font-bold text-muted-foreground mb-1 block">Product</label>
 <select value={productFilter} onChange={e => setProductFilter(e.target.value)} className="premium-input text-xs p-2 bg-subtle font-semibold">
 <option value="all">All Products</option>
 {uniqueProducts.map(p => <option key={p} value={p}>{p}</option>)}
 </select>
 </div>
 <div>
 <label className="text-xs font-bold text-muted-foreground mb-1 block">Impact Type</label>
 <select value={impactFilter} onChange={e => setImpactFilter(e.target.value as ImpactFilter)} className="premium-input text-xs p-2 bg-subtle font-semibold">
 ...
 <div>
 <label className="text-xs font-bold text-muted-foreground mb-1 block">Reason</label>
 <select value={reasonFilter} onChange={e => setReasonFilter(e.target.value)} className="premium-input text-xs p-2 bg-subtle font-semibold">
 <option value="all">All Reasons</option>
 {uniqueReasons.map(r => <option key={r} value={r}>{r}</option>)}
 </select>
 </div>
 <div>
 <label className="text-xs font-bold text-muted-foreground mb-1 block">Changed By</label>
 <select value={changedByFilter} onChange={e => setChangedByFilter(e.target.value)} className="premium-input text-xs p-2 bg-subtle font-semibold">
 ...
 <div className="premium-card border border-border space-y-4">
 ...
 <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4 text-center">Impact Distribution (Gain/Loss by Product)</h4>
 ...
 <div className="premium-card p-5 border h-72">
 <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4 text-center">Monthly Net Revaluation Trend</h4>
 <ResponsiveContainer width="100%" height="100%">
 ...
 <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-subtle shrink-0">
 <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
 <Layers className="h-4 w-4 text-muted-foreground" /> Enterprise Revaluation Ledger
 </h3>
 <div className="flex items-center gap-2">
 <button className="px-3 py-1.5 bg-card border border-border text-muted-foreground rounded-lg text-xs font-bold hover:bg-slate-50 flex items-center gap-2">
 ...
 return (
 <tr key={entry.id} className="border-b border-border hover:bg-slate-50 transition-colors group">
 <td className="p-3 align-top">
 <p className="font-bold text-foreground">{entry.effectiveDate || entry.date}</p>
 <p className="text-[10px] text-muted-foreground">{entry.effectiveTime || '12:00'}</p>
 <div className="mt-1 flex items-center gap-1">
 <User className="h-3 w-3 text-muted-foreground" />
 <p className="text-[10px] font-semibold text-muted-foreground">{entry.changedBy || 'System'}</p>
 </div>
 </td>
 <td className="p-3 align-top">
 <p className="font-bold text-foreground">{entry.productName}</p>
 <span className="inline-block mt-1 px-2 py-0.5 bg-muted text-muted-foreground rounded text-[9px] font-bold uppercase tracking-wider">Fuel</span>
 </td>
 <td className="p-3 align-top">
 <div className="flex items-center gap-2">
 <span className="text-muted-foreground line-through">{oldRate.toFixed(2)}</span>
 <span className="text-muted-foreground">→</span>
 <span className="font-bold text-foreground">{newRate.toFixed(2)}</span>
 </div>
 ...
 <td className="p-3 align-top">
 <p className="font-bold text-foreground">{stock.toLocaleString()} L</p>
 <p className="text-[10px] text-muted-foreground mt-1">Value B/A: N/A</p> {/* Future Integration */}
 </td>
 ...
 {entry.notes && <p className="text-[10px] text-muted-foreground mt-1 truncate max-w-full max-w-[150px]" title={entry.notes}>{entry.notes}</p>}
 </td>
 <td className="p-3 align-top text-center">
 <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
 <button className="p-1.5 bg-muted hover:bg-slate-200 text-muted-foreground rounded-lg transition-colors" title="View Snapshot">

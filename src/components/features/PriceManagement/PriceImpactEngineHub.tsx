...
import { 
  ...
  FileText, Clock, Bell, Sparkles, CheckCircle2, ArrowUpRight, ArrowDownRight,
  Database, RefreshCw, Zap, Table, BarChart3, Receipt, Scale, AlertTriangle,
  Search, Calendar, Filter, Download, Printer, X, ChevronRight, ChevronDown,
...
export function PriceImpactEngineHub() {
  const { snapshots, fuelPrices, publishQuickSnapshot, approveRevision, publishRevision } = usePricingStore();
  ...
  // Collapsible Secondary Accordions
  const [showTaxBuildUp, setShowTaxBuildUp] = useState(false);
  const [showHardwareSync, setShowHardwareSync] = useState(false);

  // Quick Publish Modal State
  const [isRevisionModalOpen, setIsRevisionModalOpen] = useState(false);
  const [targetProduct, setTargetProduct] = useState('Petrol (Super)');
  const [oldRate, setOldRate] = useState(270.00);
  const [newRate, setNewRate] = useState(273.50);
  const [publisher, setPublisher] = useState('Station Owner');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Active Real Snapshots from Store
  ...
  // Display Limit: First 10 rows unless "Show Full Register" is toggled
  ...
  // Handle Row Click -> Open Slide-Over Detail Drawer
  ...
  const handlePublishNewSnapshot = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    try {
      const snap = await publishQuickSnapshot('prod_petrol_01', targetProduct, oldRate, newRate, publisher);
      setSelectedSnapshot(snap);
      setIsRevisionModalOpen(false);
      setIsDrawerOpen(true);
    } catch (err: any) {
      alert(`Error publishing revision: ${err.message}`);
    } finally {
  ...
  const handleExportCSV = () => {
    if (!filteredSnapshots.length) return alert('No snapshot records to export.');
    const headers = ['Version', 'Date', 'Time', 'Circular', 'Product', 'Old Rate', 'New Rate', 'Difference', 'Stock (L)', 'Inventory Gain', 'Status'];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* TOP TITLE BAR */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[var(--bg-card)] border border-[var(--border-main)] p-5 rounded-3xl shadow-sm">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-bold mb-1.5">
            <Database className="w-3.5 h-3.5" />
            <span>Single Source of Truth (SSOT) Architecture</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-[var(--text-main)] tracking-tight flex items-center gap-2.5">
            Price Revision Impact Engine & Register
          </h1>
          <p className="text-xs text-[var(--text-muted)] mt-0.5">
            SAP IS-Oil & Oracle NetSuite Compliant • Driven by single immutable price revision snapshots
          </p>
        </div>

        <button
          onClick={() => setIsRevisionModalOpen(true)}
          className="px-5 py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold text-xs shadow-lg shadow-emerald-500/20 flex items-center gap-2 transition-all transform active:scale-95 shrink-0"
        >
          <Zap className="w-4 h-4 fill-slate-950" />
          <span>Publish OGRA Price Change</span>
        </button>
      </div>

      {/* 1. TOP 4 CLEAN NON-DUPLICATED KPIS (ALL INTERACTIVE & CLICKABLE) */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        
        {/* KPI 1: CURRENT RATE (Click to open Publish Modal) */}
        <button
          onClick={() => setIsRevisionModalOpen(true)}
        ...
        {/* KPI 2: LATEST REVISION (Click to open Latest Snapshot Drawer) */}
        <button
          onClick={() => {
            if (latestSnap) handleRowClick(latestSnap);
            else setIsRevisionModalOpen(true);
          }}
        ...
        >
          ...
          <div className="text-[10px] text-[var(--text-muted)] mt-1">
            {latestSnap ? latestSnap.effectiveDate : 'Awaiting OGRA Circular'}
          </div>
        </button>

        {/* KPI 3: INVENTORY GAIN (Click to open Ledger Impact Tab) */}
        <button
          onClick={() => {
            if (latestSnap) {
        ...
        >
          <div className="text-xs text-[var(--text-muted)] font-medium flex items-center justify-between">
            <span>Inventory Stock Gain</span>
            <ChevronRight className="w-3.5 h-3.5 text-[var(--text-muted)] group-hover:text-emerald-500 transition-colors" />
          </div>
          <div className="text-xl font-black text-emerald-500 mt-1">
          ...
          <div className="text-[10px] text-emerald-500/80 mt-1">
            {latestSnap ? 'Journal Entry Posted' : 'No Revaluation Recorded'}
          </div>
        </button>

        {/* KPI 4: PENDING REVISION (Click to open Pending Approvals Modal) */}
        <button
          onClick={() => setIsApprovalsModalOpen(true)}
          className="bg-[var(--bg-card)] border-2 border-amber-500/40 hover:border-amber-500 rounded-2xl p-4 shadow-md text-left transition-all hover:-translate-y-0.5 group cursor-pointer animate-pulse-subtle relative overflow-hidden"
        >
          <div className="text-xs text-amber-600 dark:text-amber-400 font-bold flex items-center justify-between">
            <span>Pending Approvals</span>
            <span className="text-[10px] bg-amber-500/20 px-2 py-0.5 rounded-full text-amber-500 font-black">Approve →</span>
          </div>
      ...
      {/* SAP / ORACLE AUDIT LIFECYCLE TIMELINE */}
      <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-4 shadow-sm overflow-x-auto">
        <div className="text-xs font-bold text-[var(--text-main)] mb-3 flex items-center gap-2">
          <span>🔄</span>
          <span>SAP / Oracle Price Revision Audit Lifecycle</span>
        </div>
        <div className="flex items-center justify-between min-w-max gap-3 text-[11px]">
          {[
            ...
            { step: '5', title: 'POS Synced', desc: 'Pumps & Registers', done: !!latestSnap },
      ...
      {/* AI RECOMMENDATION CARD (CLEAN IF NO DATA) */}
      <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/30 rounded-2xl p-4 flex items-center gap-3">
        <Sparkles className="w-5 h-5 text-amber-400 shrink-0 animate-pulse" />
        <div className="text-xs">
          <span className="font-bold text-amber-300">AI Operational Insights: </span>
          <span className="text-amber-100/90">
            {latestSnap ? latestSnap.aiInsights.recommendation : 'No recommendation available. Waiting for first published revision.'}
          </span>
        </div>
      </div>

      {/* SEARCH, DATE PRESETS, FILTERS & EXPORT TOOLBAR */}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-xs text-[var(--text-main)] focus:outline-none focus:border-emerald-500"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-main)] hover:bg-[var(--bg-hover)] text-xs font-bold flex items-center gap-1.5"
            >
      ...
      {/* ENTERPRISE PRICE REVISION REGISTER TABLE (FIRST 10 ROWS DEFAULT) */}
        ...
        <div className="flex items-center justify-between border-b border-[var(--border-main)] pb-3">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)]">
              Showing {displayedSnapshots.length} of {filteredSnapshots.length} snapshot records. Click any row to view full 6-tab details.
            </p>
          </div>
        ...
        {!hasSnapshots ? (
          <div className="flex flex-col items-center justify-center p-12 bg-[var(--bg-subtle)] rounded-2xl border border-dashed border-[var(--border-main)] text-center space-y-3">
            ...
            <p className="text-xs text-[var(--text-muted)] max-w-md">
              No price revision snapshots have been published yet. Publish an OGRA rate update to generate the first SSOT snapshot.
            </p>
            <button
              onClick={() => setIsRevisionModalOpen(true)}
              className="px-4 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold shadow-md shadow-emerald-600/20"
            >
              Publish OGRA Price Revision
            </button>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[var(--border-main)]">
                {displayedSnapshots.map((snap) => (
                  ...
                  >
                    <td className="p-3.5 font-black text-emerald-500 font-mono flex items-center gap-1.5">
                      <span>{snap.versionLabel}</span>
                    </td>
      ...
      <div className="space-y-3">
        {/* COLLAPSIBLE 1: TAX & LEVY BUILD-UP */}
        <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl overflow-hidden shadow-sm">
          <button
            onClick={() => setShowTaxBuildUp(!showTaxBuildUp)}
            className="w-full p-4 flex items-center justify-between font-bold text-xs text-[var(--text-main)] hover:bg-[var(--bg-subtle)] transition-colors"
          >
            <div className="flex items-center gap-2">
              <span>🏛️</span>
              <span>Official OGRA Tax & Petroleum Levy Build-up (Secondary View)</span>
            </div>
            <ChevronDown className={`w-4 h-4 transition-transform ${showTaxBuildUp ? 'rotate-180' : ''}`} />
          </button>
          {showTaxBuildUp && (
            <div className="p-4 border-t border-[var(--border-main)]">
              <TaxLevyBreakdownWidget isUrdu={false} petrolPrice={latestSnap?.newRate || 273.50} dieselPrice={297.20} />
            </div>
          )}
        </div>

        {/* COLLAPSIBLE 2: HARDWARE SYNC MONITOR */}
        <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl overflow-hidden shadow-sm">
          <button
            onClick={() => setShowHardwareSync(!showHardwareSync)}
            className="w-full p-4 flex items-center justify-between font-bold text-xs text-[var(--text-main)] hover:bg-[var(--bg-subtle)] transition-colors"
          >
            <div className="flex items-center gap-2">
              <span>⚡</span>
              <span>Pump Controller & Hardware Controller Sync Monitor (Secondary View)</span>
            </div>
            <ChevronDown className={`w-4 h-4 transition-transform ${showHardwareSync ? 'rotate-180' : ''}`} />
          </button>
          {showHardwareSync && (
            <div className="p-4 border-t border-[var(--border-main)]">
              <PumpControllerSyncStatusWidget isUrdu={false} petrolRate={latestSnap?.newRate || 273.50} dieselRate={297.20} />
            </div>
          )}
        </div>
      </div>

      {/* SLIDE-OVER RIGHT DETAIL DRAWER (ROW CLICK) */}
      <AnimatePresence>
        ...
        {isRevisionModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <motion.div 
              ...
              className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-3xl p-6 w-full max-w-lg shadow-2xl space-y-5"
            >
              <div className="flex items-center justify-between border-b border-[var(--border-main)] pb-4">
                <div className="flex items-center gap-2.5">
                  <Zap className="w-5 h-5 text-emerald-500" />
                  <h3 className="text-lg font-black text-[var(--text-main)]">Publish OGRA Price Revision</h3>
                </div>
                <button onClick={() => setIsRevisionModalOpen(false)} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xl font-bold">×</button>
              </div>

              <form onSubmit={handlePublishNewSnapshot} className="space-y-4 text-xs">
                <div>
                  <label className="block font-semibold text-[var(--text-main)] mb-1">Fuel Product</label>
                  <select 
                    value={targetProduct} 
                    onChange={(e) => setTargetProduct(e.target.value)}
                    className="w-full p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-main)] font-semibold"
                  >
                    <option value="Petrol (Super)">Petrol (Super)</option>
                    <option value="High Speed Diesel">High Speed Diesel</option>
                    <option value="HOBC (Octane 97)">HOBC (Octane 97)</option>
                  </select>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block font-semibold text-[var(--text-main)] mb-1">Old Rate (Rs.)</label>
                    <input 
                      type="number" 
                      step="0.01"
                      value={oldRate} 
                      onChange={(e) => setOldRate(parseFloat(e.target.value) || 0)}
                      className="w-full p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-main)] font-mono font-bold"
                    />
                  </div>
                  <div>
                    <label className="block font-semibold text-[var(--text-main)] mb-1">New Rate (Rs.)</label>
                    <input 
                      type="number" 
                      step="0.01"
                      value={newRate} 
                      onChange={(e) => setNewRate(parseFloat(e.target.value) || 0)}
                      className="w-full p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-main)] font-mono font-bold text-emerald-500"
                    />
                  </div>
                </div>

                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                  <div className="font-semibold">Calculated Revaluation Preview</div>
                  <div className="text-sm font-black mt-1">
                    Rate Delta: +Rs. {(newRate - oldRate).toFixed(2)} | Stock Gain: +Rs. {(30700 * (newRate - oldRate)).toLocaleString()}
                  </div>
                </div>

                <div>
                  <label className="block font-semibold text-[var(--text-main)] mb-1">Publisher</label>
                  <input 
                    type="text" 
                    value={publisher} 
                    onChange={(e) => setPublisher(e.target.value)}
                    className="w-full p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] text-[var(--text-main)]"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-3 border-t border-[var(--border-main)]">
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl border border-[var(--border-main)] text-[var(--text-muted)] font-semibold hover:bg-[var(--bg-subtle)]"
                  >
                    Cancel
                  </button>
                  <button
                  ...
                  >
                    {isSubmitting ? 'Generating SSOT Snapshot...' : 'Publish & Generate SSOT Snapshot'}
                  </button>
                </div>
              </form>
            </motion.div>
        ...
        {isApprovalsModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-3xl p-6 w-full max-w-2xl shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between border-b border-[var(--border-main)] pb-4">

...
export default function PriceManagement(props: PriceManagementProps) {
 ...
 className="fixed top-0 right-0 h-full w-full max-w-4xl bg-background z-[101] shadow-2xl overflow-y-auto border-l border-border"
 >
 <div className="sticky top-0 z-10 flex justify-between items-center p-6 bg-card/80 /80 backdrop-blur-md border-b border-border">
 <h2 className="text-xl font-bold text-foreground">Update Prices & Rates</h2>
 <button 
 onClick={() => setIsUpdateDrawerOpen(false)}
 className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors text-muted-foreground"
 >
 <X className="w-6 h-6" />
 </button>
 </div>

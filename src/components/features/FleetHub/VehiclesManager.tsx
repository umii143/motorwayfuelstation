...
export default function VehiclesManager({ settings, stationId }: VehiclesManagerProps) {
 ...
 <div className="font-bold text-foreground font-mono text-base truncate">{vehicle.registrationNumber}</div>
 <div className="text-xs text-muted-foreground truncate">{vehicle.make} {vehicle.model} • {vehicle.category}</div>
 </div>
 </div>
 ),
 ...
 <span className="font-medium text-foreground truncate">{getAccountName(vehicle.accountId)}</span>
 </div>
 )
 },
 {
 ...
 <span className="text-muted-foreground text-xs italic">No tag assigned</span>
 )
 )
 },
 {
 header: 'Monthly Limit / Used',
 accessor: (vehicle) => {
 ...
 <span className="font-bold text-foreground">{vehicle.currentMonthConsumption} L</span>
 <span className="text-muted-foreground">/ {vehicle.monthlyFuelLimit} L</span>
 </div>
 ...
 className="text-orange-600 hover:text-orange-800 font-bold text-xs bg-orange-50 hover:bg-orange-100 px-3 py-1.5 rounded-lg transition cursor-pointer"
 >
 Edit
 </button>
 )
 ...
 <label className="block text-xs font-bold text-foreground mb-1">Corporate Account *</label>
 <select value={accountId} onChange={e => setAccountId(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-card">
 {accounts.map(acc => (
 <option key={acc.id} value={acc.id}>{acc.companyName}</option>
 ))}
 </select>
 ...
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">Registration Number *</label>
 <input type="text" value={registrationNumber} onChange={e => setRegistrationNumber(e.target.value.toUpperCase())} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 font-mono uppercase" placeholder="ABC-1234" />
 { }
 </div>
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">Vehicle Category</label>
 {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
 <select value={category} onChange={e => setCategory(e.target.value as any)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-card">
 ...
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">Make *</label>
 <input type="text" value={make} onChange={e => setMake(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="e.g. Toyota, Hino" />
 </div>
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">Model *</label>
 <input type="text" value={model} onChange={e => setModel(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="e.g. Corolla, 300 Series" />
 </div>
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">RFID Tag Number (Optional)</label>
 <div className="relative flex gap-2">
 <div className="relative flex-1">
 <Tag className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-indigo-400" />
 <input type="text" value={rfidTag} onChange={e => setRfidTag(e.target.value.toUpperCase())} className="w-full pl-9 pr-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono bg-indigo-50/30" placeholder="Scan or enter Tag ID" />
 </div>
 <button 
 onClick={async () => {
 ...
 <div>
 <label className="block text-xs font-bold text-foreground mb-1">Monthly Fuel Limit (Liters) *</label>
 { }
 <input type="number" value={monthlyFuelLimit} onChange={e => setMonthlyFuelLimit(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="1000" />
 </div>
 <div className="md:col-span-2">
 <label className="block text-xs font-bold text-foreground mb-1">Status</label>
 {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
 <select value={status} onChange={e => setStatus(e.target.value as any)} className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-card">

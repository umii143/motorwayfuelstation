...
export default function CustomerDetailsFullPage({ account, settings, shifts, banks, vehicles, drivers, onBack }: CustomerDetailsFullPageProps) {
 ...
 <span className="text-xl font-bold text-muted-foreground">{account.companyName.substring(0, 2).toUpperCase()}</span>
 )}
 </div>
 <div>
 ...
 {/* TABS */}
 <div className="border-b border-border px-4 lg:px-8 mt-6">
 ...
 <h3 className="text-white font-semibold mb-4 pb-3 border-b border-border">Sales Summary (This Year)</h3>
 ...
 <div className="flex justify-between items-center">
 <span className="text-muted-foreground text-sm">Total Recoveries</span>
 ...
 <input type="text" placeholder="Search transactions..." className="bg-card border border-border text-foreground text-xs rounded-lg px-3 py-2 focus:outline-hidden" />
 </div>
 ...
 {transactions.length === 0 && (
 <tr>
 <td colSpan={10} className="px-4 py-8 text-center text-muted-foreground italic">No transactions found for this customer.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 <div key={v.id} className="p-4 bg-card rounded-lg border border-border">
 <div className="font-bold text-white">{v.registrationNumber}</div>
 <div className="text-sm text-muted-foreground mt-1">{v.make} {v.model} ({v.category})</div>
 <div className="mt-3 flex gap-2 text-xs">
 <span className="px-2 py-1 bg-card rounded-md text-muted-foreground">Monthly: {settings.currency} {v.monthlyFuelLimit}</span>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="text-sm text-muted-foreground mt-1">License: {d.licenseNumber}</div>
 <div className="text-sm text-muted-foreground">Phone: {d.phone}</div>
 </div>
 ))}
 </div>
 ) : (

...
export default function AccountsManager({ settings, stationId }: AccountsManagerProps) {
 ...
 <div>
 <p className="text-muted-foreground text-xs font-semibold mb-1">Total Corporate</p>
 <h3 className="text-white text-2xl font-bold">{accounts.length}</h3>
 </div>
 </div>
 <div className="flex justify-between text-xs">
 <span className="text-muted-foreground">Active <span className="text-emerald-400 ml-1 font-semibold">{activeAccountsCount}</span></span>
 <span className="text-muted-foreground">Suspended <span className="text-amber-400 ml-1 font-semibold">{suspendedAccountsCount}</span></span>
 </div>
 </div>
 ...
 <div>
 <p className="text-muted-foreground text-xs font-semibold mb-1">Total Receivables</p>
 <h3 className="text-white text-2xl font-bold">PKR {formatCurrency(totalReceivables, settings).replace('Rs.', '').trim()}</h3>
 </div>
 </div>
 <div className="flex flex-col text-xs">
 <span className="text-muted-foreground mb-1">Credit Limit Utilized</span>
 ...
 <div>
 <p className="text-muted-foreground text-xs font-semibold mb-1">Total Sales (MTD)</p>
 <h3 className="text-white text-2xl font-bold">PKR {formatCurrency(salesMTD, settings).replace('Rs.', '').trim()}</h3>
 </div>
 </div>
 <div className="flex items-center text-xs">
 <span className="text-muted-foreground mr-2">This Month</span>
 ...
 <div>
 <p className="text-muted-foreground text-xs font-semibold mb-1">Avg Credit Limit</p>
 ...
 className="w-full bg-card border border-border text-foreground text-sm rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:border-slate-600 transition-colors"
 />
 </div>
 <button 
 onClick={() => setStatusFilter(statusFilter === 'All Status' ? 'Active' : statusFilter === 'Active' ? 'Suspended' : 'All Status')}
 className="flex items-center gap-2 bg-card border border-border text-muted-foreground px-4 py-2.5 rounded-lg text-sm hover:bg-slate-800 transition-colors"
 >
 {statusFilter} <ChevronDown className="w-4 h-4" />
 </button>
 </div>
 <div className="flex items-center gap-3">
 <button className="flex items-center gap-2 bg-card border border-border text-muted-foreground px-4 py-2.5 rounded-lg text-sm hover:bg-slate-800 transition-colors">
 ...
 className="bg-card border border-border rounded-xl p-5 hover:border-slate-600 transition-colors cursor-pointer group flex flex-col"
 >
 <div className="flex items-start justify-between mb-6">
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-full bg-card flex items-center justify-center border border-border overflow-hidden shadow-inner">
 {logoUrl ? (
 <img src={logoUrl} alt={acc.companyName} className="w-full h-full object-contain p-1.5" />
 ) : (
 <span className="text-sm font-bold text-muted-foreground">{acc.companyName.substring(0, 2).toUpperCase()}</span>
 )}
 </div>
 <div>
 <h4 className="text-white font-bold text-sm truncate max-w-[140px]">{acc.companyName}</h4>
 <p className="text-muted-foreground text-[11px] mt-0.5">Corporate Fleet</p>
 </div>
 </div>
 <span className={`px-2 py-0.5 rounded text-[10px] font-bold${isActive ? 'bg-emerald-500/10 text-emerald-500' : acc.status === 'suspended' ? 'bg-amber-500/10 text-amber-500' : 'bg-card text-slate-500'}`}>
 {acc.status}
 </span>
 </div>
 ...
 <div>
 <p className="text-muted-foreground text-[10px] uppercase font-semibold mb-1">Receivable</p>
 ...
 <div>
 <p className="text-muted-foreground text-[10px] uppercase font-semibold mb-1">Credit Limit</p>
 <p className="text-white text-sm font-bold">
 PKR {formatCurrency(acc.creditLimit || 0, settings).replace('Rs.', '').trim()}
 </p>
 </div>
 <div className="col-span-2">
 <p className="text-muted-foreground text-[10px] uppercase font-semibold mb-1">Contact Details</p>
 <div className="flex items-center justify-between text-white text-xs font-semibold">
 <span className="flex items-center gap-1"><UsersRound className="w-3 h-3 text-muted-foreground"/> {acc.contactPerson}</span>
 ...
 <p className="text-xs text-muted-foreground">Adjust your search or add a new customer.</p>
 </div>
 )}
 ...
 className="bg-card rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-border flex flex-col max-h-[90vh]"
 >
 ...
 <button onClick={() => setIsModalOpen(false)} className="text-muted-foreground hover:text-rose-400 transition-colors">
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Company Name *</label>
 <input type="text" value={companyName} onChange={e => setCompanyName(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="e.g. DHL Express" />
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Contact Person *</label>
 <input type="text" value={contactPerson} onChange={e => setContactPerson(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="Name of Fleet Manager" />
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Phone Number *</label>
 <input type="text" value={phone} onChange={e => setPhone(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="03xx-xxxxxxx" />
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Email Address</label>
 <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="billing@company.com" />
 </div>
 <div className="md:col-span-2">
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Office Address</label>
 <input type="text" value={address} onChange={e => setAddress(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="Company HQ Address" />
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Credit Limit ({settings.currency}) *</label>
 <input type="number" value={creditLimit} onChange={e => setCreditLimit(e.target.value)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500" placeholder="500000" />
 </div>
 { }
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1.5">Account Status</label>
 {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
 <select value={status} onChange={e => setStatus(e.target.value as any)} className="w-full px-3 py-2.5 bg-background border border-border text-white rounded-lg text-sm focus:outline-none focus:border-orange-500">
 ...
 <button onClick={() => setIsModalOpen(false)} className="px-4 py-2.5 text-sm font-bold text-muted-foreground hover:text-white transition-colors">Cancel</button>

...
export default function BillingAndReports({ settings, stationId }: BillingAndReportsProps) {
 ...
 <span className="text-xs font-bold text-muted-foreground uppercase">Total Billed</span>
 <span className="text-2xl font-black font-mono text-rose-600 mt-1">{settings.currency} {totalConsumption.toLocaleString()}</span>
 </div>
 <div className="premium-card p-4 border flex flex-col justify-center">
 <span className="text-xs font-bold text-muted-foreground uppercase">Total Recovered</span>
 ...
 <span className={`text-2xl font-black font-mono mt-1${totalConsumption - totalPayments > 0 ? 'text-rose-600' : 'text-foreground'}`}>
 ...
 <button 
 className="flex items-center gap-1.5 bg-card text-foreground px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-slate-800 transition whitespace-nowrap shrink-0 w-full sm:w-auto justify-center"
 >
 ...
 <span className="font-bold text-foreground text-[11px]">{new Date(txn.date).toLocaleDateString()}</span>
 <span className="text-[10px] text-muted-foreground">{new Date(txn.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
 </div>
 ),
 ...
 header: 'Account',
 accessor: (txn) => (
 <span className="font-medium text-[11px] text-foreground truncate block max-w-[120px]">
 ...
 <div className="flex flex-col">
 <span className="font-bold text-foreground text-[11px]">{txn.quantity}L {txn.productId === 'p_pmg' ? 'Petrol' : 'Diesel'}</span>
 <span className="text-[10px] text-muted-foreground">Rate: {txn.rate}/L</span>
 </div>
 ) : (
 <div className="flex items-center gap-1.5">
 <Receipt className="h-3.5 w-3.5 text-emerald-500" />
 <div className="flex flex-col">
 <span className="font-bold text-foreground text-[11px]">Payment</span>
 <span className="text-[10px] text-muted-foreground uppercase">{txn.paymentMode}</span>
 </div>
 </div>
 )
 )

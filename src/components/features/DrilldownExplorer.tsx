...
export default function DrilldownExplorer({ settings, initialView, onClose }: DrilldownExplorerProps) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-[#0f0f15] min-h-screen p-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4 mb-6">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-1 mt-2 text-xs font-extrabold text-slate-400">
            ...
            {viewStack.map((view, vIdx) => (
              <React.Fragment key={vIdx}>
                ...
                <span 
                  ...
                  className={`cursor-pointer hover:underline ${vIdx === viewStack.length - 1 ? 'text-slate-800 dark:text-slate-200 underline' : 'text-slate-500'}`}
                >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 bg-slate-800 text-white text-xs font-bold px-3 py-1.8 rounded-lg hover:bg-slate-900 transition-colors cursor-pointer"
          >
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5 cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-xl p-5 shadow-xs space-y-4">
          <div className="space-y-1">
          ...
          <div className="border-t border-slate-100 dark:border-white/5 pt-3 space-y-2 text-xs font-semibold text-slate-600 dark:text-slate-350">
            <div className="flex justify-between">
              <span>{t('Running Records Count', 'رکارڈز کل تعداد')}:</span>
              <span className="font-mono font-bold text-slate-900 dark:text-white">{filteredRows.length}</span>
            </div>
            {Object.keys(columnSums).map(key => {
              const head = compiledData.headers.find(h => h.key === key);
              return (
                <div key={key} className="flex justify-between border-t border-slate-50 dark:border-white/5 pt-2">
          ...
          <div className="rounded-lg bg-orange-50/20 border border-orange-100 dark:border-orange-500/10 p-3 text-[11px] text-slate-555 leading-relaxed">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-hidden"
            />
          </div>
          ...
          <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden shadow-xs">
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="w-full text-left border-collapse text-xs">
                {/* Sticky Header */}
                <thead className="sticky top-0 bg-slate-50 dark:bg-[#191928] border-b border-slate-200 dark:border-white/10 z-10 font-bold text-slate-700 dark:text-slate-200">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5 font-semibold">
                  {filteredRows.length === 0 ? (
                    <tr>
                      <td colSpan={compiledData.headers.length + 1} className="p-8 text-center text-slate-400 italic">
                    ...
                    filteredRows.map((row, rIdx) => (
                      <tr 
                        ...
                        className="hover:bg-orange-50/10 dark:hover:bg-white/5 cursor-pointer transition-colors even:bg-slate-50/40 dark:even:bg-white/2"
                      >
                        <td className="p-2 w-[50px] text-center font-mono text-slate-400 border-r border-slate-100 dark:border-white/5">
                          {rIdx + 1}
                        </td>
                        {compiledData.headers.map(head => {
                          ...
                          return (
                            <td key={head.key} className={`p-2 ${isNum ? 'text-right font-mono font-bold text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-350'}`}>
                              {isNum ? (
                                typeof val === 'number' ? (
            ...
            {filteredRows.length > 0 && Object.keys(columnSums).length > 0 && (
              <div className="bg-slate-900 text-white p-3.5 border-t border-slate-800 flex justify-between items-center text-xs font-black">
      ...
      {selectedTxDetail && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-end animate-fade-in">
          <div className="bg-white dark:bg-[#151521] w-full max-w-lg h-screen flex flex-col border-l border-slate-200 dark:border-white/10 animate-slide-in">
            {/* Header */}
            <div className="h-16 border-b border-slate-200 dark:border-white/10 flex items-center justify-between px-6 bg-slate-50 dark:bg-white/5">
              <h3 className="font-black text-sm text-slate-900 dark:text-white capitalize">
                {selectedTxDetail.type || 'Transaction Voucher Detail'}
              </h3>
              <button
                onClick={() => setSelectedTxDetail(null)}
                className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs font-semibold">
              <div className="bg-orange-50/20 border border-orange-100 dark:border-orange-500/10 rounded-xl p-4 space-y-3">
                ...
                {Object.keys(selectedTxDetail).map(key => {
                  ...
                  return (
                    <div key={key} className="flex justify-between border-b border-slate-100/50 dark:border-white/2 pb-1.5 capitalize text-slate-750 dark:text-slate-250">
              ...
              <div className="space-y-2">
                ...
                <div className="border border-slate-200 dark:border-white/10 rounded-lg overflow-hidden">
                  <div className="bg-slate-50 dark:bg-white/5 p-2 grid grid-cols-3 font-bold border-b border-slate-200 dark:border-white/10">
                    <span>Account</span>
                  ...
                  {selectedTxDetail.amount !== undefined ? (
                    <div className="divide-y divide-slate-150/40 p-2 font-mono">
                      <div className="grid grid-cols-3 py-1 text-slate-800 dark:text-slate-200">
            ...
            <div className="h-16 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 flex items-center justify-end px-6">
              <button 
                onClick={() => window.print()}
                className="flex items-center gap-1 bg-slate-800 text-white px-4 py-2 rounded-lg font-bold hover:bg-slate-900 cursor-pointer"
              >

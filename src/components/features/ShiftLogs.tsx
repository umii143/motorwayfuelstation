...
export default function ShiftLogs({
 ...
 <button className="flex items-center gap-2 px-4 py-2 border border-border bg-card text-foreground rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
 <Download className="w-4 h-4" /> Export
 </button>
 <div className="flex items-center px-4 py-2 border border-border bg-card text-foreground rounded-lg text-sm font-medium shadow-sm">
 <Calendar className="w-4 h-4 mr-2" />
 17 May 2025 - 17 Jun 2025
 </div>
 <button className="p-2 border border-border bg-card text-foreground rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
 ...
 <div className="flex items-start gap-3 bg-card/5 border border-border rounded-lg p-3 backdrop-blur-sm">
 ...
 <div className="text-4xl font-black text-foreground mb-1">{aggregates.avgSHI}%</div>
 <div className="text-sm font-bold text-muted-foreground">Station Health Index</div>
 <div className="mt-4 px-3 py-1 rounded-full bg-muted text-xs font-bold text-muted-foreground">
 ...
 className="w-full pl-9 pr-4 py-2 bg-muted border border-border rounded-lg text-sm text-foreground placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500"
 />
 </div>
 ...
 className="bg-muted border border-border rounded-lg text-sm px-3 py-2 text-foreground outline-none focus:ring-2 focus:ring-orange-500 flex-1 sm:flex-initial min-w-[110px]"
 >
 ...
 className="flex items-center gap-2 px-3 py-2 text-muted-foreground hover:text-slate-700 dark:hover:text-slate-200 text-sm font-medium transition-colors border border-transparent hover:border-border dark:hover:border-slate-700 rounded-lg"
 >
 ...
 <div className="bg-background text-xs font-black uppercase tracking-widest text-muted-foreground border-b border-border flex items-center w-full">
 ...
 <div 
 className="divide-y divide-border dark:divide-slate-800 relative block min-w-[850px]"
 ...
 <h3 className="text-lg font-bold text-foreground mb-1">No shift logs found</h3>
 ...
 className={`cursor-pointer transition-colors absolute w-full flex items-center min-w-[850px]${isSelected ? 'bg-orange-50 dark:bg-orange-500/10' : 'hover:bg-slate-50 dark:hover:bg-card/50'}`}
 style={{
 top: 0,
 left: 0,
 ...
 >
 <div className="px-6 py-4 text-sm font-bold text-foreground w-[15%] shrink-0 truncate">#{shift.id.slice(-5)}</div>
 <div className="px-6 py-4 text-sm text-muted-foreground w-[15%] shrink-0 truncate">{shift.type === 'day' ? 'Day Shift' : 'Night Shift'}</div>
 <div className="px-6 py-4 text-sm text-muted-foreground w-[20%] shrink-0 truncate">{getStaffName(shift.staffId)}</div>
 <div className="px-6 py-4 text-sm text-muted-foreground w-[20%] shrink-0 truncate">{shift.date} <span className="text-muted-foreground text-xs ml-1">{shift.startTime}</span></div>
 <div className="px-6 py-4 text-sm font-bold text-foreground w-[15%] shrink-0 truncate">{formatCurrency(shiftSales).replace('PKR', '').trim()}</div>
 <div className="px-6 py-4 text-sm font-bold text-foreground w-[15%] shrink-0 truncate">{formatCurrency(shift.submittedCash || 0).replace('PKR', '').trim()}</div>
 <div className="px-6 py-4 text-center w-[15%] shrink-0 truncate">
 {shift.status === 'closed' ? (
 <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 uppercase tracking-widest">CLOSED</span>
 ) : (
 <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-600 border border-amber-500/30 uppercase tracking-widest">OPEN</span>
 )}
 </div>
 </div>
 );
 ...
 className="px-4 py-1.5 bg-muted hover:bg-slate-200 dark:hover:bg-slate-700 text-foreground rounded-full font-bold transition-colors"
 >
 Load More
 </button>
 )}
 ...
 <div className="w-80 lg:w-96 bg-background border-l border-border hidden lg:flex flex-col items-center justify-center p-8 text-center shrink-0">
 <FolderOpen className="w-16 h-16 mb-4 text-muted-foreground" />
 <h3 className="text-lg font-bold text-foreground mb-2">Shift Details</h3>
 <p className="text-sm text-muted-foreground">Select a shift from the list to view full details and drill-down information.</p>
 </div>
 )}
...
function KpiCard({ icon, iconBg, border, title, value, onClick }: any) {
 return (
 <div onClick={onClick} className={`bg-card rounded-xl border border-border p-4 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow cursor-pointer group`}>
 <div className="flex items-center gap-3 mb-4">
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center border${iconBg}${border}`}>
 {icon}
 </div>
 <div className="text-xs font-bold text-muted-foreground group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors">{title}</div>
 </div>
 <div>
 <div className="text-xl font-bold text-foreground truncate group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors">{value.replace('PKR', '').trim()}</div>
 </div>
 <div className="mt-3 pt-3 border-t border-border/50 flex items-center gap-1 text-[10px] text-muted-foreground uppercase tracking-widest font-bold group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors">
 { }
...
function GlobalTransactionModal({ title, onClose, items, columns }: any) {
 return (
 <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-card backdrop-blur-sm animate-in fade-in duration-200">
 <div className="w-full max-w-4xl bg-card rounded-2xl shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden border border-border">
 <div className="flex items-center justify-between p-5 border-b border-border bg-background">
 <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
 <FolderOpen className="w-5 h-5 text-orange-500" />
 {title}
 <span className="px-2 py-0.5 rounded-full bg-slate-200 text-[10px] font-black text-muted-foreground ml-2">{items.length} Entries</span>
 { }
 </h3>
 <button onClick={onClose} className="p-2 text-muted-foreground hover:text-slate-600 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors">
 { }
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <tr key={item.id || idx} className="hover:bg-muted/50 dark:hover:bg-slate-800/80 transition-colors">
 {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
 {columns.map((col: any, colIdx: number) => (
 <td key={colIdx} className="px-5 py-3 text-sm text-foreground whitespace-nowrap">
 {col.render(item)}
 </td>
 ))}
 </tr>

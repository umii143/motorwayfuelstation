...
import {
 Package, Clock, Hash, Search, TrendingUp,
 TrendingDown, ChevronDown, ChevronUp, Droplets, BarChart2
} from 'lucide-react';
...
function AgingBadge({ days }: { days: number }) {
 ...
 return (
 <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border${cat.color === 'green' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
...
export default function BatchHistory({ batches, products, language }: BatchHistoryProps) {
 ...
 return 'bg-muted text-slate-500 border-border';
 };
 ...
 <div key={i} className="bg-subtle /50 rounded-lg p-2 border border-theme-main">
 <p className="text-[9px] text-muted-foreground flex items-center gap-1 mb-0.5 uppercase tracking-wide">
 <stat.icon className={`size-3${stat.color}`} />
 {stat.label}
 </p>
 <p className="font-bold text-foreground text-xs truncate">{stat.value}</p>
 </div>
 ))}
 </div>
 ...
 className={`fp-date-tab${filterStatus === s ? 'fp-date-tab--active !text-foreground dark:!text-slate-100 !border-slate-800 dark:!border-slate-500 bg-slate-200/50 /50' : ''}`}>
 {s.charAt(0).toUpperCase() + s.slice(1)}
 </button>
 ))}
 <div className="ml-auto flex items-center gap-1.5 text-[10px] text-muted-foreground font-bold uppercase tracking-wide">
 Sort:
 ...
 <span className="text-[9px] text-muted-foreground font-normal">Realized Margin</span>
 </div>
 </th>
 ...
 <tr
 className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer${aging.status === 'critical' && batch.status === 'active' ? 'bg-red-500/10' : ''}`}
 onClick={() => setExpandedId(isExpanded ? null : batch.id)}
 >
 <td className="py-2.5 px-3">
 <div className="flex items-center gap-1.5">
 <Hash className="size-3.5 text-muted-foreground" />
 <span className="font-mono text-foreground font-bold text-[11px] bg-theme-main px-1.5 py-0.5 rounded">
 {batch.batchNumber}
 </span>
 {batch.invoiceNumber && (
 <span className="text-[10px] text-muted-foreground hidden xl:inline">#{batch.invoiceNumber}</span>
 )}
 </div>
 </td>
 ...
 {batch.deliveryTime && <span className="text-muted-foreground ml-4 text-[9px]">{batch.deliveryTime}</span>}
 </div>
 </td>
 <td className="py-2.5 px-3">
 ...
 <p className="text-[9px] text-muted-foreground mt-0.5">{pctSold.toFixed(0)}% sold</p>
 </div>
 )}
 </div>
 </td>
 ...
 <p className="text-[9px] text-muted-foreground">Inv: Rs.{batch.invoiceCostPerLiter.toFixed(2)}</p>
 )}
 </div>
 </td>
 <td className="py-2.5 px-3">
 ...
 <span className="text-[9px] text-muted-foreground">Realized:</span>

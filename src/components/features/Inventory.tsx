...
export default function Inventory({
 ...
 <div>
 <span className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-widest block group-hover:text-orange-500 transition-colors">{t('Total Fuel In Storage Tanks', 'ٹینکس میں موجود کل فیول والیم')}</span>
 <strong className="font-mono text-base font-bold text-foreground tracking-tight mt-1 block">
 ...
 <div>
 <span className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-widest block group-hover:text-sky-600 transition-colors">{t('Lubricants / Accessories', 'کل انجن آئل اسٹاک')}</span>
 <strong className="font-mono text-base font-bold text-foreground tracking-tight mt-1 block">
 {lubeMetrics.totalQty.toLocaleString()} Units
 </strong>
 </div>
 </div>
 <div className="grid grid-cols-2 gap-2 mt-2 border-t pt-2 border-border">
 <div>
 <span className="text-[9px] font-bold text-muted-foreground uppercase">Stock Value</span>
 <div className="text-xs font-mono font-bold text-foreground">Rs. {lubeMetrics.totalValue.toLocaleString()}</div>
 </div>
 <div>
 ...
 <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl${lowStockCount > 0 ? 'bg-red-100 text-red-655' : 'bg-muted text-slate-400'}`}>
 <AlertTriangle className="h-5 w-5" />
 </div>
 <div>
 <span className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-widest block">{t('Low Stock Alarms', 'انتباہی اسٹاک الرٹ')}</span>
 <strong className={`font-mono text-base font-bold tracking-tight mt-1 block${lowStockCount > 0 ? 'text-red-650' : 'text-foreground'}`}>
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-theme-card text-foreground font-bold rounded-lg shadow-sm border border-theme-main text-xs"
 >
 ...
 <strong className="text-xs font-bold text-foreground truncate block group-hover:text-orange-600 transition-colors leading-tight">{t(prod.name, prod.urduName)}</strong>
 <span className="text-[9px] uppercase font-bold text-muted-foreground mt-0.5 block truncate">
 ...
 <div className="flex justify-between items-center py-2 border-t border-b border-theme-main text-foreground font-mono text-[10px] mt-1.5 mb-1.5">
 <div className="truncate">
 <span className="text-muted-foreground text-[9px] uppercase block font-sans truncate">{t('Stock', 'اسٹاک')}</span>
 <strong className="text-foreground text-sm font-bold truncate block">{prod.currentStock.toLocaleString()} <span className="text-[10px] text-muted-foreground">{prod.unit}</span></strong>
 </div>
 <div className="text-right truncate pl-2">
 <span className="text-muted-foreground text-[9px] uppercase block font-sans truncate">{t('Price', 'قیمت')}</span>
 <strong className="text-orange-600 text-sm font-bold truncate block">Rs. {prod.rate.toFixed(2)}</strong>
 </div>
 </div>

 <div className="flex justify-between items-center bg-subtle /50 p-1.5 rounded text-[9px] font-bold text-muted-foreground mb-2">
 <span>Min: <strong className="text-foreground">{prod.minStock.toLocaleString()}</strong></span>
 ...
 className="flex-1 flex items-center justify-center gap-1 py-1 rounded bg-theme-card border border-theme-main text-muted-foreground hover:border-orange-500 hover:text-orange-500 font-bold text-[10px] transition-colors"
 >
 ...
 <h3 className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5 border-b border-theme-main pb-2">
 ...
 <p className="py-8 text-center text-muted-foreground text-[10px] font-sans">
 ...
 <strong className="text-foreground block truncate max-w-[100px] text-[11px]">
 {prod ? t(prod.name, prod.urduName) : txn.itemId}
 </strong>
 <span className="text-[9px] text-muted-foreground block mt-0.5">{txn.date} • {txn.by}</span>
 </div>
 </div>
 );
 },
 ...
 <strong className={`font-mono text-[10px] block${isRec ? 'text-teal-500' : isSale ? 'text-foreground' : txn.quantity > 0 ? 'text-teal-500' : 'text-red-500'}`}>
 {isRec ? '+' : isSale ? '-' : txn.quantity > 0 ? '+' : ''}
 {txn.quantity.toLocaleString()} {prod?.unit || 'Ltr'}
 </strong>
 {txn.amount && (
 <span className="text-[9px] text-muted-foreground font-mono block mt-0.5">Rs. {txn.amount.toLocaleString()}</span>
 )}
 </div>
 );
 },
 ...
 {stockTransactions.length === 0 ? (
 <p className="py-12 text-center text-muted-foreground text-xs font-sans">
 {t('No inventory transactions logged yet.', 'اسٹاک کی کوئی انٹری اس شیٹ میں موجود نہیں۔')}
 </p>
 ) : (
 ...
 <span className="text-[10px] text-muted-foreground block mt-0.5">{txn.date} • {txn.by}</span>
 </div>
 </div>
 );
 },
 ...
 <strong className={`font-mono text-xs block${isRec ? 'text-teal-500' : isSale ? 'text-foreground' : txn.quantity > 0 ? 'text-teal-500' : 'text-red-500'}`}>
 {isRec ? '+' : isSale ? '-' : txn.quantity > 0 ? '+' : ''}
 {txn.quantity.toLocaleString()} {prod?.unit || 'Ltr'}
 </strong>
 {txn.amount && (
 <span className="text-[10px] text-muted-foreground font-mono block mt-0.5">Rs. {txn.amount.toLocaleString()}</span>
 )}
 </div>
 );
 },
 ...
 {tanks.length === 0 ? (
 <div className="p-4 text-center rounded-xl bg-theme-card border border-theme-main text-muted-foreground text-xs">
 ...
 {/* CYLINDER VISUAL */}
 <div className="relative w-12 h-24 border border-theme-main rounded bg-muted flex items-end shrink-0 shadow-inner overflow-hidden">
 {/* Fuel liquid level */}
 <div 
 ...
 />
 <div className="absolute inset-0 flex flex-col justify-between items-center py-1 text-[8px] font-mono font-bold text-foreground z-10 text-center leading-tight">
 <span>{tnk.capacity.toLocaleString()}L</span>
 <span className="bg-card/80 px-1 py-0.5 rounded text-[9px] font-mono font-extrabold">{fillPct}%</span>
 ...
 <span className="text-[9px] text-muted-foreground uppercase flex items-center gap-1 mt-0.5 truncate">
 ...
 <span className="text-muted-foreground font-sans">{t('Level:', 'اسٹاک:')}</span>
 <strong className="text-foreground">{tnk.currentStock.toLocaleString()}L</strong>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-muted-foreground font-sans">{t('Safe:', 'محفوظ:')}</span>
 <strong className="text-teal-600">&gt;{tnk.safeLevel.toLocaleString()}L</strong>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-muted-foreground font-sans">{t('Crit:', 'الرٹ:')}</span>
 <strong className="text-red-500">&lt;{tnk.criticalLevel.toLocaleString()}L</strong>
 </div>
 </div>
 ...
 <div>
 <label className="block text-muted-foreground font-bold mb-1 text-[11px]">{t('Select Storage Tank:', 'سٹوریج ٹینک منتخب کریں:')}</label>
 <select
 value={calcTankId}
 ...
 <div>
 <label className="block text-muted-foreground font-bold mb-1 text-[11px]">{t('Measured Height (cm):', 'پیمائش سٹک (cm):')}</label>
 <input
 type="number"
 min="0"
 max="400"
 ...
 <span className="text-[9px] uppercase font-bold tracking-wider text-teal-600 block">🎉 {t('CONVERTED VOLUME', 'حساب شدہ والیم')}</span>
 <strong className="text-xl font-mono text-teal-700 block">{calculatedCalibratedVolume.toLocaleString()} <span className="text-[10px] font-sans font-normal text-teal-600/70">Litres</span></strong>
 </div>
 )}
 </div>
 </div>
 ...
 <div>
 <h3 className="font-sans text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5">
 ...
 accessor: (log) => <span className="font-mono text-muted-foreground whitespace-nowrap">{log.date}</span>,
 isSecondaryMobile: true
 },
 {
 ...
 return <span className="font-bold text-foreground">{prod ? t(prod.name, prod.urduName) : log.productId.toUpperCase()}</span>;
 },
 isPrimaryMobile: true
 },
 {
 header: t('Old Tariff Rate', 'کلو ز ریٹ'),
 accessor: (log) => <span className="font-mono text-muted-foreground">Rs. {(log.oldRate || 0).toFixed(2)}</span>
 },
 {
 header: t('New Tariff Revised', 'نیا نافذ ریٹ'),
 accessor: (log) => <span className="font-mono font-bold text-foreground">Rs. {(log.newRate || 0).toFixed(2)}</span>
 },
 {
 ...
 accessor: (log) => <span className="font-mono font-bold text-foreground">{(log.stockAtTime || 0).toLocaleString()} Ltr</span>
 },
 {
 ...
 <span className="block font-semibold text-foreground truncate">{log.reason}</span>
 <span className="block text-[9px] text-muted-foreground font-mono">By: {log.changedBy}</span>
 </div>
 )
 }
 ]}
 ...
 <button onClick={() => setShowReconcileModal(false)} className="text-muted-foreground hover:text-slate-650 cursor-pointer font-bold text-xl">&times;</button>
 </div>

 <p className="font-sans text-[11px] text-muted-foreground leading-normal pb-3 border-b border-border mb-4">
 {t(
 ...
 <button
 type="submit"
 className="w-full py-3 bg-card hover:bg-slate-900 text-foreground font-sans text-sm font-bold tracking-wider rounded-lg shadow-md mt-4 cursor-pointer"
 >
 ...
 <button onClick={() => setShowAddProductModal(false)} className="text-muted-foreground hover:text-slate-700 text-xl font-bold cursor-pointer">&times;</button>
 </div>

 <form onSubmit={handleProductModalSubmit} className="space-y-4">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Product Name (English):', 'نام (انگریزی):')}</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Product Name (Urdu):', 'نام (اردو):')}</label>
 <input type="text" value={prodUrduName} onChange={e => setProdUrduName(e.target.value)}
 placeholder="مثال: موبل ون"
 className="premium-input border bg-theme-card px-3 font-sans text-sm focus:border-emerald-500 outline-none" />
 </div>
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Purchase Price (Rs):', 'خرید قیمت (روپے):')}</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Selling Rate (Rs):', 'فروخت قیمت (روپے):')}</label>
 <input type="number" required min="0" step="0.01" value={prodRate} onChange={e => setProdRate(e.target.value)}
 placeholder="e.g. 1500"
 className="premium-input border bg-theme-card px-3 font-mono text-sm focus:border-emerald-500 outline-none" />
 </div>
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Unit of Measure:', 'پیمائش کی اکائی:')}</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Product Type:', 'پروڈکٹ کی قسم:')}</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Min Stock Alert Level:', 'انتباہی اسٹاک حد:')}</label>
 ...
 <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('Opening Stock Quantity:', 'ابتدائی اسٹاک مقدار:')}</label>

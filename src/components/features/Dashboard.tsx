...
export default function Dashboard({
  ...
  return (
    <div id="fuelpro_dashboard_view" className="space-y-6 pb-16 lg:pb-0">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2 self-start sm:self-center">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-sans text-sm font-semibold text-slate-800 shadow-xs focus:border-orange-500 focus:outline-hidden"
          >
      ...
      {/* DAILY SALES VS ESTIMATED MARGIN TREND (AreaChart) */}
      <motion.div 
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.35, ease: "easeOut" }}
        className="rounded-xl border border-border/50 glass p-5 shadow-md hover:shadow-lg transition-all"
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3 mb-4">
          <div>
            <h3 className="font-sans text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
              ...
              <span>{t('Daily Sales & Estimated Margin Trend', 'یومیہ فروخت اور تخمینہ منافع کا رجحان')}</span>
            </h3>
            <p className="font-sans text-xs text-slate-400 mt-1">
              {isLube
                ? t('Time-series analysis of lube POS billing and estimated margin performance.', 'لیوب پی او ایس بلنگ اور تخمینہ منافع کی کارکردگی کا وقتی تجزیہ۔')
                : t('Time-series analysis of station gross fuel sales volume alongside estimated profit margins.', 'اسٹیشن کے مجموعی فیول سیلز اور آمدنی کا وقت کے حساب سے تجزیاتی جائزہ۔')}
            </p>
          </div>
          <span className="rounded-full bg-orange-50 px-3 py-1 font-sans text-[10px] font-bold text-orange-600 uppercase self-start sm:self-center">
            {t('Commercial Trend KPI', 'کاروباری کارکردگی کا رجحان')}
          </span>
        </div>

        {salesMarginTrendData.length === 1 && salesMarginTrendData[0].Sales === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-slate-400 font-sans text-xs gap-2">
            <Activity className="h-8 w-8 text-slate-350 animate-pulse" />
            <span>
              {isLube
                ? t('Post POS receipts to start plotting the lube business timeline.', 'لیوب بزنس کی ٹائم لائن شروع کرنے کے لیے پی او ایس رسیدیں پوسٹ کریں۔')
                : t('Establish shifts or log transactions to plot the financial timeline.', 'رجحان کا نقشہ تیار کرنے کے لیے پہلی شفٹ کا اندراج شروع کریں۔')}
            </span>
          </div>
        ) : (
          <div className="h-72 w-full text-xs font-sans mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={salesMarginTrendData} margin={{ top: 10, right: 10, left: -5, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorSalesTrend" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#EA580C" stopOpacity={0.15}/>
                    <stop offset="95%" stopColor="#EA580C" stopOpacity={0}/>
                  </linearGradient>
                  <linearGradient id="colorMarginTrend" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis 
                  ...
                  axisLine={false} 
                  dx={-5}
                  tickFormatter={(val) => `${getCurrencySymbol(settings)} ${val >= 1000 ? `${(val / 1000).toFixed(0)}k` : val}`}
                  className="font-mono"
                />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #E2E8F0', fontFamily: 'sans-serif' }}
                  labelStyle={{ fontWeight: 'bold', color: '#1E293B', marginBottom: '4px' }}
                  formatter={(value: any) => [`${formatCurrency(Number(value), settings)}`, '']}
                />
                <Legend 
                  verticalAlign="top" 
                  height={36} 
                ...
                <Area 
                  ...
                  name={t('Gross Sales', 'مجموعی فروخت')} 
                  activeDot={{ r: 6 }}
                />
                <Area 
                  type="monotone" 
                  ...
                  strokeWidth={2.5} 
                  fillOpacity={1} 
                  fill="url(#colorMarginTrend)" 
                  name={t('Est. Margin', 'تخمینہ منافع')} 
                  activeDot={{ r: 6 }}
                />
              </AreaChart>
            </ResponsiveContainer>

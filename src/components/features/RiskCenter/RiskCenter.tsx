...
export const RiskCenter: React.FC = () => {
 ...
 <div className="premium-card bg-card border border-border rounded-2xl px-4 py-2 flex items-center gap-3">
 <div className="text-right">
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('Enterprise Health', 'انٹرپرائز ہیلتھ')}</p>
 ...
 <div className="bg-card rounded-xl p-6 relative z-10 h-full">
 ...
 <p className="text-foreground text-lg font-medium leading-relaxed max-w-4xl">
 {aiInsight}
 </p>
 </div>
 {totalRisks > 0 && (
 ...
 {/* Inventory Risk Card */}
 <div className="bg-card rounded-2xl shadow-md border border-border overflow-hidden flex flex-col">
 <div className="p-5 bg-gradient-to-r from-orange-50 to-white dark:from-orange-950/20 dark:to-[#151521] border-b border-orange-100 flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-orange-100 rounded-lg text-orange-600">
 <AlertTriangle className="h-5 w-5" />
 </div>
 <div>
 <h3 className="font-black text-foreground">{t('Inventory Risks', 'اسٹاک کے خطرات')}</h3>
 <p className="text-xs font-semibold text-muted-foreground">{inventoryRisks.length} {t('Critical Tanks', 'نازک ٹینک')}</p>
 </div>
 </div>
 </div>
 
 <div className="p-5 flex-1 space-y-6 bg-subtle">
 {inventoryRisks.map(r => (
 <div key={r.tankId} className="premium-card bg-card p-4 border border-border rounded-2xl relative overflow-hidden group hover:shadow-md transition-shadow">
 <div className="absolute top-0 left-0 w-1 h-full bg-orange-500"></div>
 <div className="flex justify-between items-start mb-3">
 <div>
 <span className="text-sm font-black text-foreground block">{r.tankName}</span>
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{r.productId}</span>
 </div>
 ...
 <span className="text-muted-foreground">{t('Volume', 'حجم')}</span>
 <span className="text-foreground">{r.currentStock?.toLocaleString()} L</span>
 </div>
 ...
 <button className="w-full flex items-center justify-center gap-2 bg-card hover:bg-slate-800 dark:hover:bg-slate-700 text-foreground text-xs font-bold py-2.5 rounded-lg transition-colors cursor-pointer">
 ...
 <div>
 <p className="font-bold text-foreground">{t('Stock Levels Optimal', 'اسٹاک بہترین ہے')}</p>
 <p className="text-xs text-muted-foreground font-medium">{t('Predictive engine shows no stockouts.', 'کوئی اسٹاک ختم ہونے کا خطرہ نہیں ہے۔')}</p>
 </div>
 </div>
 )}
 </div>
 ...
 {/* Financial Risk Card */}
 <div className="bg-card rounded-2xl shadow-md border border-border overflow-hidden flex flex-col">
 <div className="p-5 bg-gradient-to-r from-red-50 to-white dark:from-red-950/20 dark:to-[#151521] border-b border-red-100 flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-red-100 rounded-lg text-red-600">
 <TrendingDown className="h-5 w-5" />
 </div>
 <div>
 <h3 className="font-black text-foreground">{t('Financial Risks', 'مالیاتی خطرات')}</h3>
 <p className="text-xs font-semibold text-muted-foreground">{financialRisks.length} {t('High Exposures', 'زیادہ ادھار')}</p>
 </div>
 </div>
 </div>
 ...
 return (
 <div key={c.id} className="premium-card bg-card p-4 border border-border rounded-2xl relative overflow-hidden hover:shadow-md transition-shadow">
 <div className="absolute top-0 left-0 w-1 h-full bg-red-500"></div>
 <div className="flex justify-between items-start mb-3">
 <div>
 <span className="text-sm font-black text-foreground block truncate max-w-[150px]">{c.name}</span>
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('Corporate', 'کارپوریٹ')}</span>
 </div>
 ...
 <div className="space-y-1 mb-4">
 <div className="flex justify-between text-xs font-semibold">
 <span className="text-muted-foreground">{t('Balance', 'بقایا')}</span>
 ...
 <button className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 text-xs font-bold py-2.5 rounded-lg transition-colors cursor-pointer">
 ...
 <div>
 <p className="font-bold text-foreground">{t('Credit Health Strong', 'ادھار کی صورتحال مضبوط ہے')}</p>
 <p className="text-xs text-muted-foreground font-medium">{t('No customers exceeding exposure limits.', 'کوئی گاہک ادھار کی حد سے تجاوز نہیں کر رہا۔')}</p>
 </div>
 </div>
 )}
 </div>
 ...
 <div className="bg-card rounded-2xl shadow-md border border-border overflow-hidden flex flex-col">
 <div className="p-5 bg-gradient-to-r from-purple-50 to-white dark:from-purple-950/20 dark:to-[#151521] border-b border-purple-100 flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="p-2 bg-purple-100 rounded-lg text-purple-600">
 <AlertOctagon className="h-5 w-5" />
 </div>
 <div>
 <h3 className="font-black text-foreground">{t('Operational Risks', 'آپریشنل خطرات')}</h3>
 <p className="text-xs font-semibold text-muted-foreground">{operationalRisks.length} {t('Anomalies', 'غیر معمولی تبدیلیاں')}</p>
 </div>
 </div>
 </div>
 ...
 <div key={s.id} className="premium-card bg-card p-4 border border-border rounded-2xl relative overflow-hidden hover:shadow-md transition-shadow">
 <div className="absolute top-0 left-0 w-1 h-full bg-purple-500"></div>
 
 <div className="flex justify-between items-start mb-3">
 <div>
 <span className="text-sm font-black text-foreground block">{t('Shift #', 'شفٹ #')}{s.id.slice(0, 6)}</span>
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{s.date}</span>
 </div>
 ...
 <div className="bg-subtle rounded-lg p-3 mb-4 border border-border">
 <div className="flex items-center justify-between mb-1">
 <span className="text-xs font-bold text-muted-foreground">{t('Operator', 'آپریٹر')}</span>
 <span className="text-xs font-black text-foreground">{staff?.find(st => st.id === s.staffId)?.name || s.staffId}</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="text-xs font-bold text-muted-foreground">{t('Variance', 'فرق')}</span>
 <span className="text-xs font-black text-purple-600">{s.cashVariance?.toLocaleString()} PKR</span>
 </div>
 </div>

 <button className="w-full flex items-center justify-center gap-2 bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200 text-xs font-bold py-2.5 rounded-lg transition-colors cursor-pointer">
 ...
 <div>
 <p className="font-bold text-foreground">{t('Operations Secure', 'آپریشنز محفوظ ہیں')}</p>
 <p className="text-xs text-muted-foreground font-medium">{t('All recent shifts settled within variance limits.', 'حالیہ تمام شفٹوں کا تصفیہ فرق کی حد کے اندر ہوا ہے۔')}</p>
 </div>
 </div>
 )}
 </div>

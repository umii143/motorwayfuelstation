...
export function SetupNavigationFooter({ activeViewId, onNavigate }: SetupNavigationFooterProps) {
  ...
  return (
    <div className="mt-8 flex flex-row items-center justify-between gap-4 border-t border-slate-200 dark:border-white/10 pt-6">
      <div>
        {prevStep ? (
          <button
            onClick={() => onNavigate(prevStep.viewId)}
            className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-800 dark:text-slate-200 transition-colors"
          >

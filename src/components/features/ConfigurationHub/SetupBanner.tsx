...
export function SetupBanner({ activeViewId }: { activeViewId: string }) {
 ...
 <p className="font-sans text-xs font-semibold text-muted-foreground mt-1">
 ...
 <span className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Completion</span>
 ...
 <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold whitespace-nowrap transition-colors${step.status === 'completed' ? 'bg-green-50 border-green-200 text-green-700' : ''}${isActive ? 'bg-[var(--primary-accent)] text-white border-[var(--primary-accent)] shadow-md' : ''}${step.status === 'pending' && !isActive ? 'bg-slate-50 dark:bg-card/5 border-border text-slate-600' : ''}${step.status === 'locked' ? 'bg-slate-50 dark:bg-card/5 border-slate-100 dark:border-white/5 text-slate-400 opacity-60' : ''}`}>

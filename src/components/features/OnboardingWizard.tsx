...
export default function OnboardingWizard({ onComplete, onCancel, currentLanguage }: OnboardingWizardProps) {
 ...
 return (
 <div className="fixed inset-0 z-[9000] bg-background flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden">
 {/* Background Decor */}
 ...
 {/* Connecting line */}
 <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-card -translate-y-1/2 z-0" />
 ...
 <span className="text-xs font-bold text-muted-foreground">{tanks.length} Added</span>
 </div>
 ...
 <p className="text-xs text-muted-foreground font-medium">{t.fuelType} • {t.capacity.toLocaleString()}L Capacity</p>
 </div>
 ...
 className="col-span-2 bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:border-orange-500 outline-none"
 />
 <select 
 value={newTank.fuelType}
 onChange={e => setNewTank({...newTank, fuelType: e.target.value})}
 className="bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:border-orange-500 outline-none"
 >
 ...
 className="w-full py-2 bg-card hover:bg-slate-700 disabled:opacity-50 text-muted-foreground text-sm font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
 >
 ...
 <span className="text-xs font-bold text-muted-foreground">{nozzles.length} Added</span>
 </div>
 ...
 <p className="text-xs text-muted-foreground font-medium">Pump {n.pumpId} • Connects to: {tankName}</p>
 </div>
 ...
 <span className="text-xs font-bold text-muted-foreground uppercase">Super Petrol</span>
 <input 
 type="number" 
 value={petrolRate || ''}
 ...
 <div className="bg-[var(--bg-app)] border border-[var(--border-main)] rounded-xl p-4">
 <span className="text-xs font-bold text-muted-foreground uppercase">High Speed Diesel</span>
 <input 
 type="number" 
 value={dieselRate || ''}
 ...
 <span className="text-xs font-bold text-muted-foreground">{staff.length} Added</span>
 </div>
 ...
 <p className="text-xs text-muted-foreground font-medium">PIN: {s.pin} • {s.phone || 'No Phone'}</p>
 </div>
 ...
 className="bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:border-orange-500 outline-none tracking-widest font-mono"
 />
 </div>
 <button 

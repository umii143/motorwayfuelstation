...
const SectionHeader = ({
 ...
 className="w-full flex items-center justify-between text-left py-3 px-4 rounded-xl bg-subtle hover:bg-muted transition-colors group border border-border"
 >
 ...
 <span className="font-bold text-sm text-foreground uppercase tracking-wide">{title}</span>
 {badge && (
 <span className={`text-xs px-2 py-0.5 rounded-full font-semibold${badgeColor === 'emerald' ? 'bg-emerald-100 text-emerald-700' :
 ...
 ? <ChevronUp className="size-4 text-muted-foreground group-hover:text-slate-600" />
 : <ChevronDown className="size-4 text-muted-foreground group-hover:text-slate-600" />
 }
 </button>
);
...
export default function StockInForm({
 ...
 const inputCls ="w-full rounded-lg border border-border bg-card px-3 py-2 text-foreground focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none text-sm transition-colors";
 const selectCls ="w-full rounded-lg border border-border bg-slate-50 dark:bg-white/5 px-3 py-2 text-foreground focus:border-orange-500 focus:bg-card focus:outline-none text-sm";

 // ─── POST-SAVE RECEIPT VIEW ───────────────────────────────────────────────────
 if (savedBatch) {
 return (
 <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-0 sm:p-4 bg-card backdrop-blur-sm overflow-y-auto">
 <div className="bg-card rounded-none sm:rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden sm:my-8">
 {/* Receipt Header */}
 <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 px-6 py-6 text-white">
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-3">
 <div className="size-12 rounded-xl bg-card/20 flex items-center justify-center">
 ...
 <button onClick={onClose} className="p-2 text-white/70 hover:text-white rounded-full hover:bg-card/10">
 <X className="size-5" />
 </button>
 </div>
 </div>
 ...
 <h3 className="font-bold text-sm text-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
 <Receipt className="size-4 text-orange-600" /> WHAT YOU PAID
 </h3>
 <div className="space-y-2">
 {savedBatch.invoiceTotalAmount && (
 <div className="flex justify-between text-sm">
 <span className="text-muted-foreground">Invoice ({currentSupplier?.name})</span>
 ...
 <p className="text-xs text-muted-foreground">{savedBatch.landedCostPerLiter.toFixed(4)}/L</p>
 </div>
 </div>

 {/* Profit Preview */}
 <div className="bg-card rounded-xl p-4 text-foreground">
 <h3 className="font-bold text-sm text-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
 <TrendingUp className="size-4 text-emerald-400" /> EXPECTED BATCH MARGIN
 </h3>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <p className="text-muted-foreground text-xs">Per Liter</p>
 <p className="text-2xl font-bold text-emerald-400">
 Rs. {(savedBatch.expectedBatchMarginPerLiter || 0).toFixed(2)}
 </p>
 </div>
 <div>
 <p className="text-muted-foreground text-xs">Total ({savedBatch.qtyReceived.toLocaleString()}L)</p>
 <p className="text-2xl font-bold text-emerald-400">
 Rs. {(savedBatch.expectedBatchMarginTotal || 0).toLocaleString('en-PK', { maximumFractionDigits: 0 })}
 </p>
 </div>
 </div>
 <p className="text-muted-foreground text-xs mt-2">OGRA: Rs. {savedBatch.ograPumpPrice}/L | Landed: Rs. {savedBatch.landedCostPerLiter.toFixed(2)}/L</p>
 </div>
 ...
 <p className="text-sm text-muted-foreground">Raise a claim with {pendingClaimData.supplierName}?</p>
 </div>
 </div>
 ...
 className="flex-1 py-2.5 rounded-xl border border-border text-muted-foreground font-medium hover:bg-slate-50"
 >
 Skip for Now
 </button>
 <button
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-0 sm:p-4 bg-card backdrop-blur-sm overflow-y-auto">
 <div className="bg-card rounded-none sm:rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden sm:my-8 min-h-screen sm:min-h-0 flex flex-col">
 ...
 <div className="flex items-center gap-3">
 <div className="size-10 rounded-xl bg-card/20 flex items-center justify-center shrink-0">
 ...
 <button onClick={onClose} className="p-2 text-white/70 hover:text-white rounded-full hover:bg-card/10 transition-colors">
 <X className="size-5" />
 </button>
 </div>
 ...
 <h3 className="font-bold text-sm text-foreground uppercase tracking-wide">Delivery Info</h3>
 </div>
 ...
 <div className={`rounded-lg border px-3 py-2 text-sm font-bold${metrics && metrics.qtyShort > 50 ? 'bg-red-50 border-red-300 text-red-700' :
 ...
 <div className="rounded-lg border border-border bg-subtle px-3 py-2 text-sm font-mono text-muted-foreground">
 ...
 {dipValidation && (
 <div className={`text-sm rounded-lg px-3 py-2 flex items-center gap-2${dipValidation.severity === 'error' ? 'bg-red-50 text-red-700 border border-red-200' :
 ...
 className={`flex-1 py-2.5 text-sm font-semibold transition-colors${priceEntryMode === 'total' ? 'bg-orange-600 text-white' : 'bg-card text-slate-600 hover:bg-slate-50 dark:bg-card/5'}`}>
 ● Enter Total Invoice Amount (Recommended)
 </button>
 <button type="button"
 onClick={() => setPriceEntryMode('perLiter')}
 className={`flex-1 py-2.5 text-sm font-semibold transition-colors${priceEntryMode === 'perLiter' ? 'bg-orange-600 text-white' : 'bg-card text-slate-600 hover:bg-slate-50 dark:bg-card/5'}`}>
 ...
 <div className="rounded-lg border border-border bg-subtle px-3 py-2 text-sm flex items-center justify-between">
 <span className="text-muted-foreground">Fixed by OGRA</span>
 <span className="font-bold text-foreground">Rs. {dealerMargin.toFixed(2)}/L</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-1">= Rs. {(numCarriage / numQtyReceived).toFixed(4)}/L</p>
 )}
 </Field>
 ...
 className="rounded" />
 <span className="text-xs text-muted-foreground">Auto-add to Expenses → Carriage & Freight</span>
 </label>
 </div>
 )}
 </div>
 ...
 className="rounded" />
 <span className="text-xs text-muted-foreground">Auto-add to Expenses → Driver Gratuity</span>
 </label>
 )}
 </div>
 </div>
 ...
 <div className="rounded-lg border border-border bg-subtle px-3 py-2 text-sm text-muted-foreground font-mono">
 ...
 <label key={status}
 className={`flex items-center gap-2 px-3 py-1.5 rounded-full border cursor-pointer text-sm font-medium transition-colors${(sealEval?.status === status || (!sealEval && sealStatusOverride === status))
 ? status === 'ok' ? 'bg-emerald-600 border-emerald-600 text-white'
 : 'bg-red-600 border-red-600 text-white'
 : 'bg-card border-border text-slate-600 hover:border-border'}`}>
 ...
 className={`py-2.5 rounded-xl text-sm font-semibold border transition-colors${paymentMethod === mode ? 'bg-orange-600 border-orange-600 text-white' : 'bg-card border-border text-slate-600 hover:border-orange-300'}`}>
 ...
 {/* Live Profit Preview */}
 <div className="bg-card rounded-2xl p-5 text-foreground shadow-xl relative overflow-hidden">
 ...
 <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">LIVE PROFIT PREVIEW</h3>
 </div>
 ...
 <p className="text-xs text-muted-foreground uppercase tracking-wide">What You Paid</p>
 ...
 <span className="text-xs text-muted-foreground font-semibold">Total Landed</span>
 <span className="font-bold">Rs. {metrics.totalLandedCost.toLocaleString('en-PK', { maximumFractionDigits: 0 })}</span>
 </div>
 <p className="text-xs text-muted-foreground text-right">Rs. {metrics.landedCostPerLiter.toFixed(4)}/L</p>
 </div>

 {/* What You'll Earn */}
 <div className="space-y-2 border-b border-border pb-4">
 <p className="text-xs text-muted-foreground uppercase tracking-wide">What You'll Earn</p>
 <div className="flex justify-between items-baseline">
 <span className="text-xs text-muted-foreground">Revenue ({numQtyReceived.toLocaleString()}L)</span>
 ...
 <p className="text-xs text-muted-foreground mb-1">Expected Batch Margin</p>
 ...
 <span className="text-muted-foreground">OGRA Fixed Margin</span>
 <span className="text-muted-foreground">Rs. {dealerMargin.toFixed(2)}/L</span>
 </div>
 <div className="flex justify-between">
 <span className="text-muted-foreground">Your Actual Margin</span>
 <span className="text-muted-foreground">Rs. {metrics.expectedBatchMarginPerLiter.toFixed(2)}/L</span>
 </div>
 ...
 <div key={i} className={`rounded-xl px-3 py-2.5 text-xs flex items-start gap-2${msg.severity === 'error' ? 'bg-red-50 text-red-700 border border-red-200' :
 ...
 {/* Batch Info Preview */}
 {metrics && (
 <div className="bg-subtle border border-border rounded-xl p-4 space-y-2 text-xs">
 <p className="font-bold text-foreground text-sm">Batch Preview</p>
 ...
 {/* ─── FOOTER ACTIONS ───────────────────────────────────────────────── */}
 <div className="sticky bottom-0 bg-card border-t border-border px-4 sm:px-6 py-4 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3">
 <button type="button" onClick={onClose}
 className="px-6 py-3 sm:py-2.5 rounded-xl font-medium text-muted-foreground hover:bg-muted transition-colors w-full sm:w-auto text-center border border-border">
 Cancel
 </button>
 ...
 {saving ? (
 <>
 <div className="size-4 rounded-full border-2 border-border border-t-white animate-spin" />
 Saving...
 </>
 ) : (
 <>

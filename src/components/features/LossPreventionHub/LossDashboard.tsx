...
export default function LossDashboard({ settings, stationId }: LossDashboardProps) {
 ...
 <div className="absolute -right-4 -top-4 w-24 h-24 bg-subtle rounded-full group-hover:scale-110 transition-transform"></div>
 <div className="relative">
 <div className="flex justify-between items-start mb-2">
 <span className="text-xs font-bold text-muted-foreground uppercase">MTD Incidents</span>
 <div className="p-2 bg-muted text-muted-foreground rounded-lg">
 <Scale className="h-4 w-4" />
 </div>
 </div>
 <div className="text-2xl font-black font-mono text-foreground">{mtdIncidents.length}</div>
 <div className="text-xs text-muted-foreground mt-2 font-medium">Total reported this month</div>
 </div>
 </div>
 ...
 <div className="relative">
 <div className="flex justify-between items-start mb-2">
 <span className="text-xs font-bold text-muted-foreground uppercase">Unresolved Value Loss</span>
 <div className="p-2 bg-rose-100 text-rose-600 rounded-lg">
 <TrendingDown className="h-4 w-4" />
 </div>
 </div>
 <div className="text-2xl font-black font-mono text-rose-600">{settings.currency} {totalFinancialLoss.toLocaleString()}</div>
 <div className="text-xs text-muted-foreground mt-2 font-medium">Capital tied up in shrinkage/loss</div>
 </div>
 </div>

 <div className="premium-card p-5 border relative overflow-hidden group bg-card border-border">
 <div className="absolute -right-4 -top-4 w-24 h-24 bg-card rounded-full group-hover:scale-110 transition-transform"></div>
 <div className="relative">
 <div className="flex justify-between items-start mb-2">
 <span className="text-xs font-bold text-muted-foreground uppercase">AI System Variance</span>
 <div className="p-2 bg-card text-muted-foreground rounded-lg">
 ...
 Worst Nozzle: <span className="text-muted-foreground">{varianceAnalytics.worstNozzleId}</span> ({varianceAnalytics.worstNozzleLoss.toLocaleString()}L)
 </div>
 </div>
 </div>
 </div>
 ...
 <h3 className="font-bold text-sm text-foreground">Critical & High Priority Incidents</h3>
 </div>
 ...
 <div>
 <div className="font-bold text-foreground text-sm capitalize">{incident.type.replace('_', ' ')}</div>
 <div className="text-xs text-muted-foreground mt-0.5">Date: {new Date(incident.date).toLocaleDateString()} | Ref: {incident.sourceId}</div>
 </div>
 </div>

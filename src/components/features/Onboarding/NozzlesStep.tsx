...
export function NozzlesStep({ nozzles, pumps, tanks, products, onUpdate, onContinue, language }: Props) {
 ...
 className="w-full h-16 border-2 border-dashed border-border rounded-2xl flex items-center justify-center gap-3 text-muted-foreground hover:text-purple-600 hover:border-purple-400 hover:bg-purple-50 transition-all font-bold cursor-pointer"
 >
 <Plus className="size-6" />
 {t('Add New Nozzle', 'نئی نوزل شامل کریں', language)}
 </button>
 ) : (
 <div className="border border-border rounded-2xl p-6 space-y-5 bg-subtle shadow-sm animate-in zoom-in-95 duration-200">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div className="space-y-2">
 <label className="text-sm font-bold text-foreground">{t('Pump Name', 'پمپ کا نام', language)}</label>
 <input
 className="w-full h-12 px-4 bg-card border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors font-medium text-foreground"
 placeholder="Pump 1"
 value={pumpName}
 ...
 <div className="space-y-2">
 <label className="text-sm font-bold text-foreground">{t('Nozzle Name/No', 'نوزل کا نام', language)}</label>
 <input
 className="w-full h-12 px-4 bg-card border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors font-medium text-foreground"
 ...
 <div className="space-y-2">
 <label className="text-sm font-bold text-foreground">{t('Link to Tank', 'ٹینک سے منسلک کریں', language)}</label>
 <select 
 className="w-full h-12 px-4 bg-card border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors font-medium text-foreground appearance-none cursor-pointer"
 value={tankId} 
 ...
 <div className="space-y-2">
 <label className="text-sm font-bold text-foreground">{t('Opening Reading', 'ابتدائی ریڈنگ', language)}</label>
 <input
 type="number"
 className="w-full h-12 px-4 bg-card border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors font-medium text-foreground"
 placeholder="0.00"
 ...
 className="flex-1 h-12 bg-card hover:bg-slate-900 text-foreground rounded-xl font-bold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-md"
 >
 {t('Save & Add Another', 'محفوظ کریں اور مزید شامل کریں', language)}
 </button>
 <button 
 onClick={resetForm} 
 className="flex-[0.4] h-12 bg-card border border-border hover:bg-slate-50 text-foreground rounded-xl font-bold transition-colors cursor-pointer"
 >
 ...
 <h4 className="font-bold text-lg text-foreground">{nozzle.name} <span className="text-sm font-medium text-muted-foreground ml-2">({pump?.name})</span></h4>
 </div>
 <div className="flex items-center gap-2 ml-9">
 <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-card border border-purple-200 text-purple-700">
 {details?.tank?.name}
 </span>
 ...
 className="size-10 flex items-center justify-center rounded-xl hover:bg-red-100 text-muted-foreground hover:text-red-600 transition-colors cursor-pointer"
 >
 <X className="size-5" />
 </button>
 </div>
 <div className="ml-9 text-sm pt-2 border-t border-purple-100/50">
 <span className="text-muted-foreground font-medium">{t('Opening Reading:', 'ابتدائی ریڈنگ:', language)} </span>
 <span className="font-bold text-foreground">{Number(nozzle.startReading).toLocaleString()}</span>
 </div>
 </div>
 );
 })}

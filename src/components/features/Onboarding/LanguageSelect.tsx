...
export const LanguageSelect: React.FC<LanguageSelectProps> = ({ onSelect }) => {
 ...
 className="fixed inset-0 z-[9000] bg-background flex flex-col items-center justify-center p-6"
 >
 ...
 className="text-muted-foreground text-center mb-10 text-sm font-medium"
 >
 ...
 whileTap={{ scale: 0.98 }}
 onClick={() => onSelect('en')}
 className="w-full bg-card/5 hover:bg-card/10 border border-border p-5 rounded-2xl flex flex-col items-center justify-center gap-1 transition-colors cursor-pointer"
 >
 <span className="text-xl font-bold text-white tracking-wide font-sans">English</span>
 <span className="text-muted-foreground text-[10px] uppercase tracking-widest font-bold">English Language</span>
 </motion.button>
 </div>
 </div>
 ...
 >
 <PoweredByUmarAli variant="compact" showLogo={false} className="text-muted-foreground" />
 </motion.div>
 </motion.div>
 );
};

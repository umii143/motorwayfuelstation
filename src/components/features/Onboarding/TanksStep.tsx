...
export function TanksStep({ tanks, products, onUpdate, onContinue, language }: Props) {
 ...
 <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-card border border-blue-200 text-blue-700">
 ...
 <div>
 <span className="text-muted-foreground font-medium">{t('Capacity:', 'گنجائش:', language)} </span>
 <span className="font-bold text-foreground">{Number(tank.capacity).toLocaleString()} L</span>
 </div>
 <div>
 <span className="text-muted-foreground font-medium">{t('Stock:', 'اسٹاک:', language)} </span>
 <span className="font-bold text-foreground">{Number(tank.currentStock).toLocaleString()} L</span>
 </div>
 </div>
 </div>
 ))}

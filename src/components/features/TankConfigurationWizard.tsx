...
export default function TankConfigurationWizard({ onComplete, onCancel, currentLanguage = 'en', existingProducts = [], existingTanks = [] }: TankConfigurationWizardProps) {
 ...
 return (
 <div className="fixed inset-0 z-[9000] bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden">
 {/* Background Decor */}
 ...
 <button onClick={onCancel} className="p-2 text-muted-foreground hover:text-[var(--text-main)] hover:bg-slate-100 dark:hover:bg-card/5 rounded-full transition-colors">
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 'bg-[var(--bg-card)] border-border text-slate-500'
 }`}>
 ...
 {products.map((p, idx) => (
 <div key={idx} className="flex items-center justify-between bg-subtle /50 rounded-xl p-3 border border-border/50">
 <div>
 <p className="text-sm font-bold text-[var(--text-main)]">{p.name} <span className="text-xs text-muted-foreground font-normal">({p.type})</span></p>
 ...
 <div className="bg-subtle border border-border rounded-2xl p-4 space-y-4">
 ...
 <p className="text-xs font-semibold text-muted-foreground">Links to: <span className="text-[var(--primary-accent)]">{t.productName || 'None'}</span> • {t.capacity.toLocaleString()}L</p>
 </div>
 ...
 <p className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-2">Added Nozzles</p>
 {nozzles.map((n: any, idx: any) => (
 <div key={idx} className="flex items-center justify-between bg-subtle /50 rounded-xl p-3 border border-border/50">
 <div>
 <p className="text-sm font-bold text-[var(--text-main)]">{n.name}</p>
 <p className="text-xs font-semibold text-muted-foreground">Pump {n.pumpId} &bull; Links to: <span className="text-emerald-500">{n.tankName || 'None'}</span></p>
 </div>
 ...
 <div className="p-6 border-t border-[var(--border-main)] shrink-0 bg-subtle rounded-b-3xl">
 ...
 className="px-5 py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider bg-card border border-border text-[var(--text-main)] hover:bg-slate-50 dark:hover:bg-[var(--bg-hover)] transition-colors flex items-center gap-2 disabled:opacity-50 flex-1 sm:flex-none justify-center"
 >
 ...
 onClick={nextStep}
 disabled={isGenerating}
 className="px-5 py-3.5 rounded-xl font-bold text-xs uppercase tracking-wider bg-transparent border border-border text-muted-foreground hover:bg-slate-100 dark:hover:bg-card/5 transition-colors flex-1 sm:flex-none"
 >
 Skip Step
 </button>
 ) : (

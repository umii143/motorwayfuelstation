...
import { Plus, X, Gauge, AlertTriangle } from 'lucide-react';
...
export default function NozzleWizard({
 ...
 <div
 key={nozzle.id}
 className="border border-border bg-card shadow-sm rounded-2xl p-5 space-y-3 animate-in fade-in slide-in-from-left-4 transition-all hover:border-purple-300 hover:shadow-md"
 ...
 <h4 className="font-bold text-lg text-foreground">{nozzle.name} <span className="text-sm font-medium text-muted-foreground ml-1">({pump?.name})</span></h4>
 </div>
 ...
 <div className="ml-11 text-sm pt-3 border-t border-border mt-3">
 <span className="text-muted-foreground text-[10px] font-bold uppercase tracking-wider">{t('Opening Reading', 'ابتدائی ریڈنگ', language)} </span>
 <span className="font-black text-foreground ml-2">{Number(nozzle.startReading).toLocaleString()}</span>
 </div>
 </div>
 );
 })}

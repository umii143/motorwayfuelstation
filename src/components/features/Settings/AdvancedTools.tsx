...
export default function AdvancedTools({ settings, activeStationId }: { settings: GlobalSettings, activeStationId: string }) {
 ...
 return (
 <div key={tool.id} className="bg-card rounded-xl border border-border shadow-xs overflow-hidden flex flex-col transition-all hover:shadow-md">
 ...
 <div className="px-6 py-4 bg-subtle border-t border-border">
 <button 
 onClick={tool.action}
 disabled={isRunning}
 className="w-full flex justify-center items-center gap-2 px-4 py-2.5 sm:py-2 min-h-[48px] sm:min-h-[40px].5 bg-card border border-border text-foreground rounded-lg text-sm font-bold shadow-xs hover:bg-muted transition-colors disabled:opacity-70"
 >
 {isRunning ? (
 <>

...
export default function MidShiftRateModal({
 ...
 <label className="block text-sm font-medium text-foreground mb-1">{nz.name}</label>
 <div className="relative">
 <input
 type="number"
 value={readings[nz.id] || ''}
 onChange={(e) => setReadings({ ...readings, [nz.id]: e.target.value })}
 className="w-full pl-4 pr-12 py-2.5 bg-subtle border border-border rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
 placeholder="0.00"
 />
 ...
 <div className="p-6 border-t border-border bg-subtle flex justify-end gap-3">
 <button
 onClick={onCancel}

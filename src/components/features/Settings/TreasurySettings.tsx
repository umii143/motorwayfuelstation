...
export default function TreasurySettings({ settings, onUpdateSettings, activeStationId }: TreasurySettingsProps) {
 ...
 <div className="px-5 py-3.5 border-b border-border bg-subtle flex items-center gap-2">
 <Banknote className="h-4 w-4 text-muted-foreground" />
 <h3 className="text-sm font-bold text-foreground">{t('Cash Flow Rules', 'نقدی کے قواعد')}</h3>
 </div>
 ...
 className="w-full pl-7 pr-3 py-2.5 bg-subtle border border-border rounded-xl text-sm text-foreground font-medium focus:outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 transition-all"
 />
 </div>
 <p className="text-[11px] text-muted-foreground">{t('Cash kept in drawer at shift start', 'شفٹ شروع ہونے پر دراز میں رقم')}</p>
 </div>
 ...
 <p className="text-[11px] text-muted-foreground">{t('Alert when cash exceeds this amount', 'اس رقم سے زیادہ ہونے پر اطلاع')}</p>
 </div>
 ...
 <p className="text-[11px] text-muted-foreground">{t('Daily petty cash allowance', 'روزانہ پیٹی کیش الاؤنس')}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-sm font-bold text-foreground">{t('Automation & Alerts', 'خودکار اطلاعات')}</h3>
 </div>
 <div className="divide-y divide-border">
 {/* Safe Drop Alert */}
 <div className="flex items-center justify-between px-5 py-4">
 <div>
 <h4 className="text-sm font-bold text-foreground">{t('Safe Drop Alerts', 'سیف ڈراپ الرٹ')}</h4>
 <p className="text-xs text-muted-foreground mt-0.5">{t('Notify cashier when cash reaches the safe drop limit', 'نقدی حد تک پہنچنے پر کیشیر کو مطلع کریں')}</p>
 </div>
 <button
 ...
 <div className="flex items-center justify-between px-5 py-4">
 <div>
 <h4 className="text-sm font-bold text-foreground">{t('EOD Auto-Reconcile', 'روزانہ خودکار ملان')}</h4>
 <p className="text-xs text-muted-foreground mt-0.5">{t('Auto-generate treasury summary at end of each day', 'ہر دن کے اختتام پر ٹریژری خلاصہ خود بنائیں')}</p>
 </div>
 <button
 ...
 <h4 className="text-sm font-bold text-foreground">{t('Cash Count Reminder', 'نقدی گنتی یاددہانی')}</h4>
 <p className="text-xs text-muted-foreground mt-0.5">{t('Remind cashier to count cash every N minutes', 'ہر N منٹ بعد کیشیر کو نقدی گننے کی یاددہانی')}</p>
 </div>
 ...
 className="w-20 px-2 py-1.5 bg-subtle border border-border rounded-lg text-xs text-center text-foreground font-bold focus:outline-none focus:border-indigo-400"
 />

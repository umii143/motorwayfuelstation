...
export default function UnifiedAccountManager({
 ...
 {/* SECTION TABS HEADER */}
 <div className="flex bg-muted p-1.5 rounded-lg border border-border gap-1.5 select-none self-start">
 <button
 ...
 subTab === 'banks' ? 'bg-card text-slate-850 shadow-2xs' : 'text-slate-500 hover:text-foreground'
 }`}
 >
 ...
 className={`flex-1 py-1.5 px-3 rounded-md font-sans text-[11px] font-bold transition-all whitespace-nowrap cursor-pointer${
 subTab === 'digital' ? 'bg-card text-slate-850 shadow-2xs' : 'text-slate-500 hover:text-foreground'
 }`}
 >
 ...
 className={`flex-1 py-1.5 px-3 rounded-md font-sans text-[11px] font-bold transition-all whitespace-nowrap cursor-pointer${
 subTab === 'products' ? 'bg-card text-slate-850 shadow-2xs' : 'text-slate-500 hover:text-foreground'
 }`}
 >
 ...
 <h4 className="font-sans text-xs font-bold text-muted-foreground uppercase tracking-widest">{t('Current Corporate Accounts:', 'دفتر کے بینک اکاؤنٹس کی فہرست :')}</h4>
 {!showBankForm && (
 <button
 ...
 <h5 className="font-bold text-foreground uppercase tracking-tight text-[11px] border-b border-dashed border-border pb-1 flex items-center gap-1">
 ...
 <button type="button" onClick={() => setShowBankForm(false)} className="px-3 py-1 bg-muted text-muted-foreground rounded text-xs font-bold uppercase">{t('Cancel', 'کینسل')}</button>
 ...
 <div className="flex flex-col items-center justify-center p-4 border border-border bg-gradient-to-br from-slate-905 from-slate-900 to-slate-800 rounded-xl text-white relative overflow-hidden shadow-md select-none h-56 self-center">
 <div className="w-full flex justify-between items-start">
 <div>
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest font-bold">{t('Commercial Ledger Card', 'کمرشل بینک لیجر کارڈ')}</span>
 ...
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest">{t('Account Holder Title', 'کھاتہ دار')}</span>
 ...
 <div>
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest">{t('Account Number/IBAN', 'کھاتہ نمبر')}</span>
 <span className="block font-mono text-[10px] break-all tracking-wide">{bankForm.accountNo || 'PK00 XXXX XXXXXXX XXXXX'}</span>
 </div>
 <div className="text-right">
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest">{t('Active Ledger Balance', 'دفتری بیلنس')}</span>
 ...
 <div className="col-span-2 text-center py-8 text-muted-foreground text-xs border border-dashed rounded-xl bg-card">{t('No registered traditional bank accounts yet.', 'کوئی عمومی بینک اکاؤنٹ رجسٹرڈ نہیں ہے۔')}</div>
 ) : (
 normalBanks.map(b => {
 const [bankName, ...titleParts] = b.name.split(' | ');
 ...
 return (
 <div key={b.id} className="rounded-xl border border-border bg-card p-4 shadow-2xs hover:shadow-xs flex justify-between items-center transition-shadow">
 <div>
 <div className="text-[10px] bg-slate-150 text-foreground px-2.5 py-0.5 inline-block rounded-full font-mono font-bold uppercase">{bankNameVal}</div>
 <h4 className="font-sans font-bold text-foreground text-xs mt-1.5 uppercase leading-none">{titleVal}</h4>
 <div className="font-mono text-[10px] text-muted-foreground mt-2">{b.accountNo}</div>
 </div>
 <div className="text-right flex flex-col justify-between h-20 items-end">
 <strong className="font-mono text-xs text-foreground block">Rs. {b.balance?.toLocaleString()}</strong>
 ...
 <div className="space-y-4">
 <div className="flex justify-between items-center">
 <h4 className="font-sans text-xs font-bold text-muted-foreground uppercase tracking-widest">{t('Mobile Wallets Account List:', 'موبائل والٹ اور ٹیلی کام تجارتی کھاتہ جات :')}</h4>
 {!showWalletForm && (
 <button
 ...
 <div className="flex gap-2 justify-end pt-2 border-t mt-3">
 <button type="button" onClick={() => setShowWalletForm(false)} className="px-3 py-1 bg-muted text-muted-foreground rounded text-xs font-bold uppercase">{t('Cancel', 'کینسل')}</button>
 ...
 <div className="flex flex-col items-center justify-center p-4 border border-border bg-card rounded-2xl relative shadow-lg min-h-[220px] select-none text-[11px] font-mono text-muted-foreground w-64 mx-auto self-center">
 ...
 <div className="w-full text-center border-b border-dashed border-border pb-2 mb-2">
 <span className="block text-white font-sans font-black uppercase text-[10px] tracking-tight">{walletForm.provider}</span>
 <span className="block text-[9px] text-muted-foreground mt-1">{t('Till Account:', 'مرچنٹ اکاؤنٹ:')} {walletForm.tillNumber || 'XXXXXX'}</span>
 </div>

 <div className="w-full space-y-1 text-[10px] text-muted-foreground">
 ...
 <span className="font-sans text-[8px] text-muted-foreground uppercase">{t('TILL WALLET BALANCE', 'موجودہ بیلنس')}</span>
 <strong className="text-emerald-400 font-bold text-xs">Rs. {walletForm.balance.toLocaleString()}</strong>
 </div>
 </div>

 <div className="w-full pt-3 mt-3 border-t border-dashed border-border text-center text-[7px] text-muted-foreground font-sans uppercase">
 ...
 <div className="col-span-2 text-center py-8 text-muted-foreground text-xs border border-dashed rounded-xl bg-card">{t('No mobile wallets registered in system.', 'کوئی موبائل یا ڈیجیٹل تجارتی والٹ درج نہیں ہے۔')}</div>
 ) : (
 ...
 return (
 <div key={w.id} className="rounded-xl border border-border bg-card p-4 shadow-2xs hover:shadow-xs flex justify-between items-center border-l-4 border-l-violet-605 border-l-violet-600 transition-shadow">
 <div>
 ...
 <h4 className="font-sans font-bold text-foreground text-xs mt-1.5 uppercase leading-none">{titleVal}</h4>
 <div className="font-mono text-[10px] text-muted-foreground mt-1.5">{t('Till Number:', 'ٹِل نمبر / سم:')} {w.accountNo}</div>
 </div>
 <div className="text-right flex flex-col justify-between h-20 items-end">
 <strong className="font-mono text-xs text-foreground block font-black">Rs. {w.balance?.toLocaleString()}</strong>
 <div className="flex gap-2 select-none">
 ...
 <div className="space-y-4">
 <div className="flex justify-between items-center">
 <h4 className="font-sans text-xs font-bold text-muted-foreground uppercase tracking-widest">{t('Fuel & Lubricants Catalog Setup:', 'اسٹیشن انونٹری اور سیلز کیٹلاگ لسٹ :')}</h4>
 {!showProductForm && (
 <button
 ...
 className="w-full rounded border border-border bg-card p-2 text-xs font-sans"
 >
 ...
 <div className="rounded-xl border border-dashed border-border p-4 bg-subtle flex flex-col justify-between select-none font-sans text-xs text-slate-650 h-56 self-center">
 <div>
 <div className="flex justify-between items-start">
 <span className="text-[10px] bg-slate-200 text-foreground px-2 py-0.5 font-bold uppercase rounded">{t('SKU Item Preview', 'کارڈ پری ویو')}</span>
 <strong className="font-mono text-xs text-orange-650 uppercase font-bold">{prodForm.type}</strong>
 </div>
 <h4 className="font-bold text-foreground text-sm mt-3 uppercase tracking-tight">{prodForm.name || t('Product Name (English)', 'سامان کا نام')}</h4>
 ...
 <div>
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest">{t('Current retail rate:', 'سیلز ریٹ:')}</span>
 <strong className="block font-sans text-xs text-foreground">Rs. {prodForm.rate} / {prodForm.unit}</strong>
 </div>
 <div className="text-right">
 <span className="block text-[8px] text-muted-foreground uppercase tracking-widest">{t('Catalog Stock levels:', 'ابتدائی انونٹری اسٹاک:')}</span>
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 {products.length === 0 ? (
 <div className="col-span-2 text-center py-8 text-muted-foreground text-xs border border-dashed rounded-xl bg-card">{t('No registered product catalogues yet.', 'کوئی بھی مصنوعات رجسٹرڈ نہیں ہے۔')}</div>
 ) : (
 products.map(p => (
 <div key={p.id} className="rounded-xl border border-border bg-card p-4 shadow-2xs hover:shadow-xs flex justify-between items-center transition-shadow">
 <div>
 <div className="text-[10px] bg-muted text-muted-foreground px-2 py-0.5. inline-block rounded font-black uppercase font-mono">{p.type}</div>
 <h4 className="font-sans font-bold text-foreground text-xs mt-1.5 uppercase leading-none">{t(p.name, p.urduName)}</h4>
 <p className="text-[10.5px] text-muted-foreground mt-1">{t('Tariff:', 'سیلز ریٹ :')} Rs. {p.rate?.toFixed(2)} / {p.unit}</p>
 </div>
 <div className="text-right flex flex-col justify-between h-16 items-end">
 <div className="text-[10.5px] text-slate-450 leading-none">{t('In Stock Inventory:', 'موجودہ اسٹاک:')} <strong className="font-mono text-xs text-foreground font-bold">{(p.currentStock || 0).toLocaleString()} {p.unit}</strong></div>

...
export default function DealerMarginWizard({ language, onLogAudit, stationId }: DealerMarginWizardProps) {
 ...
 className="flex items-center gap-1 bg-card hover:bg-slate-700 text-foreground px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
 >
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1">{t('Fuel Type', 'پراڈکٹ', language)} *</label>
 <select
 value={productType}
 onChange={(e) => setProductType(e.target.value)}
 className="w-full rounded border border-orange-200 px-3 py-2 text-sm bg-card outline-none focus:border-orange-500"
 >
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1">{t('Margin (Rs/Liter)', 'مارجن (روپے فی لیٹر)', language)} *</label>
 <input
 type="number"
 step="0.01"
 required
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1">{t('Effective From', 'تاریخ نفاذ', language)} *</label>
 <input
 type="date"
 required
 value={effectiveFrom}
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground mb-1">{t('Notification Ref / Notes', 'حوالہ جات', language)}</label>
 <input
 type="text"
 value={notes}
 ...
 <div className="bg-subtle px-4 py-3 border-b border-border flex justify-between items-center">
 <h4 className="font-bold text-foreground uppercase">{prod}</h4>
 ...
 <div>
 <div className="font-bold text-foreground text-sm">Rs. {s.marginPerLiter.toFixed(2)}</div>
 <div className="text-xs text-muted-foreground mt-0.5">Effective: {s.effectiveFrom}</div>
 {s.notes && <div className="text-[10px] text-muted-foreground mt-1 italic">{s.notes}</div>}
 </div>
 </div>
 ))}
 {settingsForProd.length === 0 && (
 <div className="p-4 text-center text-xs text-muted-foreground">No margin history found.</div>
 )}
 </div>
 </div>
 );

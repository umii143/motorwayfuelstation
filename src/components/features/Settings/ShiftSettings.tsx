...
export default function ShiftSettings({ settings, onUpdateSettings, activeStationId }: { settings: GlobalSettings, onUpdateSettings: (s: GlobalSettings) => void, activeStationId: string }) {
 ...
 <div className="bg-card rounded-xl border border-border shadow-xs overflow-hidden max-w-2xl">
 <div className="p-6 space-y-6">
 
 <div className="flex items-center justify-between">
 <div>
 <h4 className="text-sm font-bold text-foreground">{t('Enforce Shift Hours', 'شفٹ کے اوقات نافذ کریں')}</h4>
 <p className="text-xs text-muted-foreground">{t('Block POS transactions when no active shift is running.', 'جب کوئی فعال شفٹ نہ چل رہی ہو تو پی او ایس لین دین کو روکیں۔')}</p>
 </div>
 ...
 <div className="flex items-center justify-between">
 <div>
 <h4 className="text-sm font-bold text-foreground">{t('Auto-Close Abandoned Shifts', 'ترک شدہ شفٹوں کو خودکار بند کریں')}</h4>
 <p className="text-xs text-muted-foreground">{t('Automatically force-close shifts exceeding max duration.', 'زیادہ سے زیادہ دورانیے سے تجاوز کرنے والی شفٹوں کو خودکار طور پر بند کریں۔')}</p>
 </div>
 <label className="relative inline-flex items-center cursor-pointer">
 ...
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Max Shift Duration (Hours)', 'شفٹ کا زیادہ سے زیادہ دورانیہ (گھنٹے)')}</label>
 <input 
 type="number" 

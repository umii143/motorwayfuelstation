...
export default function PriceImpactSimulatorModal({
 ...
 return createPortal(
 <div className="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-card backdrop-blur-sm">
 <div className="bg-card rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
 
 <div className="bg-card p-6 flex justify-between items-center text-foreground sticky top-0 z-10">
 <div>
 ...
 <p className="text-xs text-muted-foreground font-bold mb-1">Current Stock</p>
 <p className="text-lg font-black text-foreground">{totalStock.toLocaleString()} L</p>
 </div>
 <div className="bg-subtle p-4 rounded-xl border border-border">
 <p className="text-xs text-muted-foreground font-bold mb-1">Price Diff</p>
 ...
 <p className="text-xs text-muted-foreground font-bold mb-2">Conservative (30d Avg)</p>
 ...
 <div className="p-4 rounded-xl border border-border bg-card shadow-sm">
 <p className="text-xs text-muted-foreground font-bold mb-2">Aggressive (7d Avg)</p>
 ...
 className="w-full bg-subtle border border-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-medium text-foreground"
 >
 ...
 <div className="border-2 border-dashed border-border rounded-xl p-6 bg-subtle flex flex-col items-center justify-center text-center">
 <input 
 type="file" 
 id="circular-upload" 
 className="hidden" 
 ...
 className={`cursor-pointer flex items-center gap-2 px-4 py-2 bg-card border border-border shadow-sm rounded-lg font-bold text-sm${isUploading ? 'opacity-50' : 'hover:bg-muted'}`}
 >
 <Upload className="size-4 text-muted-foreground" />
 {isUploading ? 'Uploading...' : 'Attach PDF or Image'}
 </label>
 <p className="text-xs text-muted-foreground mt-2">Optional for historical evidence.</p>
 </div>

 {attachments.length > 0 && (
 <div className="mt-4 space-y-2">
 {attachments.map(att => (
 <div key={att.id} className="flex items-center justify-between p-3 bg-card border border-border rounded-lg">
 <div className="flex items-center gap-3">
 <FileIcon className="size-5 text-indigo-500" />
 <div>
 <p className="text-sm font-bold text-foreground">{att.fileName}</p>
 ...
 <div className="p-6 border-t border-border bg-subtle flex justify-end gap-3 sticky bottom-0">
 <button
 onClick={onCancel}
 className="px-6 py-3 sm:py-2 min-h-[48px] sm:min-h-[40px].5 rounded-xl font-medium text-muted-foreground hover:bg-slate-200 transition-colors"
 >
 ...
 disabled={!reason}
 className="px-6 py-3 sm:py-2 min-h-[48px] sm:min-h-[40px].5 rounded-xl font-bold text-foreground bg-card hover:bg-slate-900 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >

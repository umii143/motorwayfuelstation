...
export default function MeterManagement({ settings, activeStationId }: { settings: GlobalSettings, activeStationId: string }) {
 ...
 activeTab === 'setup' ? 'bg-card text-rose-600 shadow-sm' : 'text-slate-600 hover:text-foreground'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold transition-colors${
 activeTab === 'history' ? 'bg-card text-rose-600 shadow-sm' : 'text-slate-600 hover:text-foreground'
 }`}
 >
 ...
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Select Nozzle', 'نوزل منتخب کریں')}</label>
 <select 
 value={selectedNozzleId}
 onChange={(e) => setSelectedNozzleId(e.target.value)}
 className="w-full px-3 py-2.5 bg-subtle border border-border rounded-lg text-sm font-semibold text-foreground focus:border-rose-500 focus:ring-1 focus:ring-rose-500"
 >
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Current Reading', 'موجودہ ریڈنگ')}</label>
 <input 
 type="text" 
 value={currentReading}
 disabled
 className="w-full px-3 py-2.5 bg-muted border border-border rounded-lg text-sm font-mono font-bold text-muted-foreground cursor-not-allowed"
 />
 </div>
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider text-rose-600">{t('New Reading', 'نئی ریڈنگ')}</label>
 <input 
 type="number" 
 value={newReading}
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Reason for Reset', 'ری سیٹ کی وجہ')}</label>
 <textarea 
 value={reason}
 onChange={(e) => setReason(e.target.value)}
 placeholder="e.g. Dispenser meter replaced, System error jump, Meter Rollover..."
 className="w-full px-3 py-2.5 bg-subtle border border-border rounded-lg text-sm text-foreground focus:border-rose-500 focus:ring-1 focus:ring-rose-500 min-h-[80px]"
 />
 </div>
 ...
 className="px-6 py-3 bg-card text-foreground rounded-lg text-sm font-bold shadow-xs hover:bg-slate-800 transition-colors flex items-center gap-2"
 >
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Difference</p>
 <p className="text-2xl font-mono font-bold text-foreground">
 {readingDifference > 0 ? '+' : ''}{readingDifference.toFixed(2)}
 </p>
 </div>
 <div className="bg-card border border-border rounded-xl p-4 shadow-xs text-center">
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Financial Check</p>
 ...
 <div className="bg-card border border-border rounded-xl p-4 shadow-xs text-center">
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Stock Snapshot</p>
 ...
 <h4 className="text-sm font-bold text-foreground mb-2">Record Capture Summary</h4>
 ...
 <button 
 onClick={() => setStep(1)}
 className="px-4 py-2 bg-card text-muted-foreground border border-border rounded-lg text-sm font-bold shadow-xs hover:bg-slate-50 transition-colors"
 >
 {t('Back', 'پیچھے')}
 </button>
 <button 
 ...
 <h3 className="text-xl font-bold text-foreground">{t('Owner Authorization', 'مالک کی اجازت')}</h3>
 <p className="text-sm text-muted-foreground mt-1">This action is irreversible and fully audited.</p>
 </div>

 <div className="space-y-4 bg-subtle p-6 rounded-xl border border-border">
 {settings.security?.requirePinForMeterReset !== false && (
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Master Owner PIN', 'ماسٹر اونر پن')}</label>
 ...
 maxLength={6}
 placeholder="******"
 className="w-full pl-10 pr-3 py-3 bg-card border border-border rounded-lg text-lg font-mono font-bold text-foreground focus:border-rose-500 focus:ring-1 focus:ring-rose-500 text-center tracking-widest"
 />
 </div>
 </div>
 )}
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Type"RESET METER" to confirm', 'تصدیق کے لیے"RESET METER" ٹائپ کریں')}</label>
 <input 
 type="text" 
 value={confirmationText}
 onChange={(e) => setConfirmationText(e.target.value)}
 placeholder="RESET METER"
 className="w-full px-3 py-3 bg-card border border-border rounded-lg text-sm font-mono font-bold text-foreground focus:border-rose-500 focus:ring-1 focus:ring-rose-500 text-center"
 />
 </div>
 ...
 <button 
 onClick={() => setStep(2)}
 className="w-full px-4 py-3 bg-transparent text-muted-foreground rounded-lg text-sm font-bold hover:text-slate-700 transition-colors"
 >
 {t('Cancel', 'کینسل')}
 </button>
 </div>
 ...
 className="w-full pl-9 pr-3 py-2 bg-subtle border border-border rounded-lg text-sm focus:border-slate-400 focus:ring-1 focus:ring-slate-400"
 />
 </div>
 </div>
 ...
 className="w-full sm:w-40 pl-9 pr-3 py-2 bg-subtle border border-border rounded-lg text-sm focus:border-slate-400 focus:ring-1 focus:ring-slate-400"
 />
 </div>
 </div>
 ...
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Date & Time', 'تاریخ اور وقت')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Nozzle', 'نوزل')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">{t('Old Reading', 'پرانی ریڈنگ')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">{t('New Reading', 'نئی ریڈنگ')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">{t('Stock At Reset', 'ری سیٹ پر اسٹاک')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">{t('Price At Reset', 'ری سیٹ پر قیمت')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Salesman', 'سیلزمین')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('Auth By', 'اجازت دہندہ')}</th>
 <th className="px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider max-w-xs">{t('Reason', 'وجہ')}</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-border">
 {filteredHistory.length > 0 ? (
 filteredHistory.map((reset) => (
 <tr key={reset.id} className="hover:bg-slate-50 transition-colors">
 ...
 <td className="px-4 py-3 text-sm text-muted-foreground max-w-xs truncate" title={reset.reason}>
 {reset.reason}
 </td>
 </tr>
 ))

...
export default function SystemAuditTrail({ language, stationId }: SystemAuditTrailProps) {
 ...
 <h3 className="font-sans text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
 ...
 className="w-full pl-9 pr-3 py-1.5 border border-border rounded-lg text-xs font-sans outline-hidden focus:border-orange-500 bg-subtle"
 />
 </div>
 ...
 className="rounded-lg border border-border bg-card p-1.5 text-xs font-sans text-muted-foreground outline-hidden focus:border-orange-500"
 >
 ...
 accessor: (lg) => (
 <span className="bg-muted text-slate-650 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-tight">
 {lg.category}
 </span>
 )
 },
 ...
 accessor: (lg) => (
 <span className="text-muted-foreground" title={lg.details}>{lg.details}</span>
 )
 },
 {
 ...
 accessor: (lg) => (
 <span className="text-sm font-medium text-foreground">{lg.user}</span>
 )
 }
 ]}

...
export default function UnifiedRoznamcha({ settings }: UnifiedRoznamchaProps) {
 ...
 className="w-full flex items-center justify-center gap-1.5 rounded-lg bg-card text-foreground text-xs font-bold py-2.5 hover:bg-slate-800 transition-colors cursor-pointer disabled:opacity-40"
 >
...
function EventRow({ e, onClick, t }: { e: BusinessEvent; onClick: () => void; t: (en: string, ur: string) => string }) {
 return (
 <button onClick={onClick} className="w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-orange-50/10 dark:hover:bg-card/5 transition-colors">

    ...
    <div className="space-y-6">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-5 shadow-md space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[var(--border-main)]">
          ...
          <span className="px-3 py-1 rounded-xl bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-500/20 font-bold text-xs font-mono">
            4 / 4 Stations Online
          </span>
        </div>
          ...
          {branches.map((st) => (
            <div key={st.name} className="p-4 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] space-y-2">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-500/20">
                  🟢 {st.status}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-1">
                ...
                <div className="text-right">
                  ...
                  <span className="font-bold text-emerald-700 dark:text-emerald-400">{formatCurrency(st.profit)}</span>
                </div>
              </div>
            ...
            {pumps.map((p) => (
              <div key={p.id} className="p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] space-y-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${p.status === 'active' ? 'bg-emerald-500/10 text-emerald-800' : 'bg-amber-500/10 text-amber-900'}`}>{p.status}</span>
                </div>
            ...
            {tankTelemetry.map((t) => (
              <div key={t.id} className={`p-3 rounded-xl border ${t.isReorderRisk ? 'bg-rose-500/10 border-rose-500/30' : 'bg-[var(--bg-subtle)] border-[var(--border-main)]'} flex justify-between items-center`}>
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400">{t.daysRemaining} Days Remaining</span>
                </div>
              </div>
            ))}
          </div>

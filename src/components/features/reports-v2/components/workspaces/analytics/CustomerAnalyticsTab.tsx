    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-black text-primary dark:text-primary font-mono mt-1">14.2 Days</div>
          <div className="text-xs text-primary dark:text-primary font-bold mt-1">Within 15-day Credit Policy</div>
        </div>
          ...
          {[
            ...
            <div key={c.customer} className="p-3.5 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${c.status === 'Overdue' ? 'bg-rose-500/10 text-rose-800' : 'bg-primary/10 text-primary'}`}>{c.status}</span>
              </div>
            </div>
          ))}
        </div>

    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-4 shadow-sm">
          ...
          <div className="text-2xl font-black text-emerald-700 dark:text-emerald-400 font-mono mt-1">{formatCurrency(metrics.grossProfit)}</div>
          <div className="text-xs text-emerald-700 dark:text-emerald-400 font-bold mt-1">12.0% Margin ({formatCurrency(metrics.targetGrossProfit)} Target)</div>
        </div>

        <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-4 shadow-sm">
          <div className="text-[10px] uppercase font-bold text-[var(--text-muted)]">Net Operational Profit</div>
          <div className="text-2xl font-black text-emerald-700 dark:text-emerald-400 font-mono mt-1">{formatCurrency(metrics.netProfit)}</div>
          <div className="text-xs text-emerald-700 dark:text-emerald-400 font-bold mt-1">↑ +5.2% MTD ({formatCurrency(metrics.targetNetProfit)} Target)</div>
        </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-2xl p-5 shadow-md space-y-4">
        ...
        <div className="space-y-3 font-mono text-xs">
          <div className="flex justify-between p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] font-bold">
            ...
            <span className="text-emerald-700 dark:text-emerald-400">{formatCurrency(metrics.grossRevenue)}</span>
          </div>
          <div className="flex justify-between p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] font-bold">
          ...
          <div className="flex justify-between p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 font-bold">
            <span className="text-emerald-800 dark:text-emerald-300">3. Gross Margin (12.0%)</span>
            <span className="text-emerald-800 dark:text-emerald-300">{formatCurrency(metrics.grossProfit)}</span>
          </div>
          <div className="flex justify-between p-3 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-main)] font-bold">

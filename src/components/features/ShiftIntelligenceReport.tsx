...
export default function ShiftIntelligenceReport({
 ...
 slate: 'bg-slate-50 /10 border-slate-500/20 text-slate-600',
 ...
 gray: 'bg-slate-50 /10 border-slate-500/20 text-slate-600'
 };
 ...
 className="bg-subtle border border-border rounded-lg text-sm px-3 py-2 font-bold text-foreground outline-none focus:ring-2 focus:ring-orange-500"
 >
 {sortedShifts.map(s => (
 ...
 <button onClick={() => window.print()} className="flex items-center gap-1.5 rounded-lg border border-border px-4 py-2 text-xs font-bold text-foreground shadow-xs hover:bg-slate-50 dark:hover:bg-card/5 cursor-pointer">
 <Printer className="h-4 w-4" /> {t('Print Report', 'رپورٹ پرنٹ کریں')}
 </button>
 </div>

 {/* ===== SHIFT REPORT HEADER ===== */}
 <div className="rounded-xl border border-border bg-gradient-to-br from-slate-50 to-white dark:from-[#151521] dark:to-[#0f0f15] p-5 shadow-sm">
 <div className="flex items-center gap-2 border-b border-border pb-3 mb-4">
 <ShieldCheck className="h-5 w-5 text-orange-600" />
 <h3 className="font-sans text-sm font-black uppercase tracking-widest text-foreground">{t('Shift Report Header', 'شفٹ رپورٹ ہیڈر')}</h3>
 ...
 {/* ===== SHIFT KPI SUMMARY ===== */}
 <div>
 <h3 className="font-sans text-sm font-black uppercase tracking-widest text-foreground mb-3 flex items-center gap-2">
 ...
 className="text-left rounded-xl border bg-card p-3.5 shadow-xs flex flex-col gap-2 hover:shadow-md hover:border-orange-500 transition-all group cursor-pointer"
 >
 <div className={`w-7 h-7 rounded-lg flex items-center justify-center border${colorMap[card.color]}`}>{card.icon}</div>
 <span className="font-sans text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-snug">{t(card.label, card.urdu)}</span>
 <strong className="font-mono text-sm font-bold text-foreground group-hover:text-orange-600 transition-colors truncate">{fmt(card.value, card.note)}</strong>
 ...
 <button key={g} onClick={() => setActiveDrill('liters_' + g)} className="rounded-xl border border-border bg-card p-4 text-left hover:border-orange-500 transition-colors cursor-pointer">
 <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block">{g.toUpperCase()}</span>
 <strong className="font-mono text-base font-bold text-foreground">{m.byGrade[g].toLocaleString()} Ltr</strong>
...
function HeaderItem({ icon, label, value, span }: { icon: React.ReactNode; label: string; value: string; span?: boolean }) {
 ...
 <span className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{icon}{label}</span>
 <strong className="block text-foreground mt-1 truncate">{value}</strong>
 </div>
 );
}
...
function SectionCard({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
 ...
 <h3 className="font-sans text-sm font-black uppercase tracking-widest text-foreground mb-4 flex items-center gap-2 border-b border-border pb-2">
 {icon} {title}
 </h3>
 {children}
 </div>
...
function WizardTimeline({ shift, m, t, settings, staff, custName }: any) {
 ...
 {steps.map((s, i) => (
 <div key={i} className="flex items-center justify-between gap-2 rounded-lg border border-border bg-subtle px-3 py-2.5">
...
function DrillDownModal({ drillKey, onClose, shift, m, settings, products, staff, customers, suppliers, banks, digitalAccounts, nozzles, tanks, lubePosSales, activityLogs, staffName, custName, bankName, t }: any) {
 ...
 <div className="w-full max-w-5xl bg-card rounded-2xl shadow-2xl flex flex-col max-h-[88vh] overflow-hidden border border-border">
 <div className="flex items-center justify-between p-5 border-b border-border bg-subtle">
 <h3 className="font-black text-base text-foreground flex items-center gap-2">
 <FileBarChart2 className="w-5 h-5 text-orange-500" />
 {title}
 <span className="px-2 py-0.5 rounded-full bg-slate-200 text-[10px] font-black text-muted-foreground ml-2">{data.length}</span>
 </h3>
 ...
 {data.length === 0 ? (
 <div className="flex flex-col items-center justify-center py-16 text-muted-foreground font-sans text-sm gap-2">
 ...
 <button onClick={onClose} className="px-4 py-2 rounded-lg bg-card text-foreground text-xs font-bold hover:bg-slate-900 cursor-pointer">{t('Close', 'بند کریں')}</button>
 </div>
 </div>
 </div>
 );

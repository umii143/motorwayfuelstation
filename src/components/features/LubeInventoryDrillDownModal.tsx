...
export default function LubeInventoryDrillDownModal({
 ...
 className="relative w-full max-w-5xl max-h-[90vh] bg-subtle rounded-2xl shadow-2xl overflow-hidden flex flex-col"
 >
 {/* HEADER */}
 ...
 <h3 className="text-2xl font-black text-foreground">{metrics.totalQty.toLocaleString()} <span className="text-sm">Units</span></h3>
 ...
 <h3 className="text-2xl font-black text-foreground">{formatCurrency(metrics.totalValue, settings)}</h3>
 <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
 ...
 <div className={`${metrics.lowStockCount > 0 ? 'bg-red-50 border-red-200' : 'bg-slate-50 dark:bg-card/5 border-border'}rounded-xl border p-5 shadow-sm`}>
 ...
 <h3 className={`text-2xl font-black${metrics.lowStockCount > 0 ? 'text-red-900' : 'text-foreground'}`}>
 ...
 return (
 <tr key={prod.id} className={`hover:bg-slate-50 transition-colors${isLow ? 'bg-red-50/30' : ''}`}>
 <td className="p-3">
 ...
 <td className="p-3 text-right">
 <span className={`font-mono text-[10px] font-black px-2 py-1 rounded-md${margin > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-muted text-slate-500'}`}>

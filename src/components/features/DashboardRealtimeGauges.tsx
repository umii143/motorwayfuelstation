...
export function DashboardRealtimeGauges({ settings, products = [], tanks = [], activeStationId }: { settings: GlobalSettings; products?: Product[]; tanks?: Tank[]; activeStationId: string }) {
 ...
 <div className="bg-card rounded-2xl p-6 shadow-xl border border-border text-center flex flex-col justify-center items-center">
 ...
 <p className="text-muted-foreground font-sans text-xs mt-1 font-bold uppercase tracking-widest">Local Station Time</p>
 </div>
 ...
 <div className="absolute inset-x-0 bottom-0 bg-muted h-2">
 <div 
 ...
 <div>
 <h4 className="font-sans font-black text-foreground text-lg">{tank.name}</h4>
 <p className="font-sans text-xs font-bold text-muted-foreground uppercase tracking-widest">Live Level</p>
 </div>
 ...
 <span className="font-sans text-sm font-bold text-muted-foreground mb-1">/ {capacity.toLocaleString()} L</span>
 </div>
 </div>
 );
 })}

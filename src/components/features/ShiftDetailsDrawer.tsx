...
export function ShiftDetailsDrawer({
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-card backdrop-blur-sm animate-in fade-in duration-200">
 <div className="w-full sm:w-[95vw] lg:w-[1200px] max-w-full h-[95vh] bg-background sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
 
 {/* HEADER */}
 <div className="flex-none bg-card border-b border-border px-4 sm:px-6 py-4 flex items-center justify-between z-10">
 <div className="flex items-center gap-4">
 <button onClick={onClose} className="p-2 -ml-2 text-muted-foreground hover:text-foreground dark:hover:text-white rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
 ...
 <button className="hidden sm:flex p-2 text-muted-foreground hover:text-foreground dark:hover:text-white rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
 <Share2 className="w-5 h-5" />
 </button>
 <button className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-foreground bg-muted hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors">
 <Edit2 className="w-4 h-4" /> Edit
 </button>
 <button className="p-2 text-muted-foreground hover:text-foreground dark:hover:text-white rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
 ...
 <div className="flex items-center gap-4">
 <div className="w-14 h-14 rounded-full bg-orange-100 flex items-center justify-center border-2 border-orange-200 overflow-hidden">
 <UserAvatarIcon />
 </div>
 <div>
 <h3 className="text-lg font-bold text-foreground">{getStaffName(shift.staffId)}</h3>
 <div className="flex items-center gap-2 mt-0.5">
 <span className="text-orange-600 font-semibold text-sm">{getStaffRole(shift.staffId)}</span>
 <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-md">EMP-001</span>
 </div>
 <div className="text-xs text-muted-foreground mt-1.5 flex items-center gap-1.5">
 Joined: 09:15 AM <span className="w-1 h-1 rounded-full bg-emerald-500"></span> <span className="text-emerald-600">On Time</span>
 </div>
 </div>
 </div>
 <div className="flex flex-col items-end">
 <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-full">Closed</span>
 ...
 <Clock className="w-3.5 h-3.5 text-muted-foreground" /> 10h 30m
 </div>
 <div className="text-[10px] text-muted-foreground mt-0.5">09:15 AM - 07:45 PM</div>
 </div>
 </div>
 </div>
 ...
 <span className="p-1.5 bg-emerald-100 text-emerald-600 rounded-lg"><TrendingUp className="w-4 h-4" /></span>
 Cash Flow Summary
 </h3>
 ...
 <span className="font-medium text-foreground">PKR 10,000</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-muted-foreground">Cash Sales</span>
 <span className="font-medium text-foreground">{formatCurrency(cashSales)}</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-muted-foreground">Other Cash In</span>
 <span className="font-medium text-foreground">{formatCurrency(totalRecoveries)}</span>
 </div>
 <div className="flex justify-between text-sm pb-3 border-b border-border/60">
 ...
 <span className="font-medium text-foreground">{formatCurrency(shift.expectedCash || 0)}</span>
 </div>
 <div className="flex justify-between text-sm">
 <span className="text-muted-foreground">Actual Closing</span>
 <span className="font-medium text-foreground">{formatCurrency(shift.submittedCash || 0)}</span>
 </div>
 <div className="flex justify-between text-sm pt-2">
 <span className="text-red-600 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Difference</span>
 <span className="font-bold text-red-600">{shift.shortage > 0 ? `- ${formatCurrency(shift.shortage)}` : formatCurrency(shift.overage)}</span>
 </div>
 </div>
 
 <div className="w-1/2 flex flex-col items-center justify-center pl-4 border-l border-border/60">
 ...
 <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Cash Collection</div>
 <div className="text-xl font-bold text-foreground">{formatCurrency(totalCashCollected)}</div>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* KEY HIGHLIGHTS */}
 <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
 <h3 className="text-sm font-bold text-foreground mb-4">Key Highlights</h3>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
 <div className="p-3 bg-background rounded-xl border border-border/60 flex flex-col items-center justify-center text-center">
 <span className="text-[10px] text-muted-foreground mb-1">Total Sales (PKR)</span>
 <span className="font-bold text-foreground text-sm">{formatCurrency(totalSales).replace('PKR', '').trim()}</span>
 <span className="text-[10px] text-emerald-600 mt-1 flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5"/> 12.5%</span>
 </div>
 <div className="p-3 bg-background rounded-xl border border-border/60 flex flex-col items-center justify-center text-center">
 <span className="text-[10px] text-muted-foreground mb-1">Total Profit (PKR)</span>
 <span className="font-bold text-foreground text-sm">{formatCurrency(totalProfit).replace('PKR', '').trim()}</span>
 <span className="text-[10px] text-emerald-600 mt-1 flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5"/> 8.3%</span>
 </div>
 <div className="p-3 bg-background rounded-xl border border-border/60 flex flex-col items-center justify-center text-center">
 <span className="text-[10px] text-muted-foreground mb-1">Credit Sales (PKR)</span>
 <span className="font-bold text-foreground text-sm">{formatCurrency(totalDebits).replace('PKR', '').trim()}</span>
 <span className="text-[10px] text-red-600 mt-1 flex items-center gap-0.5"><TrendingDown className="w-2.5 h-2.5"/> 26.0%</span>
 </div>
 <div className="p-3 bg-background rounded-xl border border-border/60 flex flex-col items-center justify-center text-center">
 <span className="text-[10px] text-muted-foreground mb-1">Transactions</span>
 <span className="font-bold text-foreground text-sm">{totalTransactions}</span>
 <span className="text-[10px] text-emerald-600 mt-1 flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5"/> 5.2%</span>
 </div>
 </div>
 </div>
 ...
 <div>
 <h3 className="text-sm font-bold text-foreground mb-3">Financial Overview</h3>
 ...
 <h3 className="text-sm font-bold text-foreground">Sales Breakdown (By Category)</h3>
 ...
 {/* ACTIONS */}
 <div className="bg-card rounded-2xl border border-border p-5 shadow-sm">
 <h3 className="text-sm font-bold text-foreground mb-4">Actions</h3>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
 ...
 <ActionButton icon={<Printer className="w-4 h-4"/>} label="Print Shift" color="text-muted-foreground" border="border-border" />
...
function ProgressBar({ label, value, color, amount }: any) {
 ...
 <div className="w-24 shrink-0 text-muted-foreground text-xs">{label}</div>
 <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
 <div className={`h-full${color}`} style={{ width: `${value}%` }}></div>
 </div>
 <div className="w-12 text-right text-xs text-muted-foreground">{value}%</div>
 <div className="w-24 text-right font-medium text-foreground text-xs">{amount}</div>
 { }
 </div>
 );
}
...
function PaymentRow({ icon, label, amount, percent }: any) {
 ...
 <div className="w-8 h-8 rounded-lg bg-subtle flex items-center justify-center text-lg">{icon}</div>
 <div className="text-sm font-medium text-foreground">{label}</div>
 </div>
 <div className="flex items-center gap-6">
 <div className="text-sm font-bold text-foreground">{amount}</div>
 <div className="px-2 py-0.5 rounded text-xs font-medium bg-emerald-50 text-emerald-600 min-w-[40px] text-center">
 {percent}
 </div>
 { }
 </div>
...
function ActionButton({ icon, label, color, border }: any) {
 return (
 <button className={`flex items-center justify-center gap-2 p-3 rounded-xl border bg-card hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors${border}${color}`}>
 {icon}

...
export function ShiftSidebar({
 ...
 return (
 <div className="w-80 lg:w-96 bg-card border-l border-border flex flex-col h-[calc(100vh-64px)] overflow-y-auto shrink-0 shadow-xl lg:shadow-none animate-in slide-in-from-right duration-300 z-40 fixed lg:sticky top-16 right-0">
 
 {/* HEADER */}
 <div className="p-5 border-b border-border flex items-center justify-between sticky top-0 bg-card/90 /90 backdrop-blur-sm z-10">
 <div>
 <h2 className="text-lg font-bold text-foreground">Shift Summary</h2>
 </div>
 ...
 <button onClick={onClose} className="p-1.5 text-muted-foreground hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors lg:hidden">
 <X className="w-5 h-5" />
 </button>
 </div>
 </div>
 ...
 <div className="p-3 rounded-xl border border-border bg-subtle /50 flex flex-col justify-center items-center text-center text-foreground">
 ...
 <button className="flex-1 py-2 rounded-lg border border-border hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-semibold flex items-center justify-center gap-2 transition-colors text-foreground">
 ...
 <div className="bg-subtle /50 px-4 py-3 border-b border-border flex items-center gap-2">
 <Wallet className="w-4 h-4 text-muted-foreground" />
 <h3 className="text-xs font-bold uppercase tracking-widest text-foreground">Financial Overview</h3>
 </div>
 ...
 <span className="text-sm font-bold text-orange-600">Net Profit / (Loss)</span>
 <span className="text-sm font-black text-orange-600">{formatCurrency(netProfit)}</span>
 </div>
 </div>
 </div>
...
function InfoRow({ label, value, valueColor ="text-foreground font-medium" }: { label: string, value: string, valueColor?: string }) {
 return (
 <div className="flex justify-between items-center text-xs">
 <span className="text-muted-foreground flex items-center gap-2">
 <div className="w-3 h-3 border border-border rounded-sm flex items-center justify-center">
 <div className="w-1.5 h-1.5 bg-slate-200 rounded-sm"></div>
 </div>
 {label}
 </span>
...
function DrillDownItem({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick?: () => void }) {
 return (
 <button onClick={onClick} className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-foreground group border border-transparent hover:border-border dark:hover:border-slate-700">
 <div className="flex items-center gap-3">
 <div className="text-muted-foreground group-hover:text-slate-600 dark:group-hover:text-white transition-colors">{icon}</div>
 <span className="text-sm font-medium">{label}</span>
 </div>
 <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-slate-600 dark:group-hover:text-white transition-colors" />
 </button>
 );
}
...
function TransactionModal({ title, onClose, items, columns }: any) {
 ...
 <div className="w-full max-w-2xl bg-card rounded-2xl shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden border border-border">
 ...
 {items.length === 0 ? (
 <tr>
 <td colSpan={columns.length} className="px-5 py-8 text-center text-muted-foreground">No records found.</td>
 </tr>
 ) : (
 ...
 <td key={colIdx} className="px-5 py-3 text-sm text-foreground">
 {col.render(item)}
 </td>
 ))}
 </tr>
...
function TimelineModal({ shift, events, onClose }: any) {
 return (
 <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-card backdrop-blur-sm animate-in fade-in duration-200">
 <div className="w-full max-w-3xl bg-card rounded-2xl shadow-2xl flex flex-col h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden border border-border">
 <div className="flex items-center justify-between p-5 border-b border-border bg-background">
 <div>
 ...
 <p className="text-sm text-muted-foreground mt-0.5">Chronological playback for Shift #{shift.id}</p>
 </div>
 <button onClick={onClose} className="p-2 text-muted-foreground hover:text-slate-600 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors">
 <X className="w-5 h-5" />
 </button>
 </div>
 <div className="overflow-y-auto flex-1 p-6 relative bg-background">
 ...
 <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center border-4 border-border${color}`}>
 ...
 <div className="text-xs font-mono text-muted-foreground">{evt.timestamp.split('T')[1]?.slice(0,5) || '12:00'}</div>
 </div>
 <div className="text-sm text-muted-foreground mb-3">{evt.description}</div>
 {evt.amount > 0 && (
 <div className="inline-block px-2 py-1 rounded bg-muted text-xs font-bold text-foreground">

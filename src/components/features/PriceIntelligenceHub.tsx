...
export default function PriceIntelligenceHub({ settings }: PriceIntelligenceHubProps) {
 ...
 <div>
 <h2 className="text-xl font-bold text-foreground font-sans tracking-tight flex items-center gap-2">
 ...
 <div className="premium-card px-5 py-3 border border-border flex items-center ga w-full sm:w-auto">
 <div>
 <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">{t("Compliance Score","کمپلائنس اسکور")}</p>
 ...
 <thead>
 <tr>
 <th className="text-muted-foreground">{t("Shift & Date","شفٹ اور تاریخ")}</th>
 <th className="text-muted-foreground">{t("Product","پراڈکٹ")}</th>
 <th className="text-muted-foreground">{t("Rate Change","نئی قیمت")}</th>
 <th className="text-muted-foreground">{t("Effective","لاگو وقت")}</th>
 <th className="text-muted-foreground">{t("Captured","درج وقت")}</th>
 <th className="text-muted-foreground">{t("Delay","تاخیر")}</th>
 <th className="text-muted-foreground">{t("Status","سٹیٹس")}</th>
 </tr>
 </thead>
 ...
 <br/>
 <span className="text-xs text-muted-foreground">{new Date(ev.shiftDate).toLocaleDateString()}</span>
 </td>

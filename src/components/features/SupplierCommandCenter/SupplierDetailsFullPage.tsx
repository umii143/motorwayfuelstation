...
export default function SupplierDetailsFullPage({ supplier, settings, shifts, banks, batches, onBack }: SupplierDetailsFullPageProps) {
 ...
 return (
 <div className="min-h-screen bg-background pb-20 -mx-4 lg:-mx-8 -mt-4 lg:-mt-8">
 {/* Top Header */}
 <div className="bg-card border-b border-border sticky top-0 z-30">
 <div className="px-6 py-4">
 <button onClick={onBack} className="flex items-center gap-2 text-muted-foreground hover:text-white mb-4 transition-colors cursor-pointer w-max">
 <ArrowLeft className="w-4 h-4" /> Back to Suppliers
 </button>
 
 <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
 <div className="flex items-center gap-4">
 <div className="w-16 h-16 rounded-full bg-card flex items-center justify-center border border-border overflow-hidden shadow-inner p-2">
 {logoUrl ? (
 <img src={logoUrl} alt={supplier.name} className="w-full h-full object-contain" />
 ) : (
 <span className="text-xl font-bold text-muted-foreground">{supplier.name.substring(0, 2).toUpperCase()}</span>
 )}
 </div>
 <div>
 ...
 <div className="bg-card rounded-lg p-3 border border-border min-w-[140px]">
 <p className="text-xs text-muted-foreground mb-1">Supplier Health</p>
 ...
 <div>
 <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">{t.date}</p>
 <p className="text-xs text-white">{t.type === 'Purchase' ? 'Purchase Created' : 'Payment Sent'}</p>
 </div>
 </div>
 {i < Math.min(transactions.length - 1, 2) && <div className="w-8 h-px bg-muted"></div>}
 </React.Fragment>
 ))}
 {transactions.length === 0 && (
 <p className="text-xs text-muted-foreground italic">No recent timeline activity.</p>
 )}
 </div>
 ...
 {/* Enterprise Tabs */}
 <div className="px-6 flex gap-8 border-t border-border overflow-x-auto no-scrollbar">
 ...
 <td className="py-3 px-4 text-muted-foreground w-1/3">Company Name</td>
 <td className="py-3 px-4 text-white font-medium">{supplier.name}</td>
 </tr>
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">Supplier Type</td>
 <td className="py-3 px-4 text-white">{supplier.supplierType || 'Fuel Supplier'}</td>
 </tr>
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">NTN</td>
 <td className="py-3 px-4 text-white">{supplier.ntn || '1234567-8'}</td>
 </tr>
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">STRN</td>
 <td className="py-3 px-4 text-white">3277876123456</td>
 </tr>
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">Contact Person</td>
 ...
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">Address</td>
 <td className="py-3 px-4 text-white">{supplier.address || 'Head Office, Pakistan'}</td>
 </tr>
 <tr className="hover:bg-slate-800/30 transition-colors">
 <td className="py-3 px-4 text-muted-foreground">Payment Terms</td>
 ...
 <h3 className="text-white font-semibold mb-4 pb-3 border-b border-border">Purchase Summary (This Year)</h3>
 ...
 className="bg-card border border-border text-foreground text-xs rounded-lg px-3 py-2 focus:outline-hidden" 
 />
 </div>
 <select className="bg-card border border-border text-foreground text-xs rounded-lg px-3 py-2 outline-hidden">
 ...
 <button className="flex items-center gap-2 bg-card hover:bg-slate-700 text-foreground px-3 py-2 rounded-lg text-xs font-semibold transition-colors">
 ...
 <td className="px-4 py-3 text-muted-foreground">{txn.ref}</td>
 ...
 <tr>
 <td colSpan={10} className="px-4 py-8 text-center text-muted-foreground italic">No transactions found.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 <div className="p-4 flex justify-center border-t border-border shrink-0">
 <button 
 onClick={() => setVisibleLimit(prev => prev + 100)}
 className="px-6 py-2 bg-card text-muted-foreground hover:bg-slate-700 hover:text-white rounded-full font-bold text-sm transition-colors border border-border"
 >
 ...
 <div className="p-3 border-t border-border flex justify-between items-center text-xs text-muted-foreground bg-card">
 ...
 <button className="px-2 py-1 rounded bg-card hover:bg-slate-700 disabled:opacity-50" disabled>Prev</button>
 <button className="px-2 py-1 rounded bg-orange-600 text-white">1</button>
 <button className="px-2 py-1 rounded bg-card hover:bg-slate-700 disabled:opacity-50" disabled>Next</button>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500"></div><span className="text-muted-foreground">Paid Amount</span></div>
 <span className="text-white font-medium">
 {pieData.reduce((s, d) => s + d.value, 0) > 0 ? ((totalPaymentsYear / pieData.reduce((s, d) => s + d.value, 0)) * 100).toFixed(1) : 0}%
 </span>
 </div>
 <div className="flex justify-between items-center text-xs">
 <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-rose-500"></div><span className="text-muted-foreground">Outstanding</span></div>
 <span className="text-white font-medium">
 {pieData.reduce((s, d) => s + d.value, 0) > 0 ? ((supplier.balance / pieData.reduce((s, d) => s + d.value, 0)) * 100).toFixed(1) : 0}%
 </span>
 </div>
 <div className="flex justify-between items-center text-xs">
 <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500"></div><span className="text-muted-foreground">Available</span></div>
 <span className="text-white font-medium">
 ...
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded bg-card flex items-center justify-center text-rose-400"><FileText className="w-4 h-4"/></div>
 <div>
 <p className="text-xs text-white font-medium">NTN Certificate</p>
 <p className="text-[10px] text-muted-foreground">Uploaded May 12, 2024</p>
 </div>
 </div>
 <button className="text-muted-foreground hover:text-white"><Download className="w-4 h-4"/></button>
 </div>
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded bg-card flex items-center justify-center text-rose-400"><FileText className="w-4 h-4"/></div>
 ...
 <button className="w-full flex items-center justify-center gap-2 bg-card hover:bg-slate-700 text-foreground py-2 rounded-lg text-xs font-semibold transition-colors border border-border">
 Supplier Statement
 </button>
 </div>
 </div>
 ...
 <p className="text-muted-foreground text-sm mt-1">This module is part of the Enterprise architecture.</p>
 </div>
 )}
 </div>
 </div>

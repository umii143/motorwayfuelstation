...
function RecoveryModal({
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1">Recovery Amount (Rs.)</label>
 <input
 type="number"
 value={amount}
 ...
 className="w-full border border-theme-main rounded-xl px-3 py-2 text-foreground text-sm font-mono focus:outline-none focus:border-emerald-500"
 autoFocus
 />
 {parseFloat(amount) > 0 && (
 ...
 <p className="text-xs text-muted-foreground mt-1 font-mono">{pct}% recovered</p>
 </div>
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1">Resolution Notes</label>
 <input
 type="text"
 value={notes}
 onChange={e => setNotes(e.target.value)}
 placeholder="e.g. Credit note received from PSO..."
 className="w-full border border-theme-main rounded-xl px-3 py-2 text-foreground text-sm focus:outline-none focus:border-emerald-500"
 />
 </div>
 <div className="flex gap-2 pt-1">
 <button onClick={onClose} className="flex-1 py-2 rounded-xl border border-theme-main text-muted-foreground text-sm font-semibold hover:bg-slate-50">Cancel</button>
 <button
 onClick={() => {
...
export default function SupplierClaimsPanel({ batches, suppliers, language }: SupplierClaimsPanelProps) {
 ...
 const inputCls ="w-full rounded-xl border border-theme-main bg-theme-card px-3 py-2 text-foreground focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none text-sm";
 const selectCls ="w-full rounded-xl border border-theme-main bg-theme-bg px-3 py-2 text-foreground focus:border-orange-500 focus:bg-card focus:outline-none text-sm";
 ...
 {[
 { label: t('Total Claims', 'کل کلیمز'), value: intelligence.total, icon: AlertTriangle, bg: 'bg-theme-bg border-theme-main', color: 'text-slate-700', valueColor: 'text-foreground' },
 ...
 <div className="size-10 rounded-xl bg-card/60 flex items-center justify-center shrink-0">
 ...
 <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t('Claim Recovery Pipeline', 'کلیم وصولی')}</span>
 ...
 activeView === v ? 'bg-theme-card text-foreground shadow-sm' : 'text-slate-500 hover:text-slate-700'
 }`}
 >
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Linked Batch *', 'منسلک بیچ *')}</label>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Supplier *', 'سپلائر *')}</label>
 ...
 <div>
 { }
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Claim Type *', 'کلیم قسم *')}</label>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Qty Short (L)', 'کم وصول لیٹر')}</label>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Claim Amount (Rs.) *', 'کلیم رقم *')}</label>
 ...
 <div>
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Date Raised', 'تاریخ')}</label>
 ...
 <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wide mb-1.5">{t('Description', 'تفصیل')}</label>
 ...
 <button onClick={() => setShowAddForm(false)} className="px-4 py-2.5 rounded-xl border border-theme-main text-muted-foreground text-sm font-semibold hover:bg-slate-50">Cancel</button>
 <button
 ...
 <p className="text-muted-foreground text-sm py-8 text-center">{t('No supplier claims data.', 'کوئی سپلائر کلیمز نہیں۔')}</p>
 ) : (
 ...
 <span className="font-bold text-sm text-foreground">{s.supplier.name}</span>
 {s.pending > 0 && (
 ...
 <p className="font-black text-foreground">{s.count}</p>
 </div>
 <div>
 <p className="text-muted-foreground">Total</p>
 <p className="font-black text-foreground">Rs.{(s.totalClaim / 1000).toFixed(0)}K</p>
 </div>
 <div>
 ...
 <div className="premium-card border overflow-hidden">
 {/* Filters */}
 <div className="px-4 py-3 border-b border-border flex flex-row gap-2 items-center">
 <div className="flex flex-wrap gap-1.5">
 {(['all', 'pending', 'submitted', 'approved', 'rejected', 'recovered', 'partial'] as const).map(s => (
 <button key={s} onClick={() => setFilterStatus(s)}
 className={`px-2.5 py-1 rounded-full text-xs font-bold border transition-colors${filterStatus === s ? 'bg-red-600 text-white border-red-600' : 'bg-theme-card text-slate-600 border-theme-main hover:border-border'}`}>
 ...
 className="text-xs border border-theme-main rounded-lg px-2 py-1.5 bg-theme-card text-foreground focus:outline-none focus:border-orange-500 ml-auto"
 >
 ...
 return (
 <div key={claim.id} className={`transition-colors${isOverdue ? 'bg-red-50/30' : 'hover:bg-slate-50 dark:bg-card/5/50'}`}>
 <div
 ...
 <span className="font-mono text-sm font-bold text-foreground">{claim.claimNumber}</span>
 ...
 <p className="font-black text-foreground text-sm">Rs.{claim.outstandingClaim.toLocaleString('en-PK', { maximumFractionDigits: 0 })}</p>
 </div>
 ...
 {[
 { label: 'Claim Amount', value: `Rs.${claim.claimAmount.toLocaleString('en-PK', { maximumFractionDigits: 0 })}`, color: 'text-foreground', bg: 'bg-theme-bg border-slate-100 dark:border-white/5' },
 ...
 {claim.description && <p className="text-xs text-muted-foreground italic">{claim.description}</p>}

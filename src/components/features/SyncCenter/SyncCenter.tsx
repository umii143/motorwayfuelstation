    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="premium-card p-5 border">
          ...
          <p className="text-3xl font-bold text-slate-900 dark:text-white">{status.queue.length}</p>
        </div>
        <div className="premium-card p-5 border">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-3xl font-bold text-slate-900 dark:text-white">{status.pending}</p>
        </div>
        <div className="premium-card p-5 border">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-3xl font-bold text-slate-900 dark:text-white">{status.failed}</p>
        </div>
        <div className="premium-card p-5 border">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-3xl font-bold text-slate-900 dark:text-white">{driftLogs.length}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="premium-card border overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 flex items-center justify-between">
            <h3 className="font-semibold text-slate-800 dark:text-slate-200">Current Mutation Queue</h3>
          </div>
          <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
            {status.queue.length === 0 ? (
              ...
              status.queue.map(item => (
                <div key={item.id} className="p-4 hover:bg-slate-50 dark:bg-white/5 transition-colors flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-medium text-slate-900 dark:text-white capitalize">{item.entityType}</span>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-white/10 text-slate-600 capitalize">{item.operation}</span>
        ...
        <div className="premium-card border overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 flex items-center justify-between">
            <h3 className="font-semibold text-slate-800 dark:text-slate-200">Integrity Drift Logs</h3>
            {driftLogs.length > 0 && (
          ...
          <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
            {driftLogs.length === 0 ? (
              ...
              driftLogs.map(log => (
                <div key={log.id} className="p-4 hover:bg-slate-50 dark:bg-white/5 transition-colors">
                  <div className="flex items-center gap-2 mb-1">
                    <AlertTriangle className="h-4 w-4 text-red-500" />
                    <span className="text-sm font-medium text-slate-900 dark:text-white">Drift on {log.entityType}</span>
                  </div>

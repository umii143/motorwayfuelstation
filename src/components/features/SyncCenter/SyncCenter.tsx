...
export default function SyncCenter({ settings }: { settings: GlobalSettings }) {
 ...
 <p className="text-sm text-muted-foreground">Monitor offline transaction queue and synchronization health.</p>
 </div>
 ...
 className="flex items-center gap-2 px-4 py-2 bg-card text-foreground rounded-lg hover:bg-slate-800 disabled:opacity-50 transition-colors"
 >
 ...
 <h3 className="font-medium text-foreground">Total Queue</h3>
 </div>
 <p className="text-3xl font-bold text-foreground">{status.queue.length}</p>
 </div>
 <div className="premium-card p-5 border">
 <div className="flex items-center gap-3 mb-2">
 ...
 <h3 className="font-medium text-foreground">Pending Sync</h3>
 </div>
 <p className="text-3xl font-bold text-foreground">{status.pending}</p>
 </div>
 <div className="premium-card p-5 border">
 <div className="flex items-center gap-3 mb-2">
 ...
 <h3 className="font-medium text-foreground">Failed Retries</h3>
 </div>
 <p className="text-3xl font-bold text-foreground">{status.failed}</p>
 </div>
 <div className="premium-card p-5 border">
 <div className="flex items-center gap-3 mb-2">
 ...
 <h3 className="font-medium text-foreground">Integrity Drifts</h3>
 </div>
 <p className="text-3xl font-bold text-foreground">{driftLogs.length}</p>
 </div>
 </div>

 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div className="premium-card border overflow-hidden">
 <div className="px-5 py-4 border-b border-border bg-subtle flex items-center justify-between">
 <h3 className="font-semibold text-foreground">Current Mutation Queue</h3>
 </div>
 ...
 <span className="text-sm font-medium text-foreground capitalize">{item.entityType}</span>
 <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground capitalize">{item.operation}</span>
 {item.priority === 'critical' && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600">Critical</span>}
 </div>
 <p className="text-xs text-muted-foreground">ID: {item.referenceId || item.docId}</p>
 ...
 <p className="text-[10px] text-muted-foreground mt-1">Retries: {item.retryCount}/60</p>
 </div>
 </div>
 ))
 )}
 ...
 <div className="premium-card border overflow-hidden">
 <div className="px-5 py-4 border-b border-border bg-subtle flex items-center justify-between">
 <h3 className="font-semibold text-foreground">Integrity Drift Logs</h3>
 {driftLogs.length > 0 && (
 ...
 <span className="text-sm font-medium text-foreground">Drift on {log.entityType}</span>
 </div>
 <p className="text-xs text-muted-foreground mb-2">Ref: {log.referenceId}</p>
 <div className="p-2 bg-red-50 rounded text-xs text-red-700 border border-red-100">
 {log.error}
 </div>
 <p className="text-[10px] text-muted-foreground mt-2">Logged: {new Date(log.timestamp).toLocaleString()}</p>
 </div>
 ))
 )}
 </div>

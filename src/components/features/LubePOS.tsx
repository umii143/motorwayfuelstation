...
const cartItems = useMemo(() => {
 ...
 return (
 <div className="flex flex-col h-[calc(100vh-5rem)] lg:h-[calc(100vh-6rem)] w-full max-w-[1600px] mx-auto bg-card rounded-none sm:rounded-[2rem] overflow-hidden shadow-2xl border border-border font-sans text-muted-foreground">
 
 {/* HEADER SECTION */}
 <div className="flex-none px-4 sm:px-6 py-3 sm:py-4 bg-card backdrop-blur-md border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3 z-10 relative">
 ...
 className="w-full pl-10 pr-12 py-2 sm:py-2.5 bg-card border border-border rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all"
 />
 <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-muted-foreground border border-border rounded bg-card px-1.5 py-0.5">
 Ctrl / K
 </div>
 </div>
 ...
 className="rounded-xl border border-border bg-card px-4 py-2.5 text-xs font-bold text-muted-foreground transition-all hover:bg-slate-800 hover:text-white flex items-center justify-center gap-2"
 >
 ...
 {showHistory ? (
 <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-card custom-scrollbar">
 <div className="mb-6 flex items-center justify-between border-b border-border pb-4">
 <div className="flex items-center gap-3">
 <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-card text-orange-500 border border-border">
 <History className="h-5 w-5" />
 </div>
 <h3 className="text-lg font-black text-white">{t('Invoice History', 'بل ہسٹری')}</h3>
 </div>
 {/* Mobile back button */}
 <button onClick={() => setShowHistory(false)} className="lg:hidden px-4 py-2 bg-card text-foreground rounded-lg text-xs font-bold">Back to POS</button>
 </div>
 <div className="space-y-3 max-w-4xl mx-auto">
 {lubePosSales.slice().reverse().map((sale) => (
 <div key={sale.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border border-border bg-card p-4 shadow-sm hover:border-slate-600 transition-colors">
 <div>
 ...
 <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto border-t sm:border-0 border-border pt-3 sm:pt-0">
 <div className="text-right mr-4">
 <span className="block text-[10px] font-bold uppercase text-muted-foreground">{t('Total', 'کل')}</span>
 <strong className={`text-base font-black${sale.isReturn ? 'text-rose-400' : 'text-emerald-400'}`}>
 {formatCurrency(sale.total, settings)}
 </strong>
 </div>
 <div className="flex gap-2">
 <button onClick={() => setLastSale(sale)} className="rounded-lg bg-card p-2 text-blue-400 hover:bg-slate-700 border border-border" title="Print Invoice">
 <Printer className="h-4 w-4" />
 </button>
 {!sale.isReturn && !lubePosSales.some(s => s.returnedSaleId === sale.id) && (
 <button onClick={() => handleReturn(sale)} className="rounded-lg bg-card p-2 text-rose-400 hover:bg-slate-700 border border-border" title="Return / Refund">
 ...
 className="snap-start flex-shrink-0 flex items-center gap-3 w-[260px] sm:w-[280px] bg-card border border-border rounded-2xl p-3 hover:border-slate-500 hover:bg-slate-800 transition-all cursor-pointer group shadow-lg"
 onClick={() => { if (product.currentStock > 0) addToCart(product.id) }}
 >
 <div className="h-12 w-10 shrink-0 bg-card rounded-lg flex items-center justify-center border border-border">
 <Package className="h-6 w-6 text-muted-foreground group-hover:text-orange-400 transition-colors" />
 </div>
 ...
 <button className="h-8 w-8 shrink-0 rounded-lg bg-card border border-border flex items-center justify-center text-muted-foreground group-hover:bg-orange-500 group-hover:text-white group-hover:border-orange-500 transition-colors">
 ...
 {/* Category Tabs */}
 <div className="flex-none flex gap-2 overflow-x-auto pb-4 scrollbar-hide mb-2 border-b border-border">
 {categories.map((cat) => (
 <button
 key={cat}
 ...
 : 'bg-[#1e293b] border border-slate-700 text-slate-300 hover:bg-card hover:text-white'
 }`}
 >
 {cat === 'All' ? t('All Products', 'تمام پروڈکٹس') : cat}
 </button>
 ))}
 <button className="flex-shrink-0 px-4 py-2.5 rounded-full text-[11px] font-bold bg-card border border-border text-muted-foreground hover:bg-slate-800 flex items-center gap-1">
 More <ArrowRight className="h-3 w-3" />
 </button>
 <div className="flex-1"></div>
 <button className="hidden sm:flex flex-shrink-0 h-10 w-10 rounded-full bg-card border border-border items-center justify-center text-muted-foreground hover:bg-slate-800">
 ...
 key={product.id}
 className={`relative flex flex-col bg-card border rounded-2xl p-3 sm:p-4 shadow-lg transition-all duration-300 group${
 product.currentStock <= 0
 ? 'border-slate-800 bg-card/50 opacity-50 grayscale cursor-not-allowed'
 : 'border-slate-700/60 hover:border-orange-500/50 hover:bg-card cursor-pointer hover:shadow-orange-500/10'
 }`}
 onClick={() => { if (product.currentStock > 0) addToCart(product.id) }}
 >
 {/* Icon & Details */}
 ...
 <div className="h-12 w-10 sm:h-16 sm:w-12 bg-card rounded-lg flex items-center justify-center border border-border">
 <Package className="h-6 w-6 sm:h-8 sm:w-8 text-muted-foreground group-hover:text-orange-400 transition-colors" />
 </div>
 <button 
 ...
 <span className="text-[10px] sm:text-xs text-muted-foreground mt-1 uppercase tracking-widest">{product.category || 'N/A'} • {product.unit}</span>
 </div>
 ...
 <span className="text-[9px] sm:text-[10px] font-bold text-muted-foreground tracking-wider">
 ...
 className="h-8 w-8 sm:h-10 sm:w-10 shrink-0 rounded-xl bg-card border border-border flex items-center justify-center text-muted-foreground group-hover:bg-orange-600 group-hover:text-white group-hover:border-orange-500 transition-colors disabled:opacity-50"
 >
 ...
 <div className="hidden lg:flex absolute bottom-6 left-6 right-[460px] h-16 bg-card backdrop-blur-xl border border-border rounded-2xl items-center px-2 shadow-[0_20px_40px_rgb(0,0,0,0.3)] z-20">
 <div className="flex gap-2 w-full">
 <button className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl hover:bg-slate-800 text-muted-foreground transition-colors">
 <span className="h-6 w-6 rounded bg-card flex items-center justify-center text-[10px] border border-border">F6</span>
 <span className="text-xs font-bold">Hold Bill</span>
 </button>
 <button className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl hover:bg-slate-800 text-muted-foreground transition-colors">
 <span className="h-6 w-6 rounded bg-card flex items-center justify-center text-[10px] border border-border">F7</span>
 <span className="text-xs font-bold">Park Order</span>
 </button>
 <button onClick={() => setShowHistory(true)} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl hover:bg-slate-800 text-muted-foreground transition-colors">
 <span className="h-6 w-6 rounded bg-card flex items-center justify-center text-[10px] border border-border">F8</span>
 <span className="text-xs font-bold">Recent Bills</span>
 </button>
 <div className="w-px bg-muted my-2 mx-1"></div>
 ...
 <span className="h-6 w-6 rounded bg-card flex items-center justify-center text-[10px] border border-rose-900/50">F9</span>
 ...
 <div className="w-full lg:w-[400px] xl:w-[440px] flex flex-col bg-card border-l border-border shadow-[-10px_0_40px_rgb(0,0,0,0.2)] z-30 flex-none shrink-0 border-t lg:border-t-0 rounded-t-[2rem] lg:rounded-none min-h-[650px] lg:min-h-0 h-auto mt-4 lg:mt-0">
 ...
 className="w-full appearance-none rounded-xl border border-border bg-card px-4 py-3 sm:py-3.5 pl-10 text-[12px] sm:text-[13px] font-bold text-foreground shadow-inner focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all cursor-pointer"
 >
 ...
 <div className="h-6 w-6 bg-card rounded flex items-center justify-center border border-border">
 ...
 exit={{ opacity: 0, x: -20 }}
 key={item.productId}
 className="flex items-center gap-3 pb-3 sm:pb-4 border-b border-border last:border-0"
 >
 {/* Thumbnail */}
 <div className="h-10 w-8 sm:h-12 sm:w-10 bg-card rounded-md flex items-center justify-center border border-border shrink-0">
 ...
 {/* Qty Controls */}
 <div className="flex items-center gap-1 bg-card rounded-lg border border-border p-0.5 shrink-0">
 <button onClick={() => updateCartQty(item.productId, item.quantity - 1)} className="w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center text-muted-foreground hover:text-white hover:bg-slate-800 rounded-md transition-all">
 ...
 <button onClick={() => updateCartQty(item.productId, 0)} className="text-muted-foreground hover:text-rose-500 transition-colors">
 ...
 <div className="px-4 sm:px-5 py-2 sm:py-3 bg-card/60 border-y border-border flex items-center justify-between cursor-pointer hover:bg-slate-800 transition-colors">
 ...
 {/* Totals Summary */}
 <div className="px-4 sm:px-5 py-3 sm:py-4 bg-card space-y-2 sm:space-y-3 text-[11px] sm:text-xs">
 ...
 className="w-16 bg-card border border-border rounded px-2 py-1 text-right text-emerald-400 font-bold focus:outline-none focus:border-emerald-500"
 placeholder="0"
 />
 ...
 <div className="flex justify-between items-end pt-2 sm:pt-3 border-t border-border mt-2 sm:mt-3">
 ...
 {/* Payment Options */}
 <div className="p-4 sm:p-5 bg-card border-t border-border pb-20 lg:pb-5">
 <div className="grid grid-cols-4 gap-2 mb-4">
 <button onClick={() => setPaymentMode('cash')} className={`py-2 rounded-xl text-[10px] sm:text-[11px] font-bold border transition-all flex flex-col items-center gap-1${paymentMode === 'cash' ? 'bg-card border-slate-600 text-white' : 'bg-transparent border-slate-800 text-slate-500 hover:border-slate-600'}`}>
 <Landmark className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Cash
 </button>
 <button onClick={() => setPaymentMode('credit')} className={`py-2 rounded-xl text-[10px] sm:text-[11px] font-bold border transition-all flex flex-col items-center gap-1${paymentMode === 'credit' ? 'bg-card border-slate-600 text-white' : 'bg-transparent border-slate-800 text-slate-500 hover:border-slate-600'}`}>
 <User className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Credit
 </button>
 <button onClick={() => setPaymentMode('bank')} className={`py-2 rounded-xl text-[10px] sm:text-[11px] font-bold border transition-all flex flex-col items-center gap-1${paymentMode === 'bank' ? 'bg-card border-slate-600 text-white' : 'bg-transparent border-slate-800 text-slate-500 hover:border-slate-600'}`}>
 <Landmark className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Bank
 </button>
 <button onClick={() => setPaymentMode('digital')} className={`py-2 rounded-xl text-[10px] sm:text-[11px] font-bold border transition-all flex flex-col items-center gap-1${paymentMode === 'digital' ? 'bg-card border-slate-600 text-white' : 'bg-transparent border-slate-800 text-slate-500 hover:border-slate-600'}`}>
 ...
 {paymentMode !== 'credit' && (
 <div className="flex items-center justify-between bg-card rounded-xl border border-border px-4 py-2 sm:py-3 mb-3 sm:mb-4">
 <span className="text-[11px] sm:text-xs font-bold text-muted-foreground">Amount Received</span>
 <input 
 value={amountReceived} 
 onChange={(e) => setAmountReceived(e.target.value)} 
 placeholder={totals.total.toString()} 
 className="w-24 sm:w-32 text-right font-black text-sm sm:text-base text-white bg-transparent border-b border-border focus:border-orange-500 focus:outline-none pb-1" 
 />
 </div>
 )}
 ...
 {paymentMode === 'bank' && (
 <select value={bankAccountId} onChange={(e) => setBankAccountId(e.target.value)} className="w-full mb-3 sm:mb-4 rounded-xl border border-border bg-card px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-bold text-foreground focus:outline-none focus:border-blue-400">
 <option value="">{t('Select receiving bank', 'بینک منتخب کریں')}</option>
 {banks.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
 </select>
 )}
 {paymentMode === 'digital' && (
 <select value={digitalAccountId} onChange={(e) => setDigitalAccountId(e.target.value)} className="w-full mb-3 sm:mb-4 rounded-xl border border-border bg-card px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-bold text-foreground focus:outline-none focus:border-blue-400">
 ...
 <span className="text-[11px] sm:text-xs font-bold text-muted-foreground">Change</span>

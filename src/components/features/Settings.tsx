...
export default function SettingsPanel({
 ...
 <div className="bg-card border border-border p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
 <div>
 <h3 className="text-xl font-bold text-foreground mb-1">Guided Hardware Setup</h3>
 <p className="text-sm text-muted-foreground">Configure your Tanks, Pumps, and Nozzles in one simple flow.</p>
 </div>
 ...
 <Settings className="h-16 w-16 mb-4 text-foreground" />
 ...
 return (
 <div className="flex flex-col h-full min-h-[calc(100dvh-120px)] overflow-hidden rounded-2xl border border-border shadow-sm bg-card max-w-full">

 {/* ── HEADER ────────────────────────────────────────────────── */}
 <div className="flex items-center justify-between border-b border-border bg-card px-4 py-3.5 shrink-0 gap-3">
 <div className="flex items-center gap-2.5 min-w-0">
 <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-orange-50 shrink-0">
 <Settings className="h-5 w-5 text-orange-600" />
 </div>
 <div className="min-w-0">
 <h2 className="font-sans text-base sm:text-lg font-bold tracking-tight text-foreground truncate">
 {isLube ? t('Lube Configuration Center', 'لیوب ترتیبات مرکز') : t('Configuration Center', 'ترتیبات مرکز')}
 </h2>
 <p className="font-sans text-[11px] text-muted-foreground hidden sm:block truncate">
 ...
 <div className="flex flex-1 overflow-hidden bg-subtle relative flex-col lg:flex-row">

 {/* ── MOBILE: GROUPED ACCORDION NAV ─────────────────────────────── */}
 <div className="lg:hidden w-full bg-card border-b border-border shrink-0">
 ...
 hasActive ? 'bg-orange-50' : 'bg-card hover:bg-slate-50 dark:bg-card/5'
 }`}
 >
 ...
 : ((item as any).isDanger
 ? 'bg-card text-red-600 border-red-200 dark:border-red-900/20 hover:bg-red-50'
 : 'bg-card text-slate-700 border-border/60 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-card/5')
 }`}
 >
 ...
 : ((item as any).isDanger ? 'bg-red-50 dark:bg-red-950/30 text-red-500' : 'bg-muted text-slate-400')
 }`}>
 ...
 <Fingerprint className="h-4 w-4 text-slate-350" />
 )}
 <ChevronRight className={`h-4 w-4 transition-transform${isActive ? 'text-white' : 'text-slate-350 '}`} />
 </div>
 </button>
 );
 })}
 ...
 {/* ── DESKTOP SIDEBAR ────────────────────────────────────────────── */}
 <div className="hidden lg:flex flex-col w-64 xl:w-72 bg-card border-r border-border shrink-0">
 ...
 className="w-full pl-8 pr-8 py-2 text-xs bg-subtle border border-border rounded-lg text-foreground placeholder:text-slate-400 focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400 transition-all"
 />
 {sidebarSearch && (
 <button
 onClick={() => setSidebarSearch('')}
 className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-slate-600 cursor-pointer"
 >
 ...
 <div key={section.id}>
 <h3 className="px-2 mb-1 text-[9px] font-extrabold text-muted-foreground uppercase tracking-widest">
 {section.title}
 </h3>
 ...
 : 'text-slate-600 hover:bg-muted hover:text-foreground border border-transparent')
 }`}
 >
 ...
 <div className="flex items-center gap-2 px-2 py-2 rounded-lg bg-subtle">
 <div className="w-6 h-6 rounded-md bg-orange-600 flex items-center justify-center shrink-0">
 <Settings className="h-3.5 w-3.5 text-white" />
 </div>
 <div className="min-w-0">
 <p className="text-[10px] font-bold text-foreground leading-none">FuelPro Enterprise</p>
 <p className="text-[10px] text-muted-foreground leading-none mt-0.5">v3.0 · {isLubeBusinessStation(activeStationId) ? 'LubeManager' : 'Fuel Station'}</p>
 </div>
 </div>
 </div>
 </div>

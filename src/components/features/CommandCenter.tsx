...
export default function CommandCenter({ 
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Search Header */}
      <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-xs relative">
        <h2 className="font-sans text-lg font-black text-slate-800 dark:text-white tracking-tight mb-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-hidden focus:border-orange-500 transition-colors"
          />
        </div>

        {/* Global Search Results Overlay */}
        {searchResults && (
          <div className="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-[#191928] border border-slate-200 dark:border-white/10 rounded-xl shadow-xl z-50 p-4 max-h-[400px] overflow-y-auto space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-white/5 pb-2">
              <span className="text-[10px] uppercase font-bold text-slate-400">Search Results for "{globalSearch}"</span>
              ...
              <div className="space-y-1 text-xs">
                ...
                {searchResults.tanks.map(tank => (
                  <div 
                    ...
                    className="p-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-md cursor-pointer flex justify-between"
                  >
                    <span>{tank.name}</span>
                    <span className="font-mono text-slate-400">Capacity: {tank.capacity.toLocaleString()}L</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-1 text-xs">
                ...
                {searchResults.reports.map(rep => (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400 capitalize">{rep.moduleName}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {searchResults.customers.length > 0 && (
              <div className="space-y-1 text-xs">
                ...
                {searchResults.customers.map(c => (
                  ...
                  >
                    <span>{c.name}</span>
                    <span className="font-mono text-slate-400">Balance: {formatCurrency(c.balance, settings)}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {searchResults.suppliers.length > 0 && (
              <div className="space-y-1 text-xs">
                ...
                {searchResults.suppliers.map(s => (
                  ...
                  >
                    <span>{s.name}</span>
                    <span className="font-mono text-slate-400">Balance: {formatCurrency(s.balance, settings)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        <div 
          ...
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Business Health', 'تجارتی صحت')}</span>
          <div className="text-2xl font-black text-emerald-600 mt-1">92%</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Good / Stable</span>
        </div>
        <div 
          onClick={() => onSelectTab('inventory_audit')}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Inventory Risk', 'اسٹاک رسک')}</span>
          <div className="text-2xl font-black text-amber-600 mt-1">12%</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Low threshold</span>
        </div>
        <div 
          onClick={() => onSelectTab('reconciliation')}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Cash Flow', 'کیش فلو')}</span>
          <div className="text-2xl font-black text-emerald-600 mt-1">Healthy</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Inflows Tally</span>
        </div>
        <div 
          onClick={() => onTriggerDrilldown({ title: 'BI Explorer > Tanks Summary', type: 'tanks', level: 1, params: {} })}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Tank Health', 'ٹینکس صورتحال')}</span>
          <div className="text-2xl font-black text-emerald-600 mt-1">Good</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Dips Calibrated</span>
        </div>
        <div 
          onClick={() => onTriggerDrilldown({ title: 'BI Explorer > Suppliers', type: 'suppliers', level: 1, params: {} })}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Supplier Risk', 'سپلائرز خطرہ')}</span>
          <div className="text-2xl font-black text-slate-800 dark:text-white mt-1">Medium</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Pending Payments</span>
        </div>
        <div 
          onClick={() => onTriggerDrilldown({ title: 'BI Explorer > Customers', type: 'customers', level: 1, params: {} })}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Customer Credit', 'گاہک ادھار رسک')}</span>
          <div className="text-2xl font-black text-emerald-600 mt-1">Low</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Under Limit</span>
        </div>
        <div 
          onClick={() => onTriggerDrilldown({ title: 'BI Explorer > Sales', type: 'sales', level: 1, params: {} })}
          className="bg-white dark:bg-[#151521] border border-slate-205 dark:border-white/10 rounded-xl p-4 shadow-xs text-center cursor-pointer hover:border-orange-500 transition-colors"
        >
          <span className="text-[10px] font-bold text-slate-400 uppercase block">{t('Operational Score', 'انتظامی اسکور')}</span>
          <div className="text-2xl font-black text-emerald-600 mt-1">96%</div>
          <span className="text-[9px] text-slate-400 mt-0.5 block">Shifts Tally OK</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-xs">
            <h3 className="font-sans text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5 pb-2 mb-4 flex items-center gap-1.5">
              ...
              {tanks.map((tnk: Tank) => {
                ...
                return (
                  <div 
                    ...
                    className="border border-slate-100 dark:border-white/5 rounded-xl p-3.5 space-y-3 cursor-pointer hover:border-orange-500 transition-colors"
                  >
                    <div className="flex justify-between items-center text-xs font-bold">
                      <span className="text-slate-800 dark:text-slate-200">{tnk.name}</span>
          ...
          <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-semibold">
              <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                <span className="text-slate-550 dark:text-slate-400 block mb-1">{t('Station Cash Safe', 'سیف ڈرائر کیش')}</span>
                <strong className="text-lg font-mono font-black text-slate-800 dark:text-white">
                  {formatCurrency(cashStatus.safeCash, settings)}
                </strong>
              </div>
              <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                <span className="text-slate-550 dark:text-slate-400 block mb-1">{t('Bank Account Balance', 'بینک اکاؤنٹ بیلنس')}</span>
                <strong className="text-lg font-mono font-black text-slate-800 dark:text-white">
                  {formatCurrency(cashStatus.bankBalance, settings)}
                </strong>
              </div>
              <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                <span className="text-slate-550 dark:text-slate-400 block mb-1">{t('Digital Receipts Wallet', 'ڈیجیٹل والٹ رسیدیں')}</span>
                <strong className="text-lg font-mono font-black text-slate-800 dark:text-white">
          ...
          <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-xs space-y-4">
            <h3 className="font-sans text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5 pb-2 flex items-center gap-1.5">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[#151521] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-xs space-y-4">
            <h3 className="font-sans text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5 pb-2 flex items-center justify-between">
            ...
            <div className="space-y-3.5 text-xs">
              {activityLogs.slice(0, 5).map((log: AuditTrailEntry) => (
                <div key={log.id} className="relative pl-4 border-l border-slate-100 dark:border-white/5 pb-1">
                  <div className="absolute -left-1 top-1.5 w-2 h-2 rounded-full bg-orange-500" />
                  <div className="flex justify-between text-[10px] text-slate-400 mb-0.5">
                    <span>{log.timestamp}</span>
                    <span className="font-bold uppercase text-[8px] bg-slate-100 dark:bg-white/10 px-1.5 py-0.2 rounded-full">
                      {log.category}
                    </span>
                  </div>
                  <strong className="text-slate-800 dark:text-slate-200 font-extrabold block">
                    {log.action}
                  </strong>
                  <span className="text-[10.5px] text-slate-500 mt-0.5 block line-clamp-1">
                    {log.details}
                  </span>
                </div>
              ))}

  ...
  return (
    <RoleGuard allowedRoles={['Owner', 'Manager']} fallbackMessage={t('Executive Dashboard is strictly restricted.', 'ایگزیکٹو ڈیش بورڈ تک رسائی ممنوع ہے۔')}>
      <div className="space-y-6 animate-fade-in pb-12">
        ...
        {/* Global Date Filters */}
        <div className="premium-card p-4 border border-slate-200 dark:border-white/10 flex flex-wrap items-center gap-3 mb-6 bg-white dark:bg-[#151521] rounded-2xl">
          <div className="flex items-center gap-2">
            <Calendar className="h-5 w-5 text-indigo-500" />
            <span className="text-sm font-bold text-slate-700 dark:text-slate-350">{t('Analysis Period:', 'تجزیہ کی مدت:')}</span>
          </div>
          <input 
            type="date" 
            value={dateRange.from}
            onChange={e => setDateRange(prev => ({...prev, from: e.target.value}))}
            className="px-3 py-1.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-sm text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <span className="text-slate-400 font-medium">{t('to', 'تا')}</span>
          <input 
            type="date" 
            value={dateRange.to}
          ...
          {(dateRange.from || dateRange.to) && (
            <button 
              ...
              className="ml-auto px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white bg-slate-100 dark:bg-white/10 hover:bg-slate-200 rounded-lg transition-colors"
            >
        ...
        <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="min-w-full max-w-[250px]">
              <h2 className="text-lg font-bold text-slate-300 mb-2">{t('Business Health Score', 'کاروباری صحت کا اسکور')}</h2>
            ...
            <div className="flex-1 w-full bg-white/5 p-6 rounded-2xl backdrop-blur-sm border border-white/10">
              <h3 className="text-sm font-bold text-slate-300 mb-4">{t('Health Factors', 'صحت کے عوامل')}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                {health.factors.map(f => (
                  <div key={f.name} className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-slate-300">{translateFactorName(f.name)}</span>
          ...
          <div className="bg-slate-900 rounded-3xl p-6 shadow-xl border border-slate-800">
            <div className="flex items-center gap-3 mb-6 px-2">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-slate-400">{t('AI-Driven Executive Recommendations', 'مصنوعی ذہانت پر مبنی ایگزیکٹو سفارشات')}</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-8">
          ...
          {/* Revenue */}
          <div 
            className="premium-card p-6 border hover:shadow-md hover:border-blue-300 transition-all cursor-pointer group bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl"
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3">
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('MTD', 'اس مہینے')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.revenue.mtd.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Today', 'آج')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.revenue.today.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Operational Profit */}
          <div 
            className="premium-card p-6 border hover:shadow-md hover:border-emerald-300 transition-all cursor-pointer group bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl"
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3">
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Gross Margin', 'مجموعی مارجن')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.profit.marginPercent.toFixed(1)}%</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Gross (PKR)', 'مجموعی (روپے)')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.profit.gross.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div 
            className="premium-card p-6 border hover:shadow-md hover:border-purple-300 transition-all cursor-pointer group bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl"
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-slate-500 font-semibold mb-1">{t('Net Impact', 'خالص اثر')}</p>
              ...
              <div className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3">
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Profit Source', 'منافع کا ذریعہ')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{t('Stock Holding', 'اسٹاک ہولڈنگ')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Expenses */}
          <div 
            className="premium-card p-6 border hover:shadow-md hover:border-amber-300 transition-all cursor-pointer group bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl"
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-slate-500 font-semibold mb-1">{t('Total Expenses', 'کل اخراجات')} ({dateRange.from || dateRange.to ? t('Selected', 'منتخب') : t('Lifetime', 'لائف ٹائم')})</p>
                <p className="text-2xl font-black text-amber-600">Rs {kpis.expenses.total.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
              </div>
              <div className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3">
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Salary Cost', 'تنخواہ')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.expenses.salary.total.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{t('Other Cost', 'دیگر اخراجات')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{(kpis.expenses.total - kpis.expenses.salary.total).toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Cash Position */}
          <div className="premium-card p-6 border bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{t('Cash Position', 'نقدی کی صورتحال')}</h3>
            <div className="space-y-4">
              <div 
              ...
              >
                <p className="text-xs text-slate-500 font-semibold mb-1 group-hover:text-emerald-600 transition-colors">{t('Net Flow', 'خالص بہاؤ')}</p>
              ...
              <div 
                ...
                className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3 mt-3 cursor-pointer hover:bg-blue-500/10 p-2 -mx-2 rounded-lg transition-colors group"
                title="Open Credit Intelligence Center"
              >
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase group-hover:text-blue-500 transition-colors">{t('Credit Outstanding', 'بقایا ادھار')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.credit.outstanding.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase group-hover:text-blue-500 transition-colors">{t('Collection Eff.', 'وصولی کی کارکردگی')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{kpis.credit.collectionEfficiency.toFixed(1)}%</p>
                </div>
              </div>

              <div 
                onClick={() => setIsSupplierDrillDownOpen(true)}
                className="flex justify-between border-t border-slate-100 dark:border-white/5 pt-3 mt-3 cursor-pointer hover:bg-amber-500/10 p-2 -mx-2 rounded-lg transition-colors group"
                title="Open Supplier Intelligence Center"
              >
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase group-hover:text-amber-600 transition-colors">{t('Total Payables', 'کل واجب الادا')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{totalPayables.toLocaleString(undefined, {maximumFractionDigits:0})}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase group-hover:text-amber-600 transition-colors">{t('Overdue', 'تاخیر شدہ')}</p>
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{overdueCount}</p>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          ...
          {/* Inventory Insights */}
          <div className="premium-card p-6 border flex items-center justify-between bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{t('Inventory Value', 'اسٹاک کی مالیت')}</h3>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                <p className="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded">
                  {t('Potential:', 'ممکنہ:')} {kpis.inventory.potentialRevenue.toLocaleString(undefined, {maximumFractionDigits:0})}
                </p>
                <p className="text-[10px] font-bold text-slate-500 bg-slate-50 dark:bg-white/5 px-2 py-1 rounded">
                  {t('Coverage:', 'احاطہ:')} {kpis.inventory.stockCoverageDays} {t('Days', 'دن')}
                </p>
              </div>
            </div>
            <div className="h-16 w-16 bg-blue-50 dark:bg-blue-500/10 text-blue-500 rounded-full flex items-center justify-center">
          ...
          <div className="premium-card p-6 border flex items-center justify-between bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl shadow-xs">
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{t('Credit Risk Engine', 'ادھار رسک انجن')}</h3>
              ...
              <p className="text-xs font-bold text-slate-500 mt-2">{kpis.credit.overdueCustomers} {t('Accounts Near Limit', 'حد کے قریب اکاؤنٹس')}</p>
            </div>
          ...
          {/* Salary Analytics */}
          <div 
            className="premium-card p-6 border hover:shadow-md hover:border-purple-300 transition-all cursor-pointer group bg-white dark:bg-[#151521] border-slate-200 dark:border-white/10 rounded-2xl shadow-xs"
            onClick={() => setActiveDrillDown('expenses')}
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-xs mt-2">
                <span className="text-slate-500 font-bold">{t('Monthly Avg:', 'ماہانہ اوسط:')}</span>
                <span className="text-slate-700 dark:text-slate-300 font-black">Rs {kpis.expenses.salary.monthlyAvg.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold">{t('% of Total Exp:', 'کل اخراجات کا فیصد:')}</span>
                <span className="text-slate-700 dark:text-slate-300 font-black">{kpis.expenses.salary.percentageOfExpenses.toFixed(1)}%</span>
              </div>
            </div>
          </div>

...
export default function CustomerCreditDrillDownModal({
 ...
 <div className="bg-subtle w-full max-w-full max-w-[1400px] h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-border">
 ...
 <div className="bg-card rounded-xl p-4 shadow-sm relative overflow-hidden lg:col-span-2">
 <div className="absolute -right-4 -top-4 size-16 bg-card/5 rounded-full blur-xl pointer-events-none"></div>
 <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Total Outstanding Exposure</p>
 <p className="text-3xl font-black text-rose-400">
 {formatCurrency(stats.totalOutstanding, settings)}
 </p>
 </div>
 <div className="premium-card p-4 border">
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Month Recoveries</p>
 <p className="text-xl font-black text-emerald-600">+{formatCurrency(stats.thisMonthRecoveries, settings)}</p>
 </div>
 <div className="premium-card p-4 border">
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">New Credit (Month)</p>
 <p className="text-xl font-black text-rose-600">-{formatCurrency(stats.thisMonthNewCredit, settings)}</p>
 </div>
 <div className="premium-card p-4 border">
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Net Movement</p>
 ...
 <div className="premium-card p-4 border">
 <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Collection Eff.</p>
 ...
 <div className="premium-card border border-border">
 <h3 className="text-sm font-bold text-foreground mb-4 flex items-center gap-2">
 <BarChart3 className="h-4 w-4 text-muted-foreground" /> Credit Ageing Profile
 </h3>
 <div className="space-y-3">
 ...
 <div className="p-5 border-b border-border flex justify-between items-center bg-subtle">
 ...
 <div key={debtor.id} className="p-4 border-b border-border last:border-0 hover:bg-slate-50">
 <div className="flex justify-between items-start mb-2">
 <div>
 <p className="text-sm font-bold text-foreground">{debtor.name}</p>
 <p className="text-[10px] text-muted-foreground">Last Pmt: {debtor.lastPayment}</p>
 </div>
 ...
 <p className="text-lg font-black text-foreground">{formatCurrency(debtor.balance, settings)}</p>
 <div className="text-right">
 <p className="text-[10px] font-bold text-muted-foreground">{debtor.usage.toFixed(0)}% of Limit</p>
 ...
 <div className="p-8 text-center text-muted-foreground text-sm font-medium">No outstanding debt.</div>
 )}
 </div>
 </div>
 </div>
 ...
 {/* Grid Filters */}
 <div className="premium-card border border-border p-4 flex flex-wrap ga items-end shrink-0">
 <div className="flex-1 min-w-full max-w-[200px]">
 <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 block">Search Customer</label>
 <select 
 value={customerFilter} 
 onChange={e => setCustomerFilter(e.target.value)}
 className="premium-input text-xs p-2 bg-subtle font-semibold"
 >
 ...
 <div>
 <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 block">From</label>
 <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="premium-input text-xs p-2 bg-subtle" />
 </div>
 <div>
 <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1 block">To</label>
 <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="premium-input text-xs p-2 bg-subtle" />
 </div>
 <div className="flex gap-2">
 <button className="px-3 py-2.5 sm:py-2 min-h-[48px] sm:min-h-[40px] bg-muted hover:bg-slate-200 text-muted-foreground rounded-lg text-xs font-bold transition-colors">
 ...
 <p className="font-bold text-foreground">{entry.date}</p>
 <p className="text-[10px] text-muted-foreground">{entry.time}</p>
 </td>
 <td className="p-3 align-top">
 <p className="font-bold text-foreground">{entry.customerName}</p>
 <p className="text-[10px] text-muted-foreground truncate max-w-full max-w-[150px]">{entry.reference}</p>
 </td>
 ...
 <span className="inline-block mt-1 px-1.5 py-0.5 bg-muted text-muted-foreground rounded text-[9px] font-bold uppercase">{entry.mode}</span>
 </div>
 ) : '-'}
 </td>
 <td className="p-3 align-top text-right">
 <p className={`font-black${entry.runningBalance > 0 ? 'text-foreground' : 'text-emerald-600'}`}>
 {formatCurrency(entry.runningBalance, settings)}
 </p>
 </td>
 <td className="p-3 align-top">
 <div className="flex flex-col gap-1">
 <span className="text-[10px] font-semibold text-muted-foreground">Oper: {entry.operator?.substring(0, 8)}</span>

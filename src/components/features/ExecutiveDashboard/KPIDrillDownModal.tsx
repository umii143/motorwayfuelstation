...
export const KPIDrillDownModal: React.FC<KPIDrillDownModalProps> = ({ isOpen, onClose, kpis, metric }) => {
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase">Calculation Formula</p>
 <p className="text-sm font-semibold text-foreground mt-2">Rate Diff × Stock On Hand</p>
 </div>
 <div className="bg-subtle rounded-xl p-4 border border-border">
 <p className="text-xs font-bold text-muted-foreground uppercase">Source Records</p>
 <p className="text-sm font-semibold text-foreground mt-2">Rate History Ledger</p>
 </div>
 <div className="bg-subtle rounded-xl p-4 border border-border">
 <p className="text-xs font-bold text-muted-foreground uppercase">Total Lifetime Impact</p>
 ...
 <div className="premium-card border-2 border-border">
 <h3 className="text-lg font-black text-foreground mb-2">Inventory Revaluation</h3>
 ...
 <p className="text-xs font-bold text-muted-foreground uppercase">Calculation Formula</p>
 {metric === 'profit' ? (
 <p className="text-sm font-semibold text-foreground mt-2">Revenue - COGS - Expenses</p>
 ) : metric === 'revenue' ? (
 <p className="text-sm font-semibold text-foreground mt-2">Qty Sold × Selling Price</p>
 ) : (
 <p className="text-sm font-semibold text-foreground mt-2">Sum of Categorized Shift Expenses</p>
 )}
 </div>
 <div className="bg-subtle rounded-xl p-4 border border-border">
 <p className="text-xs font-bold text-muted-foreground uppercase">Source Records</p>
 <p className="text-sm font-semibold text-foreground mt-2">Active Shifts & Transactions</p>
 </div>
 <div className="bg-subtle rounded-xl p-4 border border-border">
 <p className="text-xs font-bold text-muted-foreground uppercase">Timestamp</p>
 <p className="text-sm font-semibold text-foreground mt-2">{new Date().toLocaleString()}</p>
 </div>
 </div>

 <div className="premium-card border-2 border-border">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-lg font-black text-foreground">Reconciliation Engine</h3>
 ...
 <span className="text-sm font-semibold text-muted-foreground">Sum(Product Revenue) = Total Revenue</span>
 {isRevenueReconciled ? <CheckCircle2 className="h-4 w-4 text-emerald-500" /> : <AlertOctagon className="h-4 w-4 text-red-500" />}
 </div>
 <div className="flex justify-between items-center py-2 border-b border-border">
 <span className="text-sm font-semibold text-muted-foreground">Sum(Product Profit) = Total Gross Profit</span>
 {isProfitReconciled ? <CheckCircle2 className="h-4 w-4 text-emerald-500" /> : <AlertOctagon className="h-4 w-4 text-red-500" />}
 </div>
 <div className="flex justify-between items-center py-2">
 <span className="text-sm font-semibold text-muted-foreground">Sum(Expense Categories) = Total Expenses</span>
 ...
 <h3 className="text-sm font-bold text-muted-foreground mb-2">Revenue Category Distribution</h3>
 ...
 return (
 <div className="space-y-6">
 <h3 className="text-lg font-black text-foreground">Product Level Breakdown</h3>
 <div className="h-72">
 <ResponsiveContainer width="100%" height="100%">
 ...
 <td className="text-right text-foreground">{fmt(d.value)}</td>
 ...
 return (
 <div className="space-y-6">
 <h3 className="text-lg font-black text-foreground">Expense Categorization</h3>
 <div className="h-64">
 <ResponsiveContainer width="100%" height="100%">
 <PieChart>
 ...
 <tr>
 <td colSpan={3} className="px-4 py-2 bg-subtle">
 ...
 <td className="px-3 font-semibold text-foreground">{sd.employeeName}</td>
 <td className="px-3">{sd.description}</td>
 <td className="px-3 text-right text-muted-foreground">{fmt(sd.amount)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 return (
 <div className="space-y-6">
 <h3 className="text-lg font-black text-foreground">Historical Trend Analysis</h3>
 <div className="h-80 w-full premium-card border border-border">
 ...
 <h3 className="text-lg font-black text-foreground">Traceability Ledger</h3>
 <span className="text-xs font-bold text-muted-foreground uppercase">{breakdowns.ledgerTransactions.length} Records Found</span>
 </div>
 ...
 <td className="font-semibold text-foreground">{tx.date}</td>
 ...
 <td className="text-muted-foreground truncate max-w-xs">{tx.description}</td>
 ...
 <p className="font-semibold text-muted-foreground">No source transactions found</p>
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-card backdrop-blur-sm animate-fade-in">
 <div className="bg-card rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up border border-border">
 
 {/* Header */}
 <div className="px-6 py-5 border-b border-border flex items-center justify-between bg-subtle">
 ...
 <div>
 {/* @ts-expect-error */}
 <h2 className="text-xl font-black text-foreground">{titleMap[metric]}</h2>
 <p className="text-sm font-bold text-muted-foreground">Drill Down & Analytics</p>
 </div>
 </div>
 <div className="flex items-center gap-6">
 <div className="text-right hidden sm:block">
 <p className="text-xs font-bold text-muted-foreground uppercase">YTD Total</p>
 <p className="text-xl font-black text-foreground">{fmt(totalValue)}</p>
 </div>
 <button onClick={onClose} className="p-2 text-muted-foreground hover:text-slate-600 hover:bg-slate-200 rounded-full transition-colors">
 <X className="h-6 w-6" />
 </button>
 </div>
 </div>
 ...
 {/* Tabs */}
 <div className="px-6 border-b border-border flex gap-6 overflow-x-auto">
 {tabs.map(t => (
 ...
 {/* Content */}
 <div className="p-6 overflow-y-auto flex-1 bg-subtle">
 {renderTabContent()}
 </div>

...
export const KPIDrillDownModal: React.FC<KPIDrillDownModalProps> = ({ isOpen, onClose, kpis, metric }) => {
  ...
  const renderTabContent = () => {
    if (metric === 'revaluation') {
      return (
        <div className="space-y-6">
          ...
          <div className="premium-card border-2 border-slate-200 dark:border-white/10">
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-200 mb-2">Inventory Revaluation</h3>
    ...
    switch (activeTab) {
          ...
          <div className="space-y-6">
            ...
            <div className="premium-card border-2 border-slate-200 dark:border-white/10">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">Reconciliation Engine</h3>
      ...
      case 'products':
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">Product Level Breakdown</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
      ...
      case 'expenses':
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">Expense Categorization</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    ...
                    return (
                      <React.Fragment key={i}>
                        ...
                        {d.name === 'salary' && breakdowns.salaryDetails && breakdowns.salaryDetails.length > 0 && (
                          <tr>
                            <td colSpan={3} className="px-4 py-2 bg-slate-50 dark:bg-white/5/50">
                              <div className="border border-slate-200 dark:border-white/10 rounded-lg overflow-hidden my-2">
                                <table className="premium-table">
                                  ...
                                  <tbody>
                                    {breakdowns.salaryDetails.map((sd, idx) => (
                                      <tr key={idx} className="hover:bg-slate-50 dark:bg-white/5">
      ...
      case 'trends':
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">Historical Trend Analysis</h3>
            <div className="h-80 w-full premium-card border border-slate-200 dark:border-white/10">
        ...
        return (
          <div className="space-y-4">
            <div className="flex justify-between items-end mb-2">
              <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">Traceability Ledger</h3>
              <span className="text-xs font-bold text-slate-400 uppercase">{breakdowns.ledgerTransactions.length} Records Found</span>
            </div>
            <div className="overflow-hidden border border-slate-200 dark:border-white/10 rounded-xl bg-white dark:bg-[#151521] shadow-sm">
                  ...
                  <tbody>
                    {breakdowns.ledgerTransactions.length > 0 ? (
                      breakdowns.ledgerTransactions.map((tx, i) => (
                        <tr key={`${tx.id}-${i}`} className="hover:bg-slate-50 dark:bg-white/5 transition-colors">
                          ...
                          <td className="text-slate-600">
                            <span className={`px-2 py-1 rounded text-xs font-bold ${
                              ...
                              'bg-slate-100 dark:bg-white/10 text-slate-700'
                            }`}>
                              {tx.type}
                            </span>
                          </td>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-[#151521] rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-slide-up border border-slate-200 dark:border-white/10">
        ...
        <div className="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50 dark:bg-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              {/* @ts-expect-error */}
              <h2 className="text-xl font-black text-slate-900 dark:text-white">{titleMap[metric]}</h2>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xl font-black text-slate-800 dark:text-slate-200">{fmt(totalValue)}</p>
            </div>
        ...
        {/* Tabs */}
        <div className="px-6 border-b border-slate-100 dark:border-white/5 flex gap-6 overflow-x-auto">
          {tabs.map(t => (
        ...
        {/* Content */}
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50 dark:bg-white/5/50">
          {renderTabContent()}
        </div>

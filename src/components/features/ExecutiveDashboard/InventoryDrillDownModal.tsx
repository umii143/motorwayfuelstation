...
export default function InventoryDrillDownModal({
 ...
 className="relative w-full max-w-6xl max-h-[90vh] bg-subtle rounded-2xl shadow-2xl overflow-hidden flex flex-col"
 >
 {/* HEADER */}
 ...
 <div>
 <h2 className="text-xl font-black text-foreground tracking-tight">Enterprise Inventory Intelligence</h2>
 <p className="text-sm font-medium text-muted-foreground">Revaluation, Stock Worth & Price Change Analytics</p>
 </div>
 </div>
 <button 
 onClick={onClose}
 className="h-10 w-10 flex items-center justify-center rounded-full bg-muted text-muted-foreground hover:bg-slate-200 hover:text-slate-700 transition-colors cursor-pointer"
 >
 <X className="h-5 w-5" />
 </button>
 </div>
 ...
 <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest block mb-1">Current Stock Value</span>
 <h3 className="text-2xl font-black text-foreground">{formatCurrency(currentStockValue, settings)}</h3>
 ...
 />
 <span className="text-xs font-bold text-foreground">Exclude Manual Corrections</span>
 </label>
 </div>
 </div>
 ...
 <span className="text-sm font-bold text-foreground">{prod}</span>
 ...
 <div className="text-center py-6 text-sm text-muted-foreground font-medium">No price changes recorded yet.</div>
 )}
 </div>
 </div>

 {/* Info Block */}
 <div className="bg-subtle rounded-xl border border-border p-5 shadow-sm space-y-4">
 ...
 <div className="bg-card text-foreground p-3 rounded-lg font-mono text-xs">
 ...
 <h3 className="text-sm font-bold text-foreground uppercase tracking-wider">Comprehensive Price Revision Ledger</h3>
 ...
 className="pl-8 pr-3 py-1.5 border border-border rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-purple-500 bg-card w-64"
 />
 </div>
 </div>
 ...
 <tr>
 <td colSpan={6} className="p-8 text-center text-muted-foreground text-sm font-medium">
 ...
 <td className="p-3">
 {/* @ts-expect-error */}
 <div className="font-bold text-foreground text-xs">{new Date(txn.timestamp).toLocaleDateString()}</div>
 {/* @ts-expect-error */}
 <div className="text-[10px] font-medium text-muted-foreground">{new Date(txn.timestamp).toLocaleTimeString()}</div>
 </td>
 <td className="p-3">
 <div className="font-bold text-foreground text-xs">{prod?.name || 'any'}</div>
 </td>
 <td className="p-3">
 <div className="flex items-center justify-center gap-2">
 <span className="font-mono text-xs text-muted-foreground">{(txn.oldRate || 0).toFixed(2)}</span>
 <ArrowRightLeft className="h-3 w-3 text-muted-foreground" />
 <span className="font-mono text-xs font-bold text-foreground">{(txn.newRate || 0).toFixed(2)}</span>
 </div>
 ...
 <span className="font-mono text-xs font-bold text-foreground">{txn.stockOnHand?.toLocaleString() || 0}</span>
 </td>
 <td className="p-3 text-right">
 ...
 <td className="p-3">
 <div className="font-bold text-foreground text-xs truncate max-w-full max-w-[150px]">{txn.reason}</div>
 <div className="text-[10px] font-medium text-muted-foreground">{txn.changedBy}</div>
 </td>
 </tr>
 );
 })

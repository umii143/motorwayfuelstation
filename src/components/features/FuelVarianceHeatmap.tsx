...
export function FuelVarianceHeatmap({ tanks, shifts, nozzles }: FuelVarianceHeatmapProps) {
 ...
 <div>
 <h3 className="text-base font-bold text-foreground">Fuel Variance Heatmap</h3>
 <p className="text-xs text-muted-foreground">7-Day Tank Shrinkage & Expansion Analysis</p>
 </div>
 </div>
 ...
 <p className="text-muted-foreground font-bold">No Data Available</p>
 <p className="text-xs text-muted-foreground">Close at least one shift with tank dips to see variance heatmap.</p>
 </div>
 </td></tr>
 ) : (
 ...
 <div className="text-xs text-muted-foreground">{data.tank.capacity.toLocaleString()}L Capacity</div>
 </td>
 ...
 <div className={`flex items-center gap-1 font-black text-sm${data.total7DayLoss < -30 ? 'text-rose-500' : 'text-slate-600 '}`}>
 {data.total7DayLoss < -30 ? <AlertTriangle className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
 {data.total7DayLoss}L
 </div>
 <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Total Loss</div>
 </div>
 </td>
 </tr>
 ))

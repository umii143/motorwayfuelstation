...
export default function WhatsAppAlerts({ settings, onUpdateSettings }: Props) {
 ...
 <div className="absolute right-[-40px] top-[-40px] h-64 w-64 bg-card/5 rounded-full blur-3xl pointer-events-none transition-all duration-700 group-hover:bg-card/10" />
 ...
 <span className="inline-flex items-center gap-1.5 bg-card/15 backdrop-blur-md border border-border px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase mb-4 shadow-sm text-emerald-50">
 ...
 <h3 className="font-sans text-2xl font-black text-foreground tracking-tight">
 {t('Device Linking', 'ڈیوائس لنکنگ')}
 </h3>
 </div>

 <div className="bg-subtle border border-border rounded-2xl p-6 mb-8 flex flex-col md:flex-row items-center gap-8">
 <div className="flex-1 space-y-4">
 <h4 className="font-sans font-black text-foreground text-lg">
 ...
 <ol className="list-decimal pl-4 text-xs font-semibold text-muted-foreground space-y-2 font-sans">
 ...
 <div className="w-64 h-64 bg-muted rounded-2xl border border-border flex items-center justify-center relative overflow-hidden">
 {waStatus.ready ? (
 ...
 <p className="text-xs text-muted-foreground mt-1">Ready to send alerts.</p>
 </div>
 ) : waStatus.qr ? (
 <div className="premium-card p-2 border-border inline-block">
 <QRCodeSVG 
 value={waStatus.qr} 
 size={200} 
 bgColor="#ffffff" 
 ...
 <p className="text-xs font-bold text-muted-foreground">Loading Client...</p>
 </div>
 ) : (
 <div className="text-center p-4">
 <QrCode className="h-10 w-10 text-muted-foreground mx-auto mb-2" />
 <p className="text-xs font-semibold text-muted-foreground">Click generate to view QR</p>
 </div>
 )}
 </div>
 </div>
 ...
 <div>
 <label className="block font-sans text-sm font-black text-foreground mb-2.5">
 ...
 className="relative w-full bg-subtle border border-border rounded-xl pl-4 pr-12 py-3.5 text-lg font-mono font-bold focus:bg-card focus:border-emerald-500 focus:ring-emerald-500 transition-all shadow-sm"
 placeholder="+92 3XX XXXXXXX"
 />
 <MessageCircle className="absolute right-4 top-4 h-6 w-6 text-muted-foreground group-hover:text-emerald-500 transition-colors" />
 </div>
 <p className="font-sans text-xs font-semibold text-muted-foreground mt-2.5">
 {t('Must include country code (e.g., +92 for Pakistan)', 'ملکی کوڈ شامل کرنا لازمی ہے (جیسے پاکستان کے لیے +92)')}
 </p>
 </div>

 <div className="pt-2">
 <h4 className="font-sans text-sm font-black text-foreground mb-4 flex items-center gap-2">
 ...
 : 'border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-card/5/50 hover:bg-slate-50 dark:bg-card/5'
 }`}
 >
 ...
 className="h-5 w-5 rounded border-border text-emerald-600 focus:ring-emerald-600 focus:ring-offset-emerald-50 cursor-pointer transition-colors"
 />
 </div>
 <label htmlFor={item.id} className="cursor-pointer flex-1">
 <span className="block font-sans font-black text-foreground text-sm mb-1">{item.label}</span>
 <span className="block font-sans text-xs font-semibold text-muted-foreground leading-relaxed">{item.desc}</span>
 </label>
 </div>
 ))}
 </div>
 ...
 <div className="pt-8 mt-4 border-t border-border flex justify-end">
 <button
 onClick={handleSave}
 ...
 : 'bg-card hover:bg-card shadow-xl shadow-slate-900/20 hover:shadow-slate-900/30'
 }`}
 >
 {saved ? (
 <>

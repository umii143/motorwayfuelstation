...
export const BIDashboard: React.FC = () => {
 ...
 <div className="premium-card p-6 border border-border bg-card rounded-2xl">
 ...
 <h2 className="text-xl font-black text-foreground tracking-tight mt-8">
 ...
 <h3 className="text-lg font-bold text-foreground">{forecast.tankName}</h3>
 <p className="text-xs font-semibold text-muted-foreground mb-4">{forecast.productId}</p>
 
 <div className="space-y-3">
 <div className="flex justify-between items-center">
 <span className="text-sm text-muted-foreground">{t('Current Stock', 'موجودہ اسٹاک')}</span>
 <span className="text-sm font-bold">{forecast.currentStock.toLocaleString()} L</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-sm text-muted-foreground">{t('Daily Burn Rate', 'روزانہ کے استعمال کی شرح')}</span>
 <span className="text-sm font-bold">{Math.round(forecast.averageConsumption).toLocaleString()} L/{t('day', 'دن')}</span>
 </div>
 <div className="flex justify-between items-center pt-2 border-t border-border">
 <span className="text-sm text-muted-foreground">{t('Runs Empty In', 'خالی ہونے کا وقت')}</span>
 ...
 <span className="text-sm text-muted-foreground">{t('Suggested Order', 'تجویز کردہ آرڈر')}</span>
...
const KPICard = React.memo(({ title, value, icon: Icon, benchmark, subValue, t }: any) => {
 return (
 <div className="premium-card p-5 border border-border bg-card rounded-2xl shadow-xs">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-sm font-bold text-muted-foreground">{title}</h3>
 <div className="p-2 rounded-lg bg-subtle text-muted-foreground">
 ...
 {subValue && !benchmark && (
 <div className="text-xs font-bold text-muted-foreground mt-2">{subValue}</div>
 )}
 </div>
 );
});

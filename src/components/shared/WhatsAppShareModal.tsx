    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-2xl bg-white dark:bg-[#151521] rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-[#25D366]/10">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-[#25D366] text-white rounded-full">
                <MessageSquare className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Share via WhatsApp</h2>
                <p className="text-xs text-slate-500 font-medium">Instantly send documents & reports</p>
              </div>
            </div>
            <button
              onClick={closeShareModal}
              className="p-2 text-slate-400 hover:bg-slate-100 dark:bg-white/10 hover:text-slate-600 rounded-full transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="flex items-center gap-3 p-3 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:bg-white/5 transition-colors">
                      <input 
                        type="radio" 
                        name="format" 
                        checked={format === 'text'} 
                    ...
                    <label className={`flex items-center gap-3 p-3 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:bg-white/5 transition-colors ${!(shareState.pdfBlob || shareState.pdfDocument) ? 'opacity-50 pointer-events-none' : ''}`}>
                      <input 
                        type="radio" 
                        name="format" 
                        checked={format === 'both'} 
              ...
              <div>
                ...
                <div className="bg-[#E5DDD5] rounded-xl p-4 h-full min-h-[300px] border border-slate-200 dark:border-white/10 relative shadow-inner">
                  {/* Mock WhatsApp Message Bubble */}
                  <div className="bg-[#DCF8C6] rounded-lg p-3 text-sm text-slate-800 dark:text-slate-200 shadow-xs relative max-w-[90%] whitespace-pre-wrap font-sans leading-relaxed">
                    {previewMessage}
          ...
          {/* Footer Actions */}
          <div className="border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 p-4 flex justify-end gap-3 rounded-b-xl">
            <button
              onClick={closeShareModal}
              className="px-5 py-2.5 text-sm font-bold text-slate-600 bg-white dark:bg-[#151521] border border-slate-300 rounded-lg hover:bg-slate-50 dark:bg-white/5 transition-colors"
            >
              Cancel
            </button>
            <button

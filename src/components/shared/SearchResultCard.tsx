...
export function SearchResultCard({
 ...
 <button
 onClick={onClick}
 className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors border-b border-outline-variant/50 last:border-0${isSelected
 ? 'bg-secondary/10 border-l-2 border-l-secondary'
 : 'hover:bg-surface-container'}`}
 >
 {/* Icon */}
 <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center${getIconBg(result.type)}`}>
 ...
 {result.badgeText && (
 <span className={`text-xs px-1.5 py-0.5 rounded font-bold${getBadgeStyles(result.badgeColor)}`}>
 {result.badgeText}
 </span>
 )}
 </div>

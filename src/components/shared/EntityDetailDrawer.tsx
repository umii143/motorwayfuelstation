...
const KIND_COLOR: Record<GraphNodeKind, string> = {
  ...
  journal: 'border-slate-500/30 text-slate-600 bg-slate-50 dark:bg-slate-500/10',
...
export default function EntityDetailDrawer({ entity, onClose, onNavigateModule, onReanchor }: EntityDetailDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-end">
      ...
      <div className="relative w-full max-w-md h-screen flex flex-col bg-white dark:bg-[#151521] border-l border-slate-200 dark:border-white/10 shadow-2xl animate-slide-in">
        {/* Header */}
        <div className="h-16 border-b border-slate-200 dark:border-white/10 flex items-center justify-between px-5 bg-slate-50 dark:bg-white/5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-black text-sm text-slate-900 dark:text-white capitalize leading-tight">{rootNode ? rootNode.label : entity.id}</h3>
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{rootNode ? kindLabel(rootNode.kind) : entity.kind} • {degree} linked</span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {rootNode && (
            <div className={`rounded-xl border p-4 ${KIND_COLOR[rootNode.kind]}`}>
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-white/60 dark:bg-black/20">{kindLabel(rootNode.kind)}</span>
              </div>
              <h4 className="font-black text-sm text-slate-900 dark:text-white mt-2">{rootNode.label}</h4>
              {rootNode.sublabel && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{rootNode.sublabel}</p>}
              {rootNode.meta && <p className="text-xs font-bold text-slate-700 dark:text-slate-200 mt-1">{rootNode.meta}</p>}
              <button
                onClick={() => onNavigateModule(rootNode.viewId, rootNode.contextData)}
                className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-lg bg-slate-900 text-white text-xs font-bold py-2 hover:bg-slate-800 transition-colors cursor-pointer"
              >
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h5 className="text-xs font-black uppercase tracking-widest text-slate-500 flex items-center gap-1.5">
            ...
            {showGraph ? (
              ...
              <div className="space-y-2">
                {linked.length === 0 && (
                  <p className="text-xs text-slate-400 italic text-center py-4">No linked records found in this station.</p>
                )}
                {linked.map(n => (
                  <button
                    key={n.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-xs text-slate-900 dark:text-white truncate">{n.label}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{kindLabel(n.kind)} • {n.sublabel}</div>
                    </div>
...
function BusinessGraphView({ nodes, edges, rootId, onSelect }: { nodes: GraphNode[]; edges: any[]; rootId?: string; onSelect: (ref: EntityRef) => void }) {
  const isUrdu = false;
  return (
    <div className="rounded-xl border border-slate-200 dark:border-white/10 p-4 bg-slate-50/40 dark:bg-white/2 space-y-3">
      {nodes.map((n, i) => {
          ...
          <div key={n.id} className="flex items-start gap-2">
            {i === 0 ? (
              ...
              <div className="flex flex-col items-center shrink-0 w-2">
                <span className="w-px h-3 bg-slate-300 dark:bg-white/20" />
                <span className="w-2 h-2 rounded-full bg-slate-400 mt-1" />
              </div>
            )}
            <button
              onClick={() => onSelect({ kind: n.kind as any, id: n.id.split(':')[1] })}
              className="flex-1 text-left rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#151521] px-3 py-2 hover:border-orange-500 transition-colors"
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-xs text-slate-800 dark:text-slate-200">{n.label}</span>
                <span className="text-[9px] font-black uppercase text-slate-400">{n.kind}</span>
              </div>
      ...
      <p className="text-[10px] text-slate-400 pt-1">{!isUrdu ? 'Click any node to re-anchor the trail.' : ''}</p>
    </div>
  );
}

...
function ResponsiveTableInner<T>({
 ...
 if (data.length === 0) {
 return (
 <div className="flex flex-col items-center justify-center py-12 px-4 text-muted-foreground bg-subtle rounded-xl border border-border border-dashed">
 ...
 className={`transition-colors${onRowClick ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-card/5' : ''}${isExpanded ? 'bg-slate-50 dark:bg-card/5' : ''}`}
 >
 {renderExpandedRow && (
 <td className="px-4 py-3">
 <button onClick={(e) => toggleRow(key, e)} className="p-1 rounded-md hover:bg-slate-200 text-muted-foreground">
 ...
 <tr>
 <td colSpan={columns.length + (renderActions ? 2 : 1)} className="p-0 border-b border-border bg-subtle">
 <AnimatePresence>
 <motion.div
 ...
 <div 
 key={key} 
 className={`bg-card rounded-xl border border-border shadow-sm overflow-hidden transition-all${onRowClick ? 'active:scale-[0.98]' : ''}`}
 onClick={() => onRowClick?.(row)}
 >
 <div className="p-4 flex items-start justify-between gap-3">
 <div className="flex-1 min-w-0">
 {/* Primary Header */}
 <div className="font-bold text-foreground text-sm mb-0.5 truncate">
 ...
 className="p-1.5 rounded-lg bg-subtle text-muted-foreground hover:bg-muted"
 >
 {isExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
 </button>
 )}
 ...
 className="overflow-hidden"
 >
 <div className="px-4 pb-4 pt-1 border-t border-border bg-subtle">
 ...
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">{col.header}</span>

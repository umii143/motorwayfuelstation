      ...
      {activeTab !== 'manual' ? (
        <div className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:ring-1 focus:ring-green-500 outline-none"
            />
          </div>
          <div className="max-h-48 overflow-y-auto border border-slate-200 dark:border-white/10 rounded-lg divide-y divide-slate-100">
            {filteredContacts.length === 0 ? (
              ...
              filteredContacts.map(contact => (
                <div
                  ...
                  className={`p-3 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:bg-white/5 transition-colors ${
                    selectedContact?.id === contact.id ? 'bg-green-50 border-l-2 border-green-500' : ''
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className="p-1.5 bg-slate-100 dark:bg-white/10 rounded-full text-slate-500">
                      {getIcon(contact.type)}
                    </div>
                    <div>
                      <div className="text-sm font-medium text-slate-800 dark:text-slate-200">{contact.name}</div>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:ring-1 focus:ring-green-500 outline-none"
          />

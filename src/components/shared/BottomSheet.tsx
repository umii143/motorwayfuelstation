...
export const BottomSheet: React.FC<BottomSheetProps> = ({ 
 ...
 <AnimatePresence>
 {isOpen && (
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-card backdrop-blur-xs">
 <motion.div 
 initial={{ opacity: 0, scale: 0.95 }}
 animate={{ opacity: 1, scale: 1 }}
 exit={{ opacity: 0, scale: 0.95 }}
 className={`w-full${maxWidth}rounded-2xl border border-border bg-card shadow-2xl flex flex-col max-h-[90dvh]`}
 >
 {title && (
 <div className="flex items-center justify-between p-6 pb-4 border-b border-border">
 <h2 className="font-sans text-lg font-bold text-foreground">{title}</h2>
 <button onClick={onClose} className="rounded-lg p-1 text-muted-foreground hover:bg-muted hover:text-slate-600 transition-colors">
 <X className="h-5 w-5" />
 </button>
 </div>
 )}
 ...
 exit={{ opacity: 0 }}
 onClick={onClose}
 className="fixed inset-0 z-[60] bg-card backdrop-blur-sm lg:hidden"
 />
 <motion.div
 drag="y"

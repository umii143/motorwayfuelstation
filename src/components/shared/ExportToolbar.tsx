...
export function ExportToolbar({ isOpen, onClose, data, columns, title, filenamePrefix }: ExportToolbarProps) {
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-center justify-center bg-card backdrop-blur-xs">
 ...
 <button
 onClick={handlePrint}
 className="flex flex-col items-center justify-center gap-2 rounded-xl border border-border bg-subtle p-4 hover:bg-muted hover:border-border transition-colors cursor-pointer"
 >
 <Printer className="h-6 w-6 text-muted-foreground" />
 <span className="font-bold text-xs text-foreground">Print / Thermal</span>
 </button>
 ...
 <span className="text-[10px] text-muted-foreground font-mono">Powered by Umar Ali ⚡</span>
 </div>
 </div>
 </div>
 );

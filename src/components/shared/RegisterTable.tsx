...
export function RegisterTable<T>({
 ...
 className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border transition-colors${showFilters || hasActiveFilters ? 'border-orange-500 text-orange-600 bg-orange-50/20' : 'border-border text-slate-500'}`}
 >
 <Filter className="w-3.5 h-3.5" />
 {t('Filters', 'فلٹرز')}
 {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />}
 </button>
 <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border border-border text-muted-foreground hover:text-foreground dark:hover:text-slate-200 transition-colors">
 <Download className="w-3.5 h-3.5" /> CSV
 </button>
 <button onClick={() => window.print()} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold border border-border text-muted-foreground hover:text-foreground dark:hover:text-slate-200 transition-colors">
 ...
 {/* Filter row */}
 {showFilters && (
 <div className="flex flex-wrap gap-2 p-3 border-b border-border bg-subtle">
 {columns.filter(c => c.filterOptions).map(c => (
 <select
 key={c.key}
 value={filters[c.key] || '__all__'}
 onChange={e => setFilters(prev => ({ ...prev, [c.key]: e.target.value }))}
 className="bg-card border border-border rounded-lg text-xs px-2 py-1.5 text-foreground outline-none"
 >
 ...
 <thead className="sticky top-0 z-10 bg-muted text-foreground font-bold">
 <tr>
 <th className="p-2.5 w-[44px] text-center border-b border-border">#</th>
 {columns.map(c => (
 <th
 key={c.key}
 onClick={() => c.isNumeric && toggleSort(c.key)}
 className={`p-2.5 border-b border-border${c.isNumeric ? 'text-right cursor-pointer select-none hover:text-orange-600' : ''}`}
 >
 ...
 {hasRunning && (
 <th className="p-2.5 text-right border-b border-border">{t('Running Total', 'رننگ ٹوٹل')}</th>
 )}
 </tr>
 </thead>
 ...
 <tr>
 <td colSpan={columns.length + 1 + (hasRunning ? 1 : 0)} className="p-10 text-center text-muted-foreground italic">
 {emptyMessage}
 </td>
 </tr>
 ) : (
 rowsWithRunning.map(({ row, running }, idx) => (
 <tr key={keyExtractor(row, idx)} className="even:bg-slate-50/40 dark:even:bg-card/2 hover:bg-orange-50/10 dark:hover:bg-white/5 transition-colors">
 <td className="p-2 text-center font-mono text-muted-foreground border-r border-border">{idx + 1}</td>
 {columns.map(c => {
 ...
 return (
 <td key={c.key} className={`p-2${c.isNumeric ? 'text-right font-mono text-foreground' : 'text-slate-600 '}`}>
 {node}
 </td>
 );
 })}
 ...
 {totalKeys.length > 0 && (
 <tfoot className="sticky bottom-0 z-10 bg-card text-foreground">

...
export function ActivityFeedWidget() {
 ...
 return (
 <div className="w-full h-full bg-card backdrop-blur-md rounded-2xl border border-border shadow-xl flex flex-col p-6 overflow-hidden">
 ...
 <div className="absolute left-2.5 top-8 bottom-0 w-px bg-card/5"></div>
 <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border border-border${shift.status === 'active' ? 'bg-orange-500/20 text-orange-400' : 'bg-card text-slate-400'}`}>
 <Power className="w-3 h-3" />
 </div>
 <div className="flex-1">
 <div className="flex justify-between items-start mb-1">
 <span className="text-xs font-black text-white">Shift {shift.status === 'active' ? 'Started' : 'Closed'}</span>
 <span className="text-[9px] font-bold text-muted-foreground">{shift.time || '12:00 PM'}</span>
 </div>
 <div className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{shift.cashierName || 'System'}</div>
 {shift.status === 'closed' && (

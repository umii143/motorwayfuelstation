...
export function WidgetWrapper({ instance, manifest, children }: WidgetWrapperProps) {
 ...
 {isEditMode && (
 <div className="absolute top-0 left-0 right-0 h-10 bg-card backdrop-blur-md z-50 flex items-center justify-between px-3 border-b border-border opacity-0 group-hover:opacity-100 transition-opacity">
 <div className="flex items-center gap-2">
 <div className="drag-handle cursor-grab active:cursor-grabbing p-1 text-muted-foreground hover:text-white">
 <GripHorizontal className="w-4 h-4" />
 </div>
 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{manifest.name}</span>
 </div>
 <div className="flex items-center gap-1">
 <button className="p-1.5 text-muted-foreground hover:text-white rounded-lg hover:bg-card/10 transition-colors">
 ...
 <div className="text-xs font-bold text-muted-foreground">Widget Crashed</div>
 <div className="text-[10px] text-muted-foreground">Failed to render {manifest.name}</div>
 </div>
 }>
 <Suspense fallback={

...
export function WidgetStudioDrawer({ isOpen, onClose }: WidgetStudioDrawerProps) {
 ...
 return (
 <>
 <div className="fixed inset-0 bg-card backdrop-blur-sm z-[100]" onClick={onClose} />
 
 <div className={`fixed top-0 right-0 h-full w-[400px] max-w-full${liquidGlass}z-[101] flex flex-col`}>
 
 {/* Header */}
 <div className="p-6 border-b border-border flex justify-between items-center bg-card/[0.02]">
 <div>
 <h2 className="text-xl font-black text-white flex items-center gap-2">
 <LayoutTemplate className="w-5 h-5 text-indigo-400" />
 Widget Studio
 </h2>
 <div className="text-xs font-medium text-muted-foreground mt-1">Customize your operating system</div>
 </div>
 <button onClick={onClose} className="p-2 text-muted-foreground hover:text-white rounded-full hover:bg-card/10 transition-colors">
 <X className="w-5 h-5" />
 </button>
 </div>

 {/* Search & Tabs */}
 <div className="px-6 pt-4 border-b border-border bg-card/[0.02]">
 ...
 className="w-full bg-black/20 border border-border rounded-xl py-2 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all"
 />
 </div>
 ...
 <div key={manifest.id} className="p-4 rounded-[16px] bg-card/[0.03] border border-white/[0.05] hover:bg-card/[0.05] transition-colors group">
 <div className="flex justify-between items-start mb-2">
 <div>
 <h3 className="text-sm font-bold text-white group-hover:text-indigo-300 transition-colors">{manifest.name}</h3>
 <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">{manifest.category} • {manifest.size}</div>
 </div>
 <button 
 ...
 <p className="text-xs text-muted-foreground mb-3">{manifest.description}</p>

...
export function DashboardShell({ onStartShiftQuick, onNavigate }: { onStartShiftQuick?: () => void, onNavigate?: (path: string) => void }) {
 ...
 const themeWrap ="min-h-screen bg-background text-foreground font-sans overflow-x-hidden pb-32 relative transition-colors duration-500";
 ...
 <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-card rounded-full blur-[160px] mix-blend-screen"></div>
 ...
 {!activeShift && (
 <button onClick={onStartShiftQuick} className="px-5 py-2.5 min-h-[44px] min-w-[44px] bg-card hover:bg-[#9A3412] text-foreground rounded-xl text-base font-bold border border-[#C2410C] shadow-[0_0_15px_rgba(194,65,12,0.3)] transition-all">
 Start Shift
 </button>
 )}
 <button onClick={() => onNavigate?.('shift_logs')} className="px-4 py-2 bg-card/10 hover:bg-card/20 text-white rounded-xl text-xs font-bold border border-border transition-colors">
 Shift Logs
 </button>
 <button 

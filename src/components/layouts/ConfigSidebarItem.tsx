...
export function ConfigSidebarItem({ step, isActive, onClick }: Props) {
 ...
 className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-150 text-left${styles.bgColor}${styles.cursor}${styles.opacity}hover:bg-[var(--bg-hover)]${!step.isAccessible ? 'pointer-events-none' : ''}`}
 >
 {/* Step number badge */}
 <span className={`flex-shrink-0 w-5 h-5 rounded-full text-xs font-bold flex items-center justify-center${step.status === 'completed' 

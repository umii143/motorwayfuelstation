...
export default function AuthInterface({ settings, onLoginSuccess }: AuthInterfaceProps) {
 ...
 return (
 <div className="h-[100dvh] w-full bg-card text-foreground flex flex-col font-sans selection:bg-orange-500/30 overflow-y-auto overflow-x-hidden scroll-smooth relative">
 ...
 <div className="flex-none flex flex-col xl:flex-row min-h-full">
 
 {/* ── LEFT SIDE (Branding & Features) ── */}
 <div className="w-full xl:w-[55%] 2xl:w-[60%] p-6 lg:p-12 2xl:p-16 flex flex-col justify-between relative order-2 xl:order-1 border-t xl:border-t-0 xl:border-r border-border bg-card">
 ...
 <p className="text-[9px] tracking-[0.2em] text-muted-foreground uppercase font-bold">
 ...
 {/* Hero Image */}
 <div className="relative w-full max-w-2xl aspect-[16/7] lg:aspect-[21/9] rounded-2xl overflow-hidden shadow-2xl shadow-orange-500/10 border border-border mt-4 group">
 ...
 {features.map(f => (
 <div key={f.name} className="flex flex-col items-center justify-center p-4 lg:p-5 rounded-2xl border border-border bg-card/[0.02] hover:bg-card/[0.04] transition-all hover:-translate-y-1 gap-3 text-center group cursor-default">
 <f.icon className="h-7 w-7 lg:h-8 lg:w-8 text-orange-500 group-hover:scale-110 transition-transform" />
 <span className="text-xs text-muted-foreground font-medium leading-tight">{f.name}</span>
 </div>
 ))}
 </div>
 ...
 <div className="w-full xl:w-[45%] 2xl:w-[40%] flex flex-col items-center justify-center p-6 lg:p-12 relative z-10 order-1 xl:order-2 bg-card min-h-[100dvh] xl:min-h-auto shadow-[-20px_0_50px_rgba(0,0,0,0.5)]">
 ...
 <div className="w-full max-w-md bg-card rounded-[2rem] border border-border p-8 shadow-2xl relative">
 
 {/* Circular Icon Top */}
 <div className="flex justify-center mb-8">
 <div className="h-20 w-20 rounded-full border border-orange-500/30 flex items-center justify-center bg-card relative shadow-[0_0_30px_rgba(249,115,22,0.15)]">
 ...
 <p className="text-muted-foreground text-sm">{t('Sign in to continue to FuelPro', 'فیول پرو میں جاری رکھنے کے لیے لاگ ان کریں')}</p>
 </motion.div>
 )}
 {authMode ==="signup" && (
 ...
 <p className="text-muted-foreground text-sm">{t('Sign up to get started with FuelPro', 'فیول پرو شروع کرنے کے لیے سائن اپ کریں')}</p>
 </motion.div>
 )}
 {authMode ==="signup_otp" && (
 ...
 <p className="text-muted-foreground text-sm">{t('Enter the 6-digit code sent to your email', 'ای میل پر بھیجا گیا 6 ہندسوں کا کوڈ درج کریں')}</p>
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <User className="h-5 w-5 text-muted-foreground group-focus-within:text-orange-500 transition-colors" />
 </div>
 <input
 type="email"
 value={email}
 ...
 className="w-full bg-transparent border border-border rounded-xl py-3.5 pl-12 pr-4 text-[16px] text-white focus:outline-none focus:border-orange-500 focus:bg-card/5 transition-all placeholder:text-slate-600"
 />
 </div>
 </div>
 ...
 <div className="w-4 h-4 rounded border border-border bg-transparent flex items-center justify-center group-hover:border-orange-500 transition-colors">
 <div className="w-2.5 h-2.5 rounded-sm bg-orange-500 opacity-0 group-hover:opacity-100 transition-opacity"></div>
 </div>
 <span className="text-xs text-muted-foreground select-none">{t("Remember Me","مجھے یاد رکھیں")}</span>
 </label>
 ...
 <div className="relative px-4 text-xs text-muted-foreground bg-card">
 ...
 disabled={isLoading}
 className="flex items-center justify-center gap-2 bg-transparent border border-border hover:border-white/20 hover:bg-card/5 rounded-xl py-3 px-4 transition-all group disabled:opacity-50"
 >
 ...
 <span className="font-sans text-xs font-medium text-muted-foreground group-hover:text-white transition-colors">
 Google
 </span>
 </button>
 <button
 type="button"
 disabled={isLoading}
 className="flex items-center justify-center gap-2 bg-transparent border border-border hover:border-white/20 hover:bg-card/5 rounded-xl py-3 px-4 transition-all group disabled:opacity-50 opacity-50 cursor-not-allowed"
 title="Coming Soon"
 >
 ...
 <span className="font-sans text-xs font-medium text-muted-foreground transition-colors">
 Microsoft
 </span>
 </button>
 </div>
 ...
 <ShieldCheck className="w-16 h-16 text-orange-500 mx-auto mb-5" />
 <h4 className="text-white text-xl font-bold mb-3">{t("Enterprise Grade Security","اعلیٰ سیکیورٹی")}</h4>
 <p className="text-muted-foreground text-sm leading-relaxed mb-8">
 {t(
"To ensure the highest level of security and prevent unauthorized access, all new registrations must be authenticated directly through Google.",
"سیکیورٹی کو یقینی بنانے اور غیر مجاز رسائی کو روکنے کے لیے، نیا اکاؤنٹ بنانے کے لیے براہ کرم گوگل کا استعمال کریں۔"
 )}
 </p>
 <button
 type="button"
 onClick={handleFirebaseGoogleLogin}
 disabled={isLoading}
 className="w-full bg-card text-foreground hover:bg-muted font-bold py-4 px-4 rounded-xl flex items-center justify-center gap-3 transition-all shadow-lg disabled:opacity-50"
 >
 {isLoading ? (
 ...
 <span>{t("Continue with Google","گوگل کے ساتھ جاری رکھیں")}</span>
 </>
 )}
 </button>
 </div>
 ...
 <div className="mt-12 text-center border-t border-border pt-8 relative">
 ...
 {/* ── BOTTOM FOOTER ── */}
 <div className="bg-card border-t border-border py-4 px-6 lg:px-12 flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
 ...
 <div>
 <p className="font-bold text-foreground text-sm">Motorway Petroleum</p>
 <p className="text-xs text-muted-foreground">Mardan, Khyber Pakhtunkhwa<br/>Pakistan</p>
 </div>
 </div>
 ...
 <span className="text-sm text-muted-foreground font-medium">03168432329</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-full border border-border flex items-center justify-center shrink-0">
 <Mail className="h-4 w-4 text-orange-500" />
 </div>
 <span className="text-sm text-muted-foreground font-medium">info@motorwaypetroleum.com</span>
 </div>
 </div>
 ...
 <p className="font-bold text-foreground text-sm">24/7 Support</p>
 <p className="text-xs text-muted-foreground">We are always here<br/>to help you!</p>
 </div>
 </div>
 ...
 <div className="bg-card py-3 flex flex-col items-center justify-center border-t border-border relative z-10 gap-1.5">

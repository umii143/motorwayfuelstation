...
export default function HelpGuideModal({ isOpen, onClose, settings }: HelpGuideModalProps) {
 ...
 >
 {/* Header */}
 <div className="flex items-center justify-between border-b border-border bg-subtle px-6 py-4">
 ...
 <button
 onClick={onClose}
 className="flex h-8 w-8 items-center justify-center rounded-lg bg-card text-muted-foreground shadow-sm hover:bg-muted hover:text-slate-600 cursor-pointer"
 >
 <X className="h-4 w-4" />
 </button>
 </div>
 ...
 className="w-full rounded-xl border border-border bg-subtle py-3 pl-11 pr-4 font-sans text-sm font-semibold text-foreground focus:border-orange-500 focus:bg-card focus:outline-hidden transition-all"
 />
 ...
 : 'border-slate-100 dark:border-white/5 hover:border-border hover:bg-slate-50 dark:bg-card/5'
 }`}
 >
 <button
 ...
 >
 <span className="font-sans font-bold text-foreground">{section.title}</span>
 <div
 className={`flex h-6 w-6 items-center justify-center rounded-full transition-colors${
 isExpanded ? 'bg-orange-100 text-orange-600' : 'bg-muted text-slate-400'
 }`}
 >
 ...
 >
 <div className="border-t border-border px-4 pb-4 pt-3 font-sans text-sm leading-relaxed text-muted-foreground">
 {section.content}
 </div>
 </motion.div>
 )}

...
export default function AIDocumentScanner({ isOpen, onClose, settings, onDataExtracted, extractionPrompt }: AIDocumentScannerProps) {
 ...
 return (
 <div className="fixed inset-0 z-[100] flex items-center justify-center bg-card p-4 backdrop-blur-sm">
 <div className="w-full max-w-3xl overflow-hidden rounded-2xl bg-card shadow-2xl flex flex-col max-h-[90vh]">
 ...
 <button onClick={handleClose} className="rounded-xl p-2 hover:bg-card/20 transition-colors cursor-pointer">
 <X className="h-5 w-5" />
 </button>
 </div>

 {/* Content */}
 <div className="flex-1 overflow-y-auto p-6 bg-subtle flex flex-col md:flex-row gap-6">
 ...
 : 'border-border bg-muted hover:border-indigo-400 hover:bg-indigo-50'
 }`}
 >
 <input 
 type="file" 
 ...
 <div className="rounded-full bg-card p-4 shadow-xs">
 ...
 <button
 onClick={reset}
 className="rounded-xl border border-border bg-card px-4 py-3 font-sans text-sm font-bold text-foreground hover:bg-slate-50 transition-all cursor-pointer"
 >
 {t('Clear', 'صاف کریں')}
 </button>
 )}
 ...
 <div className="flex-1 rounded-2xl border border-border bg-card p-5 shadow-xs relative">
 {isScanning ? (
 <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-card/80 rounded-2xl">
 ...
 <p className="font-sans font-bold text-foreground">{t('Analyzing Document...', 'دستاویز کا تجزیہ جاری ہے...')}</p>
 <p className="font-sans text-xs text-muted-foreground mt-1">{t('Gemini Vision is extracting data fields', 'جیمنی ویژن ڈیٹا نکال رہا ہے')}</p>
 </div>
 </div>
 ) : extractedData ? (

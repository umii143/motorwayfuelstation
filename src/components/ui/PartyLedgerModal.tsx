...
export default function PartyLedgerModal({
 ...
 {/* ─── HEADER ─── */}
 <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-[var(--border-main)]${accentClasses.light}/40 print:hidden`}>
 ...
 ? `${accentClasses.bg}text-white` : 'bg-[var(--bg-card)] text-[var(--text-muted)] hover:text-[var(--text-main)] border border-[var(--border-main)]' }`}
 >
 {preset.label}
 </button>
 ))}
 ...
 className={`px-6 py-2${accentClasses.light}/40${accentClasses.text}hover:opacity-80 rounded-full font-bold text-sm transition-colors border${accentClasses.border}min-h-0 min-w-0`}
 >

...
export default function ScreenLock({
 ...
 return (
 <div className="fixed inset-0 z-[9999] bg-card backdrop-blur-xl flex flex-col items-center justify-center p-4">
 <div className="absolute inset-0 bg-[url('/station-bg.jpg')] bg-cover bg-center opacity-10 mix-blend-overlay pointer-events-none" />
 
 <div className={`relative w-full max-w-sm bg-card/10 backdrop-blur-2xl border border-border p-8 rounded-3xl shadow-2xl flex flex-col items-center text-center transition-transform${shake ? 'animate-shake' : ''}`}>
 ...
 <div 
 key={i} 
 className={`w-3.5 h-3.5 rounded-full transition-all duration-300${i < pin.length ? 'bg-card scale-110 shadow-[0_0_10px_rgba(255,255,255,0.8)]' : 'bg-card/20'}`}
 />
 ))}
 </div>
 ...
 className="h-16 rounded-2xl bg-card/5 hover:bg-card/15 active:bg-card/20 border border-border text-2xl font-semibold text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {num}
 </button>
 ))}
 ...
 className={`h-16 rounded-2xl flex items-center justify-center transition-all${biometricEnabled && !lockoutUntil ? 'bg-amber-500/20 text-amber-400 border-amber-500/30 hover:bg-amber-500/30' : 'bg-card/5 text-white/20 border-white/5'}`}
 >
 ...
 className="h-16 rounded-2xl bg-card/5 hover:bg-card/15 active:bg-card/20 border border-border flex items-center justify-center text-white/80 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
 >
